...
/**
 * @brief Map the node records onto the records of the result files
 */
...
static int cmp_global_res(const fstr_res_node_rec* r1,
                          const fstr_res_node_rec* r2) {
  return (r1->global > r2->global) - (r1->global < r2->global);
}

static int find_res_index(const fstr_res_node_rec* rec, int n, int global) {
  int left  = 0;
  int right = n - 1;
  ...
  while (left <= right) {
    ...
    if (rec[center].global < global) {
      left = center + 1;
    } else if (rec[center].global > global) {
      right = center - 1;
    } else {
      return rec[center].index;
    }
  }
  return -1;
}
...
int fstr_map_glt_to_result(fstr_glt* glt, fstr_res_info** res, int area_n) {
  int i, j, count, index, area;
  fstr_res_node_rec** rec;

  rec = HECMW_calloc(area_n, sizeof(*rec));
  if (!rec) return -1;

  for (i = 0; i < area_n; i++) {
    if (res[i]->nnode_gid == 0) continue;
    rec[i] = HECMW_malloc(res[i]->nnode_gid * sizeof(**rec));
    if (!rec[i]) {
      for (j = 0; j < i; j++) HECMW_free(rec[j]);
      HECMW_free(rec);
      return -1;
    }
    for (j = 0; j < res[i]->nnode_gid; j++) {
      rec[i][j].global = res[i]->node_gid[j];
      rec[i][j].index  = j;
    }
    qsort(rec[i], res[i]->nnode_gid, sizeof(**rec), (cmp_func)cmp_global_res);
  }

  count = 0;
  for (i = 0; i < glt->node_n; i++) {
    area  = glt->nrec[i].area;
    index = find_res_index(rec[area], res[area]->nnode_gid, glt->nrec[i].global);
    if (index < 0) continue;
    glt->nrec[count]       = glt->nrec[i];
    glt->nrec[count].local = index;
    count++;
  }

  if (count < glt->node_n) {
    fstr_out_log("%d nodes are not held by the result files and are skipped\n",
                 glt->node_n - count);
  }
  glt->node_n = count;

  for (i = 0; i < area_n; i++) HECMW_free(rec[i]);
  HECMW_free(rec);
