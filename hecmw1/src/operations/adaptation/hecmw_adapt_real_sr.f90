...
!C***
!C
      subroutine  hecmw_adapt_REAL_SEND_RECV                            &
     ...
     &                  WS, WR, X, SOLVER_COMM,my_rank, NB, m)
      ...
      real   (kind=kreal), dimension(NB*m), intent(inout):: WS
      real   (kind=kreal), dimension(NB*m), intent(inout):: WR
      real   (kind=kreal), dimension(NB*N), intent(inout):: X
...
!C-- SEND
      do neib= 1, NEIBPETOT
        ...
        do k= istart+1, istart+inum
          ...
          enddo
        enddo
        call MPI_ISEND (WS(NB*istart+1), NB*inum,MPI_DOUBLE_PRECISION,  &
...
!C-- RECEIVE
      do neib= 1, NEIBPETOT
        ...
        call MPI_IRECV (WR(NB*istart+1), NB*inum, MPI_DOUBLE_PRECISION, &
     &                  NEIBPE(neib), 0, SOLVER_COMM, req2(neib), ierr)
      ...
      end subroutine hecmw_adapt_REAL_SEND_RECV
      end module     hecmw_adapt_REAL_SR
