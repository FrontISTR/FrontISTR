...
void HECMW_dlb_get_string_item(char *para, char *buf, int *start_location,
                               char para2[128]) {
  int value;
  int i, j;
...
int HECMW_dlb_get_keyword_repart(char *buf, Control_para *ctl_para) {
  ...
  if (buf[i] != '!')
    HECMW_dlb_print_exit(
        "ERROR: HEC-MW-DLB-E0002: The control parameter format error: should "
        "start from !");
  i = i + 1;
  j = 0;
  while ((buf[i] != ' ') && (buf[i] != ',') && (buf[i] != '\n')) {
  ...
  flag = 1;
  if ((strncmp(para, "DLB_CTRL", 8) != 0) &&
      (strncmp(para, "dlb_ctrl", 8) != 0)) {
    flag = 0;
    return (flag);
  }
  /*	while((buf[i]!='=') && (buf[i]!='\n'))
                  i++;
          if(buf[i]=='\n')
                  HECMW_dlb_print_exit("ERROR: HEC-MW-DLB-E0006:The control
     parameter format error: DLB_METHOD");
          i++;
          while((buf[i]==' ') && (buf[i]!='\n'))
                  i++;
          if(buf[i]=='\n')
                  HECMW_dlb_print_exit("ERROR: HEC-MW-VIS-E0006:The control
     parameter format error: visual ID");
      j=0;
          while((buf[i]!=' ') && (buf[i]!=',') && (buf[i]!='=') &&
     (buf[i]!='\n')) {
                  para[j]=buf[i];
...
void hecmw_dlb_read_control(char *contfile, Control_para *ctl_para,
                            int stat_para[NUM_CONTROL_PARAS], int pesize) {
  int i, j, k, offset;
  ...
  while (cont_flag) {
    if (fgets(buf, MAX_LINE_LEN, fp) != NULL) {
      if ((HECMW_dlb_is_blank_line(buf) == 0) &&
          (HECMW_dlb_is_comment_line(buf) == 0))
        break;
    } else
      cont_flag = 0;
  }
  ...
  hit                           = 0;
  if (cont_flag == 1) cont_flag = HECMW_dlb_get_keyword_repart(buf, ctl_para);
  while (cont_flag == 1) {
    if (fgets(buf, MAX_LINE_LEN, fp) != NULL) {
      if ((HECMW_dlb_is_blank_line(buf) == 0) &&
          (HECMW_dlb_is_comment_line(buf) == 0))
  ...
  while (cont_flag == 1) {
    ...
    */
    if ((hit >= 0) && (hit < NUM_CONTROL_PARAS)) {
      switch (hit) {
        /*	  case 0:
                          sf[k].surface_style=get_int_item(para, buf,
           &location);
                          break;
                          */
        case 1:
        ...
        case 2:
          ctl_para->balance_rate =
              (float *)calloc(ctl_para->num_criteria, sizeof(float));
          for (i = 0; i < ctl_para->num_criteria; i++) {
            ctl_para->balance_rate[i] =
                (float)HECMW_dlb_get_double_item(para, buf, &location);
          }
          break;
        case 3:
          ctl_para->num_repartition =
              HECMW_dlb_get_int_item(para, buf, &location);
          break;
        case 4:
          ctl_para->itr_rate =
              (float)HECMW_dlb_get_double_item(para, buf, &location);
        ...
        case 5:
          ...
          break;
        case 6:
          HECMW_dlb_get_string_item(para, buf, &location,
                                    ctl_para->vwgt_filename);
          break;
        case 7:
          HECMW_dlb_get_string_item(para, buf, &location,
                                    ctl_para->adjwgt_filename);
          break;
        case 8:
          ctl_para->machine_wgt = (float *)calloc(pesize, sizeof(float));
          for (i = 0; i < pesize; i++) {
            ctl_para->machine_wgt[i] =
                (float)HECMW_dlb_get_double_item(para, buf, &location);
          }
          break;
          /*	  case 9:
  ...
  /*		  if(stat_para[0]==0) {
     ...
     (strncmp(ctl_para->adaptive_repartition, "off", 3)!=0)){
                            fprintf(stderr, "adaptive_repartition should be on
     or off\n");
  ...
  */
  ...
  if (stat_para[8] == 0) {
    ctl_para->machine_wgt = (float *)calloc(pesize, sizeof(float));
    for (i = 0; i < pesize; i++) ctl_para->machine_wgt[i] = 1.0 / (float)pesize;
  }
...
void hecmw_dlb_set_default_control(Control_para *ctl_para,
  ...
  ctl_para->itr_rate        = 10000.0;
  ctl_para->wgtflag         = 0;
  for (i = 6; i < NUM_CONTROL_PARAS; i++) stat_para[i] = 0;
  stat_para[8]                                         = 1;
  ctl_para->machine_wgt = (float *)calloc(pesize, sizeof(float));
  for (i = 0; i < pesize; i++) ctl_para->machine_wgt[i] = 1.0 / (float)pesize;
