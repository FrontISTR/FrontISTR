...
static int *count_shared_surf(const struct hecmw_couple_boundary *boundary) {
  ...
  for (i = 0; i < boundary->surf->n; i++) {
    for (j = boundary->elem_node_index[i]; j < boundary->elem_node_index[i + 1];
         j++) {
...
static int send_recv_n_shared_surf(
  ...
  rtc = HECMW_couple_intra_send_recv(
      intra_tbl->n_neighbor_pe, intra_tbl->neighbor_pe, intra_tbl->export_index,
      sendbuf, intra_tbl->import_index, recvbuf, HECMW_INT, intracomm->comm);
  if (rtc) goto error;
...
static int n2s_average_tet1(const struct hecmwST_local_mesh *mesh,
                            const struct hecmw_couple_boundary *boundary,
                            int id, int *n_shared_surf,
...
static int n2s_average_hex1(const struct hecmwST_local_mesh *mesh,
                            const struct hecmw_couple_boundary *boundary,
                            int id, int *n_shared_surf,
                            struct link_list *weight_list) {
  struct link_list *p;
...
extern int n2s_average(const struct hecmwST_local_mesh *mesh,
                       const struct hecmw_couple_boundary *boundary,
  ...
  for (i = 0; i < boundary->surf->n; i++) {
    ...
    if (mesh->elem_type[elem - 1] == HECMW_ETYPE_TET1) {
      if (n2s_average_tet1(mesh, boundary, i, n_shared_surf, weight_list))
        goto error;
    } else if (mesh->elem_type[elem - 1] == HECMW_ETYPE_HEX1) {
      if (n2s_average_hex1(mesh, boundary, i, n_shared_surf, weight_list))
        goto error;
    } else {
...
extern struct hecmw_couple_weight_list *HECMW_couple_n2s_average(
  ...
  if (n2s_average(mesh, boundary, intracomm, intra_tbl, weight_info))
    goto error;
