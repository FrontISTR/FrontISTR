...
static struct boundary_info_by_ctrl *boundary_info_current =
    &boundary_info_root;
...
static void free_link_list_s(struct link_list_s *r) {
...
static void free_link_list_i(struct link_list_i *r) {
...
extern void HECMW_couple_ctrl_print_unit(FILE *fp) {
...
extern void HECMW_couple_ctrl_print_couple(FILE *fp) {
...
extern void HECMW_couple_ctrl_print_boundary(FILE *fp) {
...
static struct couple_info_by_ctrl *get_couple_by_id(const char *couple_id) {
  struct couple_info_by_ctrl *p;
...
static struct unit_info_by_ctrl *get_unit_by_id(const char *unit_id) {
  struct unit_info_by_ctrl *p;
...
static struct boundary_info_by_ctrl *get_boundary_by_id(
...
extern void HECMW_couple_free_unit_ids(
...
extern void HECMW_couple_free_couple_ids(
...
extern void HECMW_couple_free_boundary_ids(
...
extern void HECMW_couple_ctrl_free_proc(
...
extern void HECMW_couple_ctrl_free_group(struct hecmw_couple_group *grp_info) {
  int i;
...
extern void HECMW_couple_ctrl_free(void) {
  free_unit();
  free_couple();
  free_boundary();
}
...
static int get_unit_nproc(int *n_proc) {
  int token;
...
static int get_unit_ranks(struct link_list_i *p, int *counter) {
  int token, rank;
  ...
  while (1) {
    ...
    if (rank < 0) {
      HECMW_set_error(HECMWCPL_E_CTRL_INVALID_TOKEN,
                      "line %d: Process number in '!COUPLE UNIT' must be "
                      "natural number (%d)",
...
static int get_unit_1st_line(struct unit_info_by_ctrl *p) {
  int token;
  ...
  if (is_specified_name == 0) {
    HECMW_set_error(HECMWCPL_E_CPLU_NO_NAME, "line %d",
                    HECMW_ctrllex_get_lineno() - 1);
    return -1;
  }
  /* NPROC is not specified */
  if (is_specified_nproc <= 0) {
    HECMW_set_error(HECMWCPL_E_CPLU_NO_NPROC, "line %d",
                    HECMW_ctrllex_get_lineno() - 1);
    return -1;
  }
...
static int get_unit_2nd_line(struct unit_info_by_ctrl *p) {
  int token, counter;
  struct link_list_i *q;
...
extern int HECMW_couple_ctrl_unit(void) {
  int token, size;
  struct unit_info_by_ctrl *p;
...
static int get_couple_type(int *type) {
  int token;
...
static int get_couple_1st_line(struct couple_info_by_ctrl *p) {
  int token;
  int is_specified_name  = 0;
  ...
  /* NAME is not specified */
  if (!is_specified_name) {
    HECMW_set_error(HECMWCPL_E_CPL_NO_NAME, "line %d",
                    HECMW_ctrllex_get_lineno() - 1);
    return -1;
  }
  /* TYPE is not specified */
  if (!is_specified_type) {
    HECMW_set_error(HECMWCPL_E_CPL_NO_TYPE, "line %d",
                    HECMW_ctrllex_get_lineno() - 1);
    return -1;
  }
  /* UNIT1 is not specified */
  if (!is_specified_unit1) {
    HECMW_set_error(HECMWCPL_E_CPL_NO_UNIT1, "line %d",
                    HECMW_ctrllex_get_lineno() - 1);
    return -1;
  }
  /* UNIT2 is not specified */
  if (!is_specified_unit2) {
    HECMW_set_error(HECMWCPL_E_CPL_NO_UNIT2, "line %d",
                    HECMW_ctrllex_get_lineno() - 1);
    return -1;
  }
...
extern int HECMW_couple_ctrl_couple(void) {
  int token, size;
  struct couple_info_by_ctrl *p;
...
static int get_boundary_direction(int *direction) {
  int token;
  ...
  if (token == HECMW_CTRLLEX_K_UNIT1_TO_UNIT2) { /* from UNIT1 to UNIT2 */
    *direction = HECMW_COUPLE_UNIT1_TO_UNIT2;
             ...
             HECMW_CTRLLEX_K_UNIT2_TO_UNIT1) { /* from UNIT2 to UNIT1 */
...
static int get_boundary_interpolation(int *interpolation) {
  int token;
...
static int get_boundary_tolerance(double *tolerance) {
  int token;
  ...
  *tolerance = (double)HECMW_ctrllex_get_number();
  if (*tolerance < 0.0) {
    HECMW_set_error(HECMWCPL_E_CTRL_INVALID_TOKEN,
                    "line %d: 'TOLERANCE' in '!COUPLE BOUNDARY' must be grater "
                    "than or equal 0 (%e)",
...
static int get_boundary_bbcoef(double *bbcoef) {
  int token;
...
static int get_boundary_bgcoef(double *bgcoef) {
  int token;
...
static int get_boundary_1st_line(struct boundary_info_by_ctrl *p) {
  int token;
  int is_specified_name          = 0;
  ...
  while (1) {
    ...
    if (token == HECMW_CTRLLEX_K_NAME) {
      ...
      if (get_boundary_interpolation(&p->interpolation) != HECMW_SUCCESS)
        return -1;
  ...
  /* NAME is not specified */
  if (!is_specified_name) {
    HECMW_set_error(HECMWCPL_E_CPLB_NO_NAME, "line %d",
                    HECMW_ctrllex_get_lineno() - 1);
    return -1;
  }
  /* COUPLE is not specified */
  if (!is_specified_couple) {
    HECMW_set_error(HECMWCPL_E_CPLB_NO_COUPLE, "line %d",
                    HECMW_ctrllex_get_lineno() - 1);
    return -1;
  }
  /* DIRECTION is not specified */
  if (!is_specified_direction) {
    HECMW_set_error(HECMWCPL_E_CPLB_NO_DIRECTION, "line %d",
                    HECMW_ctrllex_get_lineno() - 1);
    return -1;
  }
...
static int get_boundary_geom(int *geom_type) {
  int token;
...
static int get_boundary_data(int *data_type) {
  int token;
...
static int get_boundary_group_inner(struct link_list_s *p, int *counter) {
  char *s;
  int token;
...
static int get_boundary_unit_2nd_line(struct group_info_by_ctrl *p) {
  int token, counter;
  struct link_list_s *q;
  ...
  if (counter <= 0) {
    HECMW_set_error(HECMWCPL_E_CPLB_NO_GRPNAME, "line %d",
                    HECMW_ctrllex_get_lineno() - 1);
    return -1;
  }
...
static int get_boundary_unit_1st_line(struct group_info_by_ctrl *p) {
  int token;
  ...
  if (!is_specified_geom) {
    HECMW_set_error(HECMWCPL_E_CPLB_NO_GEOM, "line %d",
                    HECMW_ctrllex_get_lineno() - 1);
    return -1;
  }
  if (!is_specified_data) {
    HECMW_set_error(HECMWCPL_E_CPLB_NO_DATA, "line %d",
                    HECMW_ctrllex_get_lineno() - 1);
    return -1;
  }
  ...
  if (p->geom_type == HECMW_COUPLE_NODE_GROUP) {
    ...
    if (p->data_type != HECMW_COUPLE_ELEMENT_GROUP &&
        p->data_type != HECMW_COUPLE_NODE_GROUP) {
      HECMW_set_error(HECMWCPL_E_CPLB_UNMATCH_GRPTYPE,
                      "line %d: ELEMENT is specified in 'GEOM', but 'DATA' is "
                      "not NODE or ELEMENT",
                      HECMW_ctrllex_get_lineno() - 1);
      return -1;
    }
  } else if (p->geom_type == HECMW_COUPLE_SURFACE_GROUP) {
    if (p->data_type != HECMW_COUPLE_SURFACE_GROUP &&
        p->data_type != HECMW_COUPLE_NODE_GROUP) {
      HECMW_set_error(HECMWCPL_E_CPLB_UNMATCH_GRPTYPE,
                      "line %d: SURFACE is specified in 'GEOM', but 'DATA' is "
                      "not NODE or SURFACE",
                      HECMW_ctrllex_get_lineno() - 1);
      return -1;
    }
...
static int get_boundary_2nd_line(struct boundary_info_by_ctrl *p) {
  int token;
  int is_specified_unit1 = 0;
  int is_specified_unit2 = 0;
  ...
  if (!is_specified_unit1) {
    HECMW_set_error(HECMWCPL_E_CPLB_NO_UNIT1, "line %d",
                    HECMW_ctrllex_get_lineno());
    return -1;
  }
  if (!is_specified_unit2) {
    HECMW_set_error(HECMWCPL_E_CPLB_NO_UNIT2, "line %d",
                    HECMW_ctrllex_get_lineno());
    return -1;
  }
...
extern int HECMW_couple_ctrl_boundary(void) {
  int token, size;
  struct boundary_info_by_ctrl *p;
  ...
  boundary_info_current->next =
      (struct boundary_info_by_ctrl *)HECMW_malloc(size);
...
extern int HECMW_couple_ctrl_get_n_unit(void) { return n_unit; }

extern int HECMW_couple_ctrl_get_n_couple(void) { return n_couple; }

extern int HECMW_couple_ctrl_get_n_boundary(void) { return n_boundary; }

extern struct hecmw_couple_ctrl_unit_ids *HECMW_couple_get_unit_ids(void) {
  int size, i, n;
  struct unit_info_by_ctrl *p;
...
extern struct hecmw_couple_ctrl_couple_ids *HECMW_couple_get_couple_ids(void) {
  int size, i, n;
  struct couple_info_by_ctrl *p;
...
extern struct hecmw_couple_ctrl_boundary_ids *HECMW_couple_get_boundary_ids(
    void) {
  int size, i, n;
  struct boundary_info_by_ctrl *p;
...
extern char *HECMW_couple_ctrl_get_unit_id(const char *couple_id,
  ...
  if (couple_id == NULL) {
    HECMW_set_error(HECMWCPL_E_INVALID_ARG,
                    "Invalid NULL pointer is found (couple_id)");
    return NULL;
  }
...
extern char *HECMW_couple_ctrl_get_couple_id(const char *boundary_id, char *buf,
  ...
  if (boundary_id == NULL) {
    HECMW_set_error(HECMWCPL_E_INVALID_ARG,
                    "Invalid NULL pointer is found (boundary_id)");
    return NULL;
  }
...
extern struct hecmw_couple_ctrl_proc *HECMW_couple_ctrl_get_proc(
    const char *unit_id) {
  int size, n;
  struct hecmw_couple_ctrl_proc *proc_info;
  struct unit_info_by_ctrl *p;
  ...
  if (unit_id == NULL) {
    HECMW_set_error(HECMWCPL_E_INVALID_ARG,
                    "Invalid NULL pointer is found (unit_id)");
    return NULL;
  }
...
extern int HECMW_couple_ctrl_get_type(const char *couple_id, int *couple_type) {
  struct couple_info_by_ctrl *p;
...
extern int HECMW_couple_ctrl_get_direction(const char *boundary_id,
                                           int *direction) {
  struct boundary_info_by_ctrl *p;
...
extern int HECMW_couple_ctrl_get_tolerance(const char *boundary_id,
                                           double *tolerance) {
  struct boundary_info_by_ctrl *p;
...
extern int HECMW_couple_ctrl_get_bbcoef(const char *boundary_id,
                                        double *bbcoef) {
  struct boundary_info_by_ctrl *p;
...
extern int HECMW_couple_ctrl_get_bgcoef(const char *boundary_id,
                                        double *bgcoef) {
  struct boundary_info_by_ctrl *p;
...
extern struct hecmw_couple_group *HECMW_couple_ctrl_get_group(
  ...
  if ((unit_specifier != HECMW_COUPLE_UNIT1) &&
      (unit_specifier != HECMW_COUPLE_UNIT2)) {
    HECMW_set_error(HECMWCPL_E_INVALID_ARG,
