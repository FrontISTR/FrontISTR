...
static int intercomm_d2s_coord(
  ...
  if (comm_dst->is_member) {
    ...
    size = sizeof(double) *
           (inter_tbl->import_index[inter_tbl->n_neighbor_pe_import] * 3 + 1);
  ...
  if (comm_src->is_member) {
    ...
    size = sizeof(double) *
           (inter_tbl->export_index[inter_tbl->n_neighbor_pe_export] * 3 + 1);
...
static int s2n_dist_node_tet1(const struct hecmwST_local_mesh *mesh_src,
...
static int s2n_dist_node_hex1(const struct hecmwST_local_mesh *mesh_src,
                              const struct hecmw_couple_boundary *boundary_src,
                              int id,
                              const struct hecmw_couple_vertex *coord_dst,
                              struct link_list *weight_list) {
...
static int s2n_dist_node(const struct hecmwST_local_mesh *mesh_src,
                         const struct hecmw_couple_boundary *boundary_src,
                         const struct hecmw_couple_inter_iftable *inter_tbl,
                         double *coord,
  ...
  for (i = 0; i < inter_tbl->export_index[inter_tbl->n_neighbor_pe_export];
    ...
    if (mesh_src->elem_type[elem - 1] == HECMW_ETYPE_TET1) {
      rtc = s2n_dist_node_tet1(mesh_src, boundary_src, id, &coord_dst,
                               &weight_list[i]);
      if (rtc != HECMW_SUCCESS) goto error;
    } else if (mesh_src->elem_type[elem - 1] == HECMW_ETYPE_HEX1) {
      rtc = s2n_dist_node_hex1(mesh_src, boundary_src, id, &coord_dst,
                               &weight_list[i]);
      if (rtc != HECMW_SUCCESS) goto error;
    } else {
...
extern struct hecmw_couple_weight_list *HECMW_couple_s2n_dist_node(
  ...
  if (comm_src->is_member) {
    ...
    if (s2n_dist_node(mesh_src, boundary_src, inter_tbl, coord, weight_info))
      goto error;
  }
