...
extern struct hecmw_couple_value *HECMW_couple_alloc_couple_value(void) {
  ...
  p = (struct hecmw_couple_value *)HECMW_malloc(
      sizeof(struct hecmw_couple_value));
  if (p == NULL) {
...
extern void HECMW_couple_print_couple_value(
  ...
  for (i = 0; i < couple_value->n; i++) {
    if (couple_value->item_type == HECMW_COUPLE_SURFACE_GROUP) {
      fprintf(fp, " %d %d", couple_value->item[2 * i],
              couple_value->item[2 * i + 1]);
    } else {
...
extern struct hecmw_couple_value *HECMW_couple_startup(
  ...
  if (boundary_id == NULL) {
    HECMW_set_error(HECMWCPL_E_INVALID_ARG,
                    "HECMW_couple_startup(): 'boundary_id' is NULL");
    return NULL;
  }
  ...
  if (couple_info->comm_src->is_member) {
    ...
    if (boundary->data_type == HECMW_COUPLE_NODE_GROUP) {
      ...
      if (couple_value->n > 0) {
        couple_value->item =
            (int *)HECMW_malloc(sizeof(int) * couple_value->n * 2);
        if (couple_value->item == NULL) {
          HECMW_set_error(errno, "");
          goto error;
        }
