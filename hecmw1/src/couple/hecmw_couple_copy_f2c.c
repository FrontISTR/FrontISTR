...
static int set_n(void *src) {
  couple_value->n = *((int *)src);
  return 0;
}

static int set_item_type(void *src) {
...
extern int HECMW_couple_copy_f2c_init(
    struct hecmw_couple_value *_couple_value) {
  couple_value = _couple_value;
  return 0;
}

extern int HECMW_couple_copy_f2c_finalize(void) {
  couple_value = NULL;
  return 0;
}
...
extern void hecmw_cpl_copy_f2c_set_if(char *struct_name, char *var_name,
...
extern void hecmw_cpl_copy_f2c_set_if_(char *struct_name, char *var_name,
                                       void *src, int *err, int slen,
                                       int vlen) {
  hecmw_cpl_copy_f2c_set_if(struct_name, var_name, src, err, slen, vlen);
}

extern void hecmw_cpl_copy_f2c_set_if__(char *struct_name, char *var_name,
                                        void *src, int *err, int slen,
                                        int vlen) {
  hecmw_cpl_copy_f2c_set_if(struct_name, var_name, src, err, slen, vlen);
}
