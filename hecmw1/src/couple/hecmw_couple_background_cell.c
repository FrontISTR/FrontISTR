...
static struct hecmw_couple_background_cell *alloc_struct_bgcell(void) {
...
static int elem_size_by_elem(const struct hecmwST_local_mesh *mesh,
...
static int elem_size_by_surf(const struct hecmwST_local_mesh *mesh,
                             const struct hecmw_couple_boundary *boundary,
                             double *min_dx, double *min_dy, double *min_dz,
                             double *max_dx, double *max_dy, double *max_dz) {
  double min_x, min_y, min_z, max_x, max_y, max_z, dx, dy, dz, coord_x, coord_y,
...
extern struct hecmw_couple_background_cell *HECMW_couple_set_background_cell(
  ...
  if (HECMW_couple_ctrl_get_bgcoef(boundary_id, &bgcell->coef) != HECMW_SUCCESS)
    goto error;

  if (boundary->geom_type ==
    ...
    goto error;
    /*		elem_size_by_elem(mesh, boundary, &min_dx, &min_dy, &min_dz, &max_dx,
     * &max_dy, &max_dz); */

  } else if (boundary->geom_type ==
             HECMW_COUPLE_SURFACE_GROUP) { /* surface group	*/
    elem_size_by_surf(mesh, boundary, &min_dx, &min_dy, &min_dz, &max_dx,
                      &max_dy, &max_dz);
