...
static void free_couple_info(struct hecmw_couple_info *p) {
  if (p == NULL) return;
...
static void free_couple_list(struct couple_info *p) {
  if (p == NULL) return;
...
static struct hecmw_couple_info *alloc_couple_info(void) {
  struct hecmw_couple_info *p = NULL;

  p = (struct hecmw_couple_info *)HECMW_malloc(
      sizeof(struct hecmw_couple_info));
  if (p == NULL) {
...
static struct couple_info *alloc_couple_list(void) {
...
static struct couple_info *cmp_couple_list(const char *boundary_id) {
...
static void del_couple_list(const char *boundary_id) {
...
static int add_couple_list(const char *boundary_id, int unit_specifier_src,
...
extern struct hecmw_couple_info *HECMW_couple_get_info(
    const char *boundary_id) {
  struct couple_info *p;

  if (boundary_id == NULL) {
    HECMW_set_error(HECMWCPL_E_INVALID_ARG,
                    "HECMW_couple_get_info(): 'boundary_id' is NULL");
    return NULL;
  }
...
extern void HECMW_couple_free_init(const char *boundary_id) {
  del_couple_list(boundary_id);
}

extern int HECMW_couple_init(const char *boundary_id,
  ...
  struct hecmw_couple_comm *comm_src = NULL, *comm_dst = NULL,
                           *intercomm                = NULL;
  struct hecmw_couple_intra_iftable *intra_tbl_src   = NULL,
                                    *intra_tbl_dst   = NULL;
  ...
  if (boundary_id == NULL) {
    HECMW_set_error(HECMWCPL_E_INVALID_ARG,
                    "HECMW_couple_init(): 'boundary_id' is NULL");
    return HECMW_ERROR;
  }
  ...
  if (is_unit1_memb && mesh_unit1 == NULL) {
    HECMW_set_error(HECMWCPL_E_INVALID_ARG,
                    "HECMW_couple_init(): 'mesh_unit1' is NULL");
    return HECMW_ERROR;
  }
  if (is_unit2_memb && mesh_unit2 == NULL) {
    HECMW_set_error(HECMWCPL_E_INVALID_ARG,
                    "HECMW_couple_init(): 'mesh_unit2' is NULL");
    return HECMW_ERROR;
  }
  ...
  if (direction == HECMW_COUPLE_UNIT1_TO_UNIT2) { /* UNIT1 -> UNIT2	*/
    ...
    mesh_dst           = mesh_unit2;
             ...
             HECMW_COUPLE_UNIT2_TO_UNIT1) { /* UNIT2 -> UNIT1	*/
  ...
  if (HECMW_couple_get_unit_id(boundary_id, unit_specifier_src, src_unit_id,
                               HECMW_NAME_LEN + 1) == NULL)
    goto error;
  if (HECMW_couple_get_unit_id(boundary_id, unit_specifier_dst, dst_unit_id,
                               HECMW_NAME_LEN + 1) == NULL)
    goto error;
  if ((comm_src =
           HECMW_couple_get_intracomm(boundary_id, unit_specifier_src)) == NULL)
    goto error;
  if ((comm_dst =
           HECMW_couple_get_intracomm(boundary_id, unit_specifier_dst)) == NULL)
    goto error;
  if ((intercomm = HECMW_couple_get_intercomm(boundary_id)) == NULL) goto error;

  if (is_src_memb) {
    boundary_src = HECMW_couple_set_boundary_info(boundary_id,
                                                  unit_specifier_src, mesh_src);
    if (boundary_src == NULL) goto error;

    bbox_src =
        HECMW_couple_set_bounding_box(boundary_id, mesh_src, boundary_src);
    if (bbox_src == NULL) goto error;

    bgcell_src = HECMW_couple_set_background_cell(boundary_id, mesh_src,
                                                  bbox_src, boundary_src);
    if (bgcell_src == NULL) goto error;

    intra_tbl_src =
        HECMW_couple_make_intra_iftable(mesh_src, boundary_src, comm_src);
    if (intra_tbl_src == NULL) goto error;
  }

  if (is_dst_memb) {
    boundary_dst = HECMW_couple_set_boundary_info(boundary_id,
                                                  unit_specifier_dst, mesh_dst);
    if (boundary_dst == NULL) goto error;

    bbox_dst =
        HECMW_couple_set_bounding_box(boundary_id, mesh_dst, boundary_dst);
    if (bbox_dst == NULL) goto error;

    bgcell_dst = HECMW_couple_set_background_cell(boundary_id, mesh_dst,
                                                  bbox_dst, boundary_dst);
    if (bgcell_dst == NULL) goto error;

    mapped_point_dst =
        HECMW_couple_set_mapped_point(boundary_id, mesh_dst, boundary_dst);
    if (mapped_point_dst == NULL) goto error;

    intra_tbl_dst =
        HECMW_couple_make_intra_iftable(mesh_dst, boundary_dst, comm_dst);
    if (intra_tbl_dst == NULL) goto error;
  }

  /* make interface table for inter-communication */
  inter_tbl = HECMW_couple_set_map_data(mesh_src, mesh_dst, comm_src, comm_dst,
                                        intercomm, boundary_src, bbox_src,
                                        bbox_dst, bgcell_src, mapped_point_dst);
  if (inter_tbl == NULL) goto error;

  /* make weight list */
  if (comm_src->is_member) {
    ip_list_pre = HECMW_couple_make_pre_ip_list(mesh_src, boundary_src,
                                                comm_src, intra_tbl_src);
    if (ip_list_pre == NULL) goto error;
  }

  ip_list_main = HECMW_couple_make_main_ip_list(
      mesh_src, mesh_dst, boundary_src, boundary_dst, mapped_point_dst,
      comm_src, comm_dst, intercomm, inter_tbl);
  if (ip_list_main == NULL) goto error;

  if (comm_dst->is_member) {
    ip_list_post = HECMW_couple_make_post_ip_list(
        mesh_dst, boundary_dst, mapped_point_dst, comm_dst, intra_tbl_dst);
  ...
  if (add_couple_list(boundary_id, unit_specifier_src, unit_specifier_dst,
                      ...
                      ip_list_pre, ip_list_main, ip_list_post))
    goto error;
