...
static void free_link_list_map(struct link_list_map *r) {
...
static struct map_info *alloc_struct_map_info(void) {
...
static void free_struct_map_info(struct map_info *p) {
  if (p == NULL) return;
...
static int bcast_mapped_point_d2s_n(
   ...
   * broadcasting
   */
  rtc = HECMW_couple_bcast(n_pe_send, pe_send, sendbuf_size, sendbuf, n_pe_recv,
                           pe_recv, recvbuf_index, recvbuf, HECMW_INT,
                           intercomm->comm);
  if (rtc != 0) goto error;
...
static int bcast_mapped_point_d2s_coord(
    const struct hecmw_couple_mapped_point *mapped_point,
    const struct hecmw_couple_comm *comm_src,
    const struct hecmw_couple_comm *comm_dst,
    const struct hecmw_couple_comm *intercomm,
  ...
  if (comm_src->is_member) {
    ...
    /* receive buffer */
    recvbuf =
        (double *)HECMW_malloc(sizeof(double) * (recvbuf_index[n_pe_recv] + 1));
    if (recvbuf == NULL) {
      HECMW_set_error(errno, "");
      goto error;
    }
  ...
  /* broadcast */
  rtc = HECMW_couple_bcast(n_pe_send, pe_send, sendbuf_size, sendbuf, n_pe_recv,
                           pe_recv, recvbuf_index, recvbuf, HECMW_DOUBLE,
                           intercomm->comm);
  if (rtc != 0) goto error;
  ...
  if (comm_src->is_member) {
    for (i = 0; i < comm_dst->psize; i++) {
      ...
      all_mapped_point[i]->coord =
          (double *)HECMW_malloc(sizeof(double) * size);
...
static int bcast_mapped_point_d2s(
    const struct hecmw_couple_mapped_point *mapped_point,
    const struct hecmw_couple_comm *comm_src,
    const struct hecmw_couple_comm *comm_dst,
    const struct hecmw_couple_comm *intercomm,
  ...
  if (bcast_mapped_point_d2s_n(mapped_point, comm_src, comm_dst, intercomm,
                               all_mapped_point))
    goto error;
  if (bcast_mapped_point_d2s_coord(mapped_point, comm_src, comm_dst, intercomm,
                                   all_mapped_point))
    goto error;
...
static int bcast_bbox_d2s(const struct hecmw_couple_bounding_box *bbox_dst,
                          const struct hecmw_couple_comm *comm_src,
                          const struct hecmw_couple_comm *comm_dst,
                          const struct hecmw_couple_comm *intercomm,
  ...
  if (comm_src->is_member) {
    for (i = 0; i < comm_dst->psize; i++) {
      HECMW_assert(recvbuf_index[i + 1] - recvbuf_index[i] ==
                   N_BOUNDING_BOX_MEMBER);
...
static int check_bbox_within_bbox(const struct hecmw_couple_box *box_src,
...
static int check_node_within_bbox(
    const struct hecmw_couple_box *box_src,
...
static int set_candidate_node(
  ...
  if (bcast_bbox_d2s(bbox_dst, comm_src, comm_dst, intercomm, all_dst_bbox))
    goto error;
  ...
  if (comm_src->is_member) {
    for (i = 0; i < comm_dst->psize; i++) {
      ...
      is_candidate[i] =
          (int *)HECMW_calloc(all_mapped_point[i]->n, sizeof(int));
      ...
      if (check_bbox_within_bbox(bbox_src->enlarged, &all_dst_bbox[i])) {
        check_node_within_bbox(bbox_src->enlarged, all_mapped_point[i],
                               is_candidate[i]);
      }
    }
  }
...
static struct map_info_to_bgcell *map_node_to_bgcell(
  ...
  /* linked list */
  bgcell_node_list = (struct link_list *)HECMW_malloc(sizeof(struct link_list) *
                                                      bgcell_src->n);
...
static struct map_info_to_bgcell *map_elem_to_bgcell(
    const struct hecmwST_local_mesh *mesh_src,
    const struct hecmw_couple_boundary *boundary_src,
    const struct hecmw_couple_bounding_box *bbox_src,
    const struct hecmw_couple_background_cell *bgcell_src) {
  ...
  /* linked list */
  bgcell_elem_list = (struct link_list *)HECMW_malloc(sizeof(struct link_list) *
                                                      bgcell_src->n);
  ...
  for (ic = 0, i = 0; i < boundary_src->elem->n; i++) {
    ...
    for (j = mesh_src->elem_node_index[elem - 1];
         j < mesh_src->elem_node_index[elem]; j++) {
...
static struct map_info_to_bgcell *map_surf_to_bgcell(
    const struct hecmwST_local_mesh *mesh_src,
    const struct hecmw_couple_boundary *boundary_src,
    const struct hecmw_couple_bounding_box *bbox_src,
    const struct hecmw_couple_background_cell *bgcell_src) {
  ...
  /* linked list */
  bgcell_surf_list = (struct link_list *)HECMW_malloc(sizeof(struct link_list) *
                                                      bgcell_src->n);
...
static int compare_mapping_info(int n_positive_old, double dot_product_old,
                                double distance_old, int n_positive_new,
                                double dot_product_new, double distance_new) {
  /*
...
static int map_point_to_surf(
    const struct hecmwST_local_mesh *mesh_src,
    const struct hecmw_couple_boundary *boundary_src,
    const struct hecmw_couple_bounding_box *bbox_src,
    const struct hecmw_couple_background_cell *bgcell_src,
    const struct hecmw_couple_mapped_point *mapped_point, int *is_candidate,
    const struct map_info_to_bgcell *map_to_bgcell, struct map_info *map_src) {
  double coord_x, coord_y, coord_z;
  int depth_x_before, depth_y_before, depth_z_before, depth_x_after,
      depth_y_after, depth_z_after;
  double dot_product, distance;
  int n_positive, elem, surf, bgcell, surf_id, nl, nm, nn, rtc, jstart, jfinal,
      i, j, l, m, n;
  ...
  for (i = 0; i < mapped_point->n; i++) {
    ...
    /* mapping */
    for (j = map_to_bgcell->index[bgcell]; j < map_to_bgcell->index[bgcell + 1];
         j++) {
      ...
      if (mesh_src->elem_type[elem - 1] == HECMW_ETYPE_TET1) {
        n_positive =
            HECMW_couple_judge_tet1(mesh_src, elem, surf, coord_x, coord_y,
                                    coord_z, &dot_product, &distance);
      } else if (mesh_src->elem_type[elem - 1] == HECMW_ETYPE_HEX1) {
        n_positive =
            HECMW_couple_judge_hex1(mesh_src, elem, surf, coord_x, coord_y,
                                    coord_z, &dot_product, &distance);
      } else {
      ...
      if (n_positive >= 0) {
        rtc = compare_mapping_info(
            map_src->n_positive[i], map_src->dot_product[i],
...
static int mapping_in_src(const struct hecmwST_local_mesh *mesh_src,
                          const struct hecmw_couple_boundary *boundary_src,
                          const struct hecmw_couple_bounding_box *bbox_src,
                          const struct hecmw_couple_background_cell *bgcell_src,
                          const struct hecmw_couple_comm *comm_dst,
  ...
  /* mapping boundary nodes on destination side to boundary surface on source
   * side */
  map_type = HECMW_COUPLE_MAP_SURF_TO_SURF; /*@@*/

  if (map_type == HECMW_COUPLE_MAP_SURF_TO_SURF) {
    map_to_bgcell =
        map_surf_to_bgcell(mesh_src, boundary_src, bbox_src, bgcell_src);
    if (map_to_bgcell == NULL) goto error;

    for (i = 0; i < comm_dst->psize; i++) {
      if (map_point_to_surf(mesh_src, boundary_src, bbox_src, bgcell_src,
                            all_mapped_point[i], is_candidate[i], map_to_bgcell,
                            map_src[i]))
        goto error;
    }
...
static int gather_id_s2d(struct map_info **map_src,
                         const struct hecmw_couple_comm *comm_src,
                         const struct hecmw_couple_comm *comm_dst,
                         const struct hecmw_couple_comm *intercomm,
                         struct map_info **map_dst) {
  int n_send_pe = 0, n_recv_pe = 0, *send_pe = NULL, *recv_pe = NULL;
  int *sendbuf_index = NULL, *recvbuf_index = NULL, *sendbuf = NULL,
      *recvbuf = NULL;
  int rtc, i, j;
  ...
  rtc = HECMW_couple_inter_send_recv(n_send_pe, send_pe, sendbuf_index, sendbuf,
                                     n_recv_pe, recv_pe, recvbuf_index, recvbuf,
...
static int gather_n_positive_s2d(struct map_info **map_src,
                                 const struct hecmw_couple_comm *comm_src,
                                 const struct hecmw_couple_comm *comm_dst,
                                 const struct hecmw_couple_comm *intercomm,
                                 struct map_info **map_dst) {
...
static int gather_dot_product_s2d(struct map_info **map_src,
                                  const struct hecmw_couple_comm *comm_src,
                                  const struct hecmw_couple_comm *comm_dst,
                                  const struct hecmw_couple_comm *intercomm,
                                  struct map_info **map_dst) {
  ...
  if (comm_src->is_member) {
    ...
    /* send buffer */
    sendbuf =
        (double *)HECMW_malloc(sizeof(double) * (sendbuf_index[n_send_pe] + 1));
    if (sendbuf == NULL) {
      HECMW_set_error(errno, "");
      goto error;
    }
  ...
  if (comm_dst->is_member) {
    ...
    /* receive buffer */
    recvbuf =
        (double *)HECMW_malloc(sizeof(double) * recvbuf_index[n_recv_pe] + 1);
    if (recvbuf == NULL) {
      HECMW_set_error(errno, "");
      goto error;
    }
  ...
  if (comm_dst->is_member) {
    for (i = 0; i < n_recv_pe; i++) {
      map_dst[i]->dot_product =
          (double *)HECMW_malloc(sizeof(double) * map_dst[i]->n);
...
static int gather_distance_s2d(struct map_info **map_src,
                               const struct hecmw_couple_comm *comm_src,
                               const struct hecmw_couple_comm *comm_dst,
                               const struct hecmw_couple_comm *intercomm,
                               struct map_info **map_dst) {
...
static int gather_map_data_s2d(
    const struct hecmw_couple_mapped_point *mapped_point,
    struct map_info **map_src, const struct hecmw_couple_comm *comm_src,
    const struct hecmw_couple_comm *comm_dst,
  ...
  if (map_type == HECMW_COUPLE_MAP_NODE_TO_SURF) {
    if (gather_id_s2d(map_src, comm_src, comm_dst, intercomm, map_dst))
      goto error;
    if (gather_n_positive_s2d(map_src, comm_src, comm_dst, intercomm, map_dst))
      goto error;
    if (gather_dot_product_s2d(map_src, comm_src, comm_dst, intercomm, map_dst))
      goto error;
    if (gather_distance_s2d(map_src, comm_src, comm_dst, intercomm, map_dst))
      goto error;
...
static struct link_list_map *set_mapping_surf(
    const struct hecmw_couple_mapped_point *mapped_point,
  ...
  for (n = 0, i = 0; i < mapped_point->n; i++) {
    ...
    for (j = 0; j < comm_src->psize; j++) {
      if (map_dst[j]->n_positive[i] >= 0) {
        rtc = compare_mapping_info(
            _n_positive, _dot_product, _distance, map_dst[j]->n_positive[i],
            map_dst[j]->dot_product[i], map_dst[j]->distance[i]);
        if (rtc == 1) {
          pe_index     = j;
...
static int set_n_neighbor_pe_import(
    struct hecmw_couple_inter_iftable *inter_tbl,
    const struct hecmw_couple_comm *comm_src,
    struct link_list_map *mapping_data_list) {
  int n, i;
...
static int set_neighbor_pe_import(struct hecmw_couple_inter_iftable *inter_tbl,
                                  const struct hecmw_couple_comm *comm_src,
                                  struct link_list_map *mapping_data_list) {
  int size, n, i;
...
static int set_import_index(struct hecmw_couple_inter_iftable *inter_tbl,
                            const struct hecmw_couple_comm *comm_src,
                            struct link_list_map *mapping_data_list) {
...
static int set_import_item(struct hecmw_couple_inter_iftable *inter_tbl,
                           const struct hecmw_couple_comm *comm_src,
                           struct link_list_map *mapping_data_list) {
  struct link_list_map *p;
  int size, n, i;
...
static int *set_export_data(struct hecmw_couple_inter_iftable *inter_tbl,
                            const struct hecmw_couple_comm *comm_src,
                            struct link_list_map *mapping_data_list) {
  struct link_list_map *p;
...
static int set_neighbor_info_import(
    struct hecmw_couple_inter_iftable *inter_tbl,
    struct link_list_map *mapping_data_list,
    const struct hecmw_couple_comm *comm_src) {
  int i;
  ...
  if (set_n_neighbor_pe_import(inter_tbl, comm_src, mapping_data_list))
    return -1;
...
static int set_neighbor_pe_export(
    struct hecmw_couple_inter_iftable *inter_tbl,
    const struct hecmw_couple_comm *comm_src,
    const struct hecmw_couple_comm *comm_dst,
    const struct hecmw_couple_comm *intercomm,
    const struct link_list_map *mapping_data_list) {
...
static int set_export_index(struct hecmw_couple_inter_iftable *inter_tbl,
                            const struct hecmw_couple_comm *comm_src,
                            const struct hecmw_couple_comm *comm_dst,
                            const struct hecmw_couple_comm *intercomm) {
  int *sendbuf_index = NULL, *recvbuf_index = NULL, *sendbuf = NULL,
  ...
  if (comm_dst->is_member) {
    ...
    /* send buffer */
    sendbuf = (int *)HECMW_malloc(sizeof(int) *
                                  (inter_tbl->n_neighbor_pe_import + 1));
    if (sendbuf == NULL) {
      HECMW_set_error(errno, "");
      goto error;
    }
  ...
  if (comm_src->is_member) {
    ...
    /* receive buffer */
    recvbuf = (int *)HECMW_malloc(sizeof(int) *
                                  (inter_tbl->n_neighbor_pe_export + 1));
    if (recvbuf == NULL) {
      HECMW_set_error(errno, "");
      goto error;
    }
  ...
  if (inter_tbl->n_neighbor_pe_import == 0 &&
      inter_tbl->n_neighbor_pe_export == 0)
  ...
  rtc = HECMW_couple_inter_send_recv(
      ...
      inter_tbl->neighbor_pe_export, recvbuf_index, recvbuf, HECMW_INT,
      intercomm->comm);
  if (rtc != 0) goto error;
  ...
  if (comm_src->is_member) {
    ...
    if (inter_tbl->n_neighbor_pe_export != 0) {
      ...
      for (i = 0; i < inter_tbl->n_neighbor_pe_export; i++) {
        inter_tbl->export_index[i + 1] =
            inter_tbl->export_index[i] + recvbuf[i];
      }
    }
  }
...
static int set_export_item(struct hecmw_couple_inter_iftable *inter_tbl,
                           const struct hecmw_couple_comm *comm_src,
                           const struct hecmw_couple_comm *comm_dst,
                           const struct hecmw_couple_comm *intercomm,
                           int *export_data) {
  ...
  if (comm_src->is_member) {
    /* receive buffer */
    size = sizeof(int) *
           (inter_tbl->export_index[inter_tbl->n_neighbor_pe_export] + 1);
  ...
  rtc = HECMW_couple_inter_send_recv(
      inter_tbl->n_neighbor_pe_import, inter_tbl->neighbor_pe_import,
      inter_tbl->import_index, export_data, inter_tbl->n_neighbor_pe_export,
      inter_tbl->neighbor_pe_export, inter_tbl->export_index,
      inter_tbl->export_item, HECMW_INT, intercomm->comm);
  if (rtc != 0) goto error;
...
static struct hecmw_couple_inter_iftable *set_inter_iftable(
    const struct hecmw_couple_comm *comm_src,
    const struct hecmw_couple_comm *comm_dst,
    const struct hecmw_couple_comm *intercomm, struct map_info **map_dst,
    const struct hecmw_couple_mapped_point *mapped_point) {
  ...
  /* export side */
  rtc = set_neighbor_pe_export(inter_tbl, comm_src, comm_dst, intercomm,
                               mapping_data_list);
  if (rtc != HECMW_SUCCESS) goto error;
...
extern struct hecmw_couple_inter_iftable *HECMW_couple_set_map_data(
  ...
  rtc = bcast_mapped_point_d2s(mapped_point, comm_src, comm_dst, intercomm,
                               all_mapped_point);
  if (rtc != HECMW_SUCCESS) goto error;
  ...
  /* */
  if (comm_src->is_member) {
    map_src = (struct map_info **)HECMW_malloc(sizeof(struct map_info *) *
                                               comm_dst->psize);
    if (map_src == NULL) {
      HECMW_set_error(errno, "");
      goto error;
  ...
  if (comm_dst->is_member) {
    map_dst = (struct map_info **)HECMW_malloc(sizeof(struct map_info *) *
                                               comm_dst->psize);
    if (map_dst == NULL) {
      HECMW_set_error(errno, "");
      goto error;
  ...
  rtc = gather_map_data_s2d(mapped_point, map_src, comm_src, comm_dst,
                            intercomm, map_dst);
  if (rtc != HECMW_SUCCESS) goto error;

  /* construct interface table for inter-communicate */
  inter_tbl =
      set_inter_iftable(comm_src, comm_dst, intercomm, map_dst, mapped_point);
  if (inter_tbl == NULL) goto error;
