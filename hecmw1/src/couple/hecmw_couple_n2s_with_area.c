...
static int n2s_with_area_tet1(const struct hecmwST_local_mesh *mesh,
...
static int n2s_with_area_hex1(const struct hecmwST_local_mesh *mesh,
                              const struct hecmw_couple_boundary *boundary,
                              int id, struct link_list *weight_list) {
...
static int n2s_with_area(const struct hecmwST_local_mesh *mesh,
                         const struct hecmw_couple_boundary *boundary,
  ...
  for (i = 0; i < boundary->surf->n; i++) {
    ...
    if (mesh->elem_type[elem - 1] == HECMW_ETYPE_TET1) {
      if (n2s_with_area_tet1(mesh, boundary, i, weight_list) != HECMW_SUCCESS)
        goto error;
    } else if (mesh->elem_type[elem - 1] == HECMW_ETYPE_HEX1) {
      if (n2s_with_area_hex1(mesh, boundary, i, weight_list) != HECMW_SUCCESS)
        goto error;
    } else {
...
extern struct hecmw_couple_weight_list *HECMW_couple_n2s_with_area(
