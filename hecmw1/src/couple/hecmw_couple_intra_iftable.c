...
extern void HECMW_couple_free_intra_iftable(
    struct hecmw_couple_intra_iftable *p) {
  if (p == NULL) return;
...
static int set_n_neighbor_pe(const struct hecmwST_local_mesh *mesh,
...
static int set_neighbor_pe(const struct hecmwST_local_mesh *mesh,
                           struct hecmw_couple_intra_iftable *intra_tbl) {
  int i;

  if (intra_tbl->n_neighbor_pe == 0) return 0;

  intra_tbl->neighbor_pe =
      (int *)HECMW_malloc(sizeof(int) * intra_tbl->n_neighbor_pe);
...
static int *mask_boundary_node(const struct hecmwST_local_mesh *mesh,
...
static int *mask_import_node(const struct hecmwST_local_mesh *mesh,
                             const int *is_boundary_node) {
  int *is_import_node = NULL;
  int node, i;

  HECMW_assert(mesh->n_neighbor_pe > 0);

  is_import_node = (int *)HECMW_calloc(
      mesh->import_index[mesh->n_neighbor_pe] + 1, sizeof(int));
  ...
  for (i = 0; i < mesh->import_index[mesh->n_neighbor_pe]; i++) {
    ...
    if (is_boundary_node[node - 1] >= 0)
      is_import_node[i] = is_boundary_node[node - 1];
  }

  return is_import_node;
}

static int *mask_export_node(const struct hecmwST_local_mesh *mesh,
                             const struct hecmw_couple_comm *intracomm,
                             int *is_import_node) {
  int *is_export_node = NULL;
  int rtc;

  is_export_node = (int *)HECMW_calloc(
      mesh->export_index[mesh->n_neighbor_pe] + 1, sizeof(int));
  ...
  rtc = HECMW_couple_intra_send_recv(mesh->n_neighbor_pe, mesh->neighbor_pe,
                                     ...
                                     mesh->export_index, is_export_node,
...
static int set_intracomm_import_node(
    const struct hecmwST_local_mesh *mesh, const int *is_boundary_node,
    const int *is_import_node, struct hecmw_couple_intra_iftable *intra_tbl) {
  int n_import, node, size, n, i, j;

  HECMW_assert(intra_tbl->n_neighbor_pe > 0);

  /* index */
  intra_tbl->import_index =
...
static int set_intracomm_export_node(
    const struct hecmwST_local_mesh *mesh, const int *is_boundary_node,
    const int *is_export_node, struct hecmw_couple_intra_iftable *intra_tbl) {
  int n_export, size, node, n, i, j;

  HECMW_assert(intra_tbl->n_neighbor_pe > 0);

  /* index */
  intra_tbl->export_index =
      (int *)HECMW_calloc(intra_tbl->n_neighbor_pe + 1, sizeof(int));
...
extern struct hecmw_couple_intra_iftable *HECMW_couple_make_intra_iftable(
  ...
  /* masking */
  if ((is_boundary_node = mask_boundary_node(mesh, boundary)) == NULL)
    goto error;
  if ((is_import_node = mask_import_node(mesh, is_boundary_node)) == NULL)
    goto error;
  if ((is_export_node = mask_export_node(mesh, intracomm, is_import_node)) ==
      NULL)
    goto error;

  /* set import/export node information */
  if (set_intracomm_import_node(mesh, is_boundary_node, is_import_node,
                                intra_tbl))
    goto error;
  if (set_intracomm_export_node(mesh, is_boundary_node, is_export_node,
                                intra_tbl))
    goto error;
