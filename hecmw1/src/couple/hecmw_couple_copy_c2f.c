...
static int set_item_type(void *dst) {
  void *src;
  int size;
...
/*------------------------------------------------------------------------------------------------*/
static int is_alloc_item(void) { return couple_value->item ? 1 : 0; }

static int is_alloc_value(void) { return couple_value->value ? 1 : 0; }
...
extern int HECMW_couple_copy_c2f_init(
    struct hecmw_couple_value *_couple_value) {
...
extern void hecmw_cpl_copy_c2f_isalloc_if(char *struct_name, char *var_name,
                                          int *is_allocated, int *err, int slen,
                                          int vlen) {
  ...
  *err = 1;
  ...
  if (couple_value == NULL) {
    HECMW_set_error(HECMWCPL_E_INVALID_NULL_PTR,
                    "hecmw_cpl_copy_c2f_isalloc_if(): 'couple_value' is not "
                    "initialized yet");
    return;
  }
  if (struct_name == NULL) {
...
extern void hecmw_cpl_copy_c2f_isalloc_if_(char *struct_name, char *var_name,
                                           int *is_allocated, int *err,
                                           int slen, int vlen) {
  hecmw_cpl_copy_c2f_isalloc_if(struct_name, var_name, is_allocated, err, slen,
                                vlen);
}

extern void hecmw_cpl_copy_c2f_isalloc_if__(char *struct_name, char *var_name,
                                            int *is_allocated, int *err,
                                            int slen, int vlen) {
  hecmw_cpl_copy_c2f_isalloc_if(struct_name, var_name, is_allocated, err, slen,
                                vlen);
...
extern void hecmw_cpl_copy_c2f_set_if(char *struct_name, char *var_name,
...
extern void hecmw_cpl_copy_c2f_set_if_(char *struct_name, char *var_name,
                                       void *dst, int *err, int slen,
                                       int vlen) {
  hecmw_cpl_copy_c2f_set_if(struct_name, var_name, dst, err, slen, vlen);
}

extern void hecmw_cpl_copy_c2f_set_if__(char *struct_name, char *var_name,
                                        void *dst, int *err, int slen,
                                        int vlen) {
  hecmw_cpl_copy_c2f_set_if(struct_name, var_name, dst, err, slen, vlen);
}
