...
extern void HECMW_couple_free_comm(struct hecmw_couple_comm *comm) {
  if (comm == NULL) return;
...
static struct hecmw_couple_comm *alloc_struct_comm(void) {
  struct hecmw_couple_comm *comm;

  comm = (struct hecmw_couple_comm *)HECMW_malloc(
      sizeof(struct hecmw_couple_comm));
  if (comm == NULL) {
...
static void free_intracomm_info(void) {
...
static void free_intercomm_info(void) {
...
static void free_couple_info(void) {
...
extern void HECMW_couple_free_couple_info(void) {
  free_intracomm_info();
  free_intercomm_info();
  free_couple_info();
}
...
static struct intracomm_info *get_intracomm_info(const char *unit_id) {
...
static struct intercomm_info *get_intercomm_info(const char *couple_id) {
...
static struct couple_info *get_couple_info(const char *boundary_id) {
...
static int init_intracomm_info(void) {
  ...
  if ((boundary_ids = HECMW_couple_get_boundary_ids()) == NULL)
    return HECMW_ERROR;
  ...
  for (i = 0; i < boundary_ids->n_boundary; i++) {
    ...
    unit1_id =
        HECMW_couple_ctrl_get_unit_id(couple_id, HECMW_COUPLE_UNIT1, NULL, 0);
    if (unit1_id == NULL) goto error;
    unit2_id =
        HECMW_couple_ctrl_get_unit_id(couple_id, HECMW_COUPLE_UNIT2, NULL, 0);
  ...
  for (i = 0; i < unit_ids->n_unit; i++) {
    if (mask[i] == HECMW_COUPLE_TRUE) {
      p->next =
          (struct intracomm_info *)HECMW_malloc(sizeof(struct intracomm_info));
      if (p->next == NULL) {
        HECMW_set_error(errno, "");
        goto error;
      }
...
static int init_intercomm_info(void) {
  ...
  for (i = 0; i < couple_ids->n_couple; i++) {
    if (mask[i] == HECMW_COUPLE_TRUE) {
      p->next =
          (struct intercomm_info *)HECMW_malloc(sizeof(struct intercomm_info));
      if (p->next == NULL) {
        HECMW_set_error(errno, "");
        goto error;
      }
      ...
      if (HECMW_couple_ctrl_get_unit_id(couple_ids->ids[i], HECMW_COUPLE_UNIT1,
                                        unit1_id, HECMW_NAME_LEN + 1) == NULL)
        goto error;
...
static int init_couple_info(void) {
  struct hecmw_couple_ctrl_boundary_ids *boundary_ids;
  struct couple_info *p;
  ...
  for (i = 0; i < boundary_ids->n_boundary; i++) {
    ...
    if (HECMW_couple_ctrl_get_unit_id(p->couple_id, HECMW_COUPLE_UNIT1,
                                      p->unit1_id, HECMW_NAME_LEN + 1) == NULL)
      goto error;
    if (HECMW_couple_ctrl_get_unit_id(p->couple_id, HECMW_COUPLE_UNIT2,
                                      p->unit2_id, HECMW_NAME_LEN + 1) == NULL)
      goto error;

    if (HECMW_couple_ctrl_get_type(p->couple_id, &p->couple_type) !=
        HECMW_SUCCESS)
      goto error;
    if (HECMW_couple_ctrl_get_direction(boundary_id, &p->direction) !=
        HECMW_SUCCESS)
      goto error;
    if (HECMW_couple_ctrl_get_tolerance(boundary_id, &p->tolerance) !=
        HECMW_SUCCESS)
      goto error;
    if (HECMW_couple_ctrl_get_bbcoef(boundary_id, &p->bbcoef) != HECMW_SUCCESS)
      goto error;
    if (HECMW_couple_ctrl_get_bgcoef(boundary_id, &p->bgcoef) != HECMW_SUCCESS)
      goto error;
...
static int set_couple_type(int *couple_type) {
  struct couple_info *p;
...
static int check_intracomm_psize_mxn(void) {
  ...
  if (psize_sum != HECMW_comm_get_size()) {
    HECMW_set_error(HECMWCPL_E_UNMATCH_PSIZE, "Total number of processes is %d",
                    psize_sum);
    return HECMW_ERROR;
  }
...
static int check_intracomm_psize_maxmn(void) {
  struct hecmw_couple_ctrl_proc *proc;
  struct intracomm_info *p;
  int psize_max = 0;
...
static int check_intracomm_psize_manual(void) {
  struct hecmw_couple_ctrl_proc *proc;
  struct intracomm_info *p;
  int *mask     = NULL;
  ...
  for (n = 0, i = 0; i <= psize_max; i++) {
    if (!mask[i]) {
      HECMW_set_error(HECMWCPL_E_DISCONTINUOUS_RANKS,
                      "Process No. %d is not used");
      goto error;
    }
    n++;
  }
...
static int check_intracomm_psize(int couple_type) {
...
static int set_intracomm_psize(void) {
  struct hecmw_couple_ctrl_proc *proc;
  struct intracomm_info *p;
...
static int set_intracomm_ranks_mxn(void) {
  struct intracomm_info *p;
  int n = 0;
  int i;
...
static int set_intracomm_ranks_maxmn(void) {
  struct intracomm_info *p;
  int i;
...
static int set_intracomm_ranks_manual(void) {
  struct hecmw_couple_ctrl_proc *proc;
  struct intracomm_info *p;
  int i;
...
static int set_intracomm_ranks(int couple_type) {
  if (couple_type == HECMW_COUPLE_TYPE_MXN) {
...
static int set_intercomm_psize_mxn(void) {
  struct intercomm_info *p;
...
static int set_intercomm_psize_maxmn(void) {
  struct intercomm_info *p;
  struct intracomm_info *q1, *q2;
...
static int set_intercomm_psize_manual(void) {
  struct intercomm_info *p;
  struct intracomm_info *q1, *q2;
  int *mask = NULL;
...
static int set_intercomm_psize(int couple_type) {
  if (couple_type == HECMW_COUPLE_TYPE_MXN) {
...
static int set_intercomm_ranks_mxn(void) {
  struct intercomm_info *p;
  struct intracomm_info *q1, *q2;
  int n, i;
...
static int set_intercomm_ranks_maxmn(void) {
  struct intercomm_info *p;
  struct intracomm_info *q1, *q2;
  int i;
...
static int set_intercomm_ranks_manual(void) {
  struct intercomm_info *p;
  struct intracomm_info *q1, *q2;
  int *mask = NULL;
  int global_psize, n, i;
...
static int set_intercomm_ranks(int couple_type) {
  if (couple_type == HECMW_COUPLE_TYPE_MXN) {
...
static int set_is_member(struct hecmw_couple_comm *comm) {
  int global_rank, i;
...
static int allgather_root(int *root) {
  ...
  rtc = HECMW_Allgather(send_buf, 1, HECMW_INT, recv_buf, 1, HECMW_INT,
                        global_comm);
...
static int set_root(struct hecmw_couple_comm *comm) {
  if (comm->is_member == 1) {
    if (comm->rank == 0) {
...
static int init_comm(struct hecmw_couple_comm *comm) {
  int rtc, _psize;

  rtc = HECMW_Group_incl(HECMW_comm_get_group(), comm->psize, comm->ranks,
                         &comm->group);
...
extern int HECMW_couple_comm_init(void) {
  struct intracomm_info *p;
...
extern char *HECMW_couple_get_unit_id(const char *boundary_id,
...
extern int HECMW_couple_is_member(const char *boundary_id) {
  struct couple_info *couple;
...
extern int HECMW_couple_is_unit_member(const char *boundary_id,
                                       int unit_specifier) {
  struct couple_info *couple;
...
extern int HECMW_couple_is_unit_member_u(const char *unit_id) {
  struct intracomm_info *intracomm;
...
extern int HECMW_couple_is_root(const char *boundary_id) {
  struct couple_info *couple;
  struct intercomm_info *intercomm;
...
extern int HECMW_couple_is_unit_root(const char *boundary_id,
                                     int unit_specifier) {
  struct couple_info *couple;
  struct intracomm_info *intracomm;
...
extern int HECMW_couple_is_unit_root_u(const char *unit_id) {
  struct intracomm_info *intracomm;
...
extern int HECMW_intercomm_get_size(const char *boundary_id) {
  struct couple_info *couple;
  struct intercomm_info *intercomm;
...
extern int HECMW_intracomm_get_size(const char *boundary_id,
                                    int unit_specifier) {
  struct couple_info *couple;
  struct intracomm_info *intracomm;
...
extern int HECMW_intracomm_get_size_u(const char *unit_id) {
  struct intracomm_info *intracomm;
...
extern int HECMW_intercomm_get_rank(const char *boundary_id) {
  struct couple_info *couple;
  struct intercomm_info *intercomm;
...
extern int HECMW_intracomm_get_rank(const char *boundary_id,
                                    int unit_specifier) {
  struct couple_info *couple;
  struct intracomm_info *intracomm;
...
extern int HECMW_intracomm_get_rank_u(const char *unit_id) {
  struct intracomm_info *intracomm;
...
extern HECMW_Comm HECMW_intercomm_get_comm(const char *boundary_id) {
  struct couple_info *couple;
  struct intercomm_info *intercomm;
...
extern HECMW_Comm HECMW_intracomm_get_comm(const char *boundary_id,
                                           int unit_specifier) {
  struct couple_info *couple;
  struct intracomm_info *intracomm;
...
extern HECMW_Comm HECMW_intracomm_get_comm_u(const char *unit_id) {
  struct intracomm_info *intracomm;
...
extern HECMW_Group HECMW_intercomm_get_group(const char *boundary_id) {
  struct couple_info *couple;
  struct intercomm_info *intercomm;
...
extern HECMW_Group HECMW_intracomm_get_group(const char *boundary_id,
                                             int unit_specifier) {
  struct couple_info *couple;
  struct intracomm_info *intracomm;
...
extern HECMW_Group HECMW_intracomm_get_group_u(const char *unit_id) {
  struct intracomm_info *intracomm;
...
extern struct hecmw_couple_comm *HECMW_couple_get_intracomm(
...
extern struct hecmw_couple_comm *HECMW_couple_get_intracomm_u(
    const char *unit_id) {
  struct intracomm_info *intracomm = NULL;
  struct hecmw_couple_comm *comm;
  int i;
...
extern struct hecmw_couple_comm *HECMW_couple_get_intercomm(
