...
static void free_couple_values(struct hecmw_couple_values *p) {
  if (p == NULL) return;
...
static struct hecmw_couple_values *alloc_couple_values(void) {
  struct hecmw_couple_values *p = NULL;

  p = (struct hecmw_couple_values *)HECMW_malloc(
      sizeof(struct hecmw_couple_values));
  if (p == NULL) {
...
static int update_import_node_value(
    struct hecmw_couple_comm *intracomm,
    struct hecmw_couple_intra_iftable *intra_tbl,
    struct hecmw_couple_value *node_value) {
  ...
  sendbuf_index =
      (int *)HECMW_calloc(intra_tbl->n_neighbor_pe + 1, sizeof(int));
  ...
  /* send buffer */
  nmemb =
      intra_tbl->export_index[intra_tbl->n_neighbor_pe] * node_value->n_dof + 1;
  ...
  for (i = 0; i < intra_tbl->export_index[intra_tbl->n_neighbor_pe]; i++) {
    ...
    for (j = 0; j < node_value->n_dof; j++) {
      sendbuf[node_value->n_dof * i + j] =
          node_value->value[node_value->n_dof * id + j];
    }
  }
  ...
  /* receive buffer */
  nmemb =
      intra_tbl->import_index[intra_tbl->n_neighbor_pe] * node_value->n_dof + 1;
   ...
   * send and receive
   */
  rtc = HECMW_couple_intra_send_recv(
      intra_tbl->n_neighbor_pe, intra_tbl->neighbor_pe, sendbuf_index, sendbuf,
      recvbuf_index, recvbuf, HECMW_DOUBLE, intracomm->comm);
  if (rtc != 0) goto error;
...
static int interpolation(const struct hecmw_couple_weight *p,
                         struct hecmw_couple_value *value_src,
  ...
  for (i = 0; i < p->n; i++) {
    for (j = p->index[i]; j < p->index[i + 1]; j++) {
      for (k = 0; k < n_dof; k++) {
        value_dst->value[i * n_dof + k] +=
            value_src->value[(p->id[j]) * n_dof + k] * p->weight[j];
      }
    }
  }
...
static int pre_interpolation(const struct hecmw_couple_weight_list *ip_list_pre,
                             struct hecmw_couple_values *values) {
  struct hecmw_couple_weight_list *p;

  for (p = ip_list_pre->next; p; p = p->next) {
    if (p->info->type ==
        HECMW_COUPLE_IP_SURF_TO_NODE) { /* surface -> node	*/
...
static int main_interpolation(
    const struct hecmw_couple_weight_list *ip_list_main,
  ...
  for (p = ip_list_main->next; p; p = p->next) {
    if (p->info->type ==
        HECMW_COUPLE_IP_NODE_TO_NODE) { /* node -> node		*/
...
static int post_interpolation(
    const struct hecmw_couple_weight_list *ip_list_post,
    struct hecmw_couple_values *values) {
  struct hecmw_couple_weight_list *p;

  for (p = ip_list_post->next; p; p = p->next) {
    if (p->info->type ==
        HECMW_COUPLE_IP_NODE_TO_SURF) { /* node -> surface	*/
...
static int send_recv_n_dof(const struct hecmw_couple_inter_iftable *inter_tbl,
  ...
  rtc = HECMW_couple_inter_send_recv(
      ...
      sendbuf_index, sendbuf, inter_tbl->n_neighbor_pe_import,
      inter_tbl->neighbor_pe_import, recvbuf_index, recvbuf, HECMW_INT,
      intercomm->comm);
  if (rtc != 0) goto error;
...
static int send_recv_couple_value(
  ...
  if (comm_src->is_member) {
    ...
    if (inter_tbl->n_neighbor_pe_export > 0) {
      ...
      /* send buffer */
      size =
          sizeof(double) * (sendbuf_index[inter_tbl->n_neighbor_pe_export] + 1);
      ...
      for (i = 0;
           i < inter_tbl->export_index[inter_tbl->n_neighbor_pe_export] * n_dof;
           i++) {
  ...
  if (comm_dst->is_member) {
    if (inter_tbl->n_neighbor_pe_import > 0) {
      ...
      for (i = 0; i < inter_tbl->n_neighbor_pe_import; i++) {
        recvbuf_index[i + 1] =
            inter_tbl->import_index[i + 1] * value_dst->n_dof;
      }

      /* receive buffer */
      size =
          sizeof(double) * (recvbuf_index[inter_tbl->n_neighbor_pe_import] + 1);
  ...
  rtc = HECMW_couple_inter_send_recv(
      inter_tbl->n_neighbor_pe_export, inter_tbl->neighbor_pe_export,
      sendbuf_index, sendbuf, inter_tbl->n_neighbor_pe_import,
      inter_tbl->neighbor_pe_import, recvbuf_index, recvbuf, HECMW_DOUBLE,
      intercomm->comm);
  if (rtc != 0) goto error;
  ...
  if (comm_dst->is_member) {
    ...
    size = sizeof(double) *
           inter_tbl->import_index[inter_tbl->n_neighbor_pe_import] * n_dof;
...
static int send_recv(const struct hecmw_couple_inter_iftable *inter_tbl,
                     const struct hecmw_couple_value *value_src,
                     struct hecmw_couple_value *value_dst,
                     const struct hecmw_couple_comm *comm_src,
                     const struct hecmw_couple_comm *comm_dst,
                     const struct hecmw_couple_comm *intercomm) {
  if (send_recv_n_dof(inter_tbl, value_src, value_dst, comm_src, comm_dst,
                      intercomm))
    return -1;
  if (send_recv_couple_value(inter_tbl, value_src, value_dst, comm_src,
                             comm_dst, intercomm))
    return -1;
...
static int set_default_value(struct hecmw_couple_value *couple_value,
                             struct hecmw_couple_values *values_src) {
  int nmemb, i;

  if (couple_value->item_type ==
      HECMW_COUPLE_NODE_GROUP) { /* Node Group		*/
    nmemb = couple_value->n * couple_value->n_dof;
    for (i = 0; i < nmemb; i++) {
      values_src->node->value[i] = couple_value->value[i];
    }

  } else if (couple_value->item_type ==
...
static int init_send_value(const struct hecmw_couple_inter_iftable *inter_tbl,
...
static int init_recv_value(const struct hecmw_couple_inter_iftable *inter_tbl,
...
static int set_result_value(struct hecmw_couple_value *couple_value,
                            struct hecmw_couple_values *values_dst) {
  if (values_dst->item_type ==
      HECMW_COUPLE_NODE_GROUP) { /* Node Group		*/
...
static int init_values(const struct hecmw_couple_boundary *boundary,
                       const struct hecmw_couple_value *couple_value,
  ...
  if (boundary->node->n > 0) {
    ...
    couple_values->node->item =
        (int *)HECMW_malloc(sizeof(int) * couple_values->node->n);
  ...
  if (boundary->elem->n > 0) {
    ...
    couple_values->elem->item =
        (int *)HECMW_malloc(sizeof(int) * couple_values->elem->n);
  ...
  if (boundary->surf->n > 0) {
    ...
    couple_values->surf->item =
        (int *)HECMW_malloc(sizeof(int) * couple_values->surf->n * 2);
...
extern int HECMW_couple(const char *boundary_id,
  ...
  if (boundary_id == NULL) {
    HECMW_set_error(HECMWCPL_E_INVALID_ARG,
                    "HECMW_couple(): 'boundary_id' is NULL");
    return HECMW_ERROR;
  }
  ...
  if (couple_info->comm_src->is_member) {
    if (couple_value == NULL) {
      HECMW_set_error(HECMWCPL_E_INVALID_ARG,
                      "HECMW_couple(): 'couple_value' is NULL");
      goto error;
    }
  }
  if (couple_info->comm_dst->is_member) {
    if (couple_value == NULL) {
      if ((couple_value = HECMW_couple_alloc_couple_value()) == NULL)
        goto error;
    }
  }
  ...
  if (couple_info->comm_src->is_member) {
    if (init_send_value(couple_info->inter_tbl, couple_value, value_send))
      goto error;
  }
  if (couple_info->comm_dst->is_member) {
  ...
  if (couple_info->comm_src->is_member) {
    ...
    if (init_values(couple_info->boundary_src, couple_value, values_src))
      goto error;
    ...
    if (update_import_node_value(couple_info->comm_src,
                                 couple_info->intra_tbl_src, values_src->node))
      goto error;
    if (main_interpolation(couple_info->ip_list_main, values_src, value_send))
      goto error;
  ...
  if (couple_info->comm_dst->is_member) {
    ...
    if (init_values(couple_info->boundary_dst, value_recv, values_dst))
      goto error;
