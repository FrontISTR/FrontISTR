...
extern struct hecmw_couple_weight_list *HECMW_couple_make_pre_ip_list(
  ...
  if (boundary_src->data_type ==
      HECMW_COUPLE_NODE_GROUP) { /* node group		*/
    p->next = NULL;

  } else if (boundary_src->data_type ==
...
extern struct hecmw_couple_weight_list *HECMW_couple_make_main_ip_list(
    const struct hecmwST_local_mesh *mesh_src,
  ...
  if (boundary_src->data_type == HECMW_COUPLE_NODE_GROUP) {
    p->next = HECMW_couple_s2n_dist_node(mesh_src, mesh_dst, comm_src, comm_dst,
                                         intercomm, boundary_src, boundary_dst,
                                         mapped_point, inter_tbl);
    if (p->next == NULL) goto error;

  } else if (boundary_src->data_type == HECMW_COUPLE_SURFACE_GROUP) {
    p->next = HECMW_couple_s2n_dist_surf(mesh_src, mesh_dst, comm_src, comm_dst,
                                         intercomm, boundary_src, boundary_dst,
                                         mapped_point, inter_tbl);
    if (p->next == NULL) goto error;
...
extern struct hecmw_couple_weight_list *HECMW_couple_make_post_ip_list(
    const struct hecmwST_local_mesh *mesh_dst,
    const struct hecmw_couple_boundary *boundary_dst,
    const struct hecmw_couple_mapped_point *mapped_point,
    const struct hecmw_couple_comm *comm_dst,
  ...
  if (boundary_dst->data_type ==
      HECMW_COUPLE_NODE_GROUP) { /* node group		*/
    p->next = NULL;

  } else if (boundary_dst->data_type ==
             HECMW_COUPLE_SURFACE_GROUP) { /* surface group	*/
    p->next = HECMW_couple_n2s_average(mesh_dst, boundary_dst, comm_dst,
                                       intra_tbl_dst);
