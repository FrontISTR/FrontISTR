...
extern struct hecmw_couple_mapped_point *HECMW_couple_alloc_mapped_point(void) {
  ...
  p = (struct hecmw_couple_mapped_point *)HECMW_malloc(
      sizeof(struct hecmw_couple_mapped_point));
  if (p == NULL) {
...
extern void HECMW_couple_free_mapped_point(
    struct hecmw_couple_mapped_point *p) {
  if (p == NULL) return;
...
static struct hecmw_couple_mapped_point *set_mapped_point_by_node(
  ...
  mapped_point->coord =
      (double *)HECMW_malloc(sizeof(double) * mapped_point->n * 3);
...
static struct hecmw_couple_mapped_point *set_mapped_point_by_elem(
    const struct hecmwST_local_mesh *mesh_dst,
    const struct hecmw_couple_boundary *boundary_dst) {
  struct hecmw_couple_mapped_point *mapped_point = NULL;
  ...
  mapped_point->coord =
      (double *)HECMW_malloc(sizeof(double) * mapped_point->n);
  if (mapped_point->coord == NULL) {
    HECMW_set_error(errno, "");
    goto error;
  }
  ...
  for (i = 0; i < mapped_point->n; i++) {
    ...
    for (j = mesh_dst->elem_node_index[elem - 1];
         j < mesh_dst->elem_node_index[elem]; j++) {
...
static struct hecmw_couple_mapped_point *set_mapped_point_by_surf(
    const struct hecmwST_local_mesh *mesh_dst,
    const struct hecmw_couple_boundary *boundary_dst) {
  struct hecmw_couple_mapped_point *mapped_point = NULL;
...
extern struct hecmw_couple_mapped_point *HECMW_couple_set_mapped_point(
  ...
  if (mesh_dst == NULL) {
    HECMW_set_error(HECMWCPL_E_INVALID_ARG,
                    "Invalid NULL pointer is found (mesh_dst)");
    return NULL;
  }
  if (boundary_dst == NULL) {
    HECMW_set_error(HECMWCPL_E_INVALID_ARG,
                    "Invalid NULL pointer is found (boundary_dst)");
    return NULL;
  }
  ...
  if (map_type == HECMW_COUPLE_MAP_NODE_TO_SURF) {
    if ((mapped_point = set_mapped_point_by_node(mesh_dst, boundary_dst)) ==
        NULL)
      goto error;
  } else {
