...
contains
  ...
  subroutine hecmw_matvec_33_A (hecMESH, hecMAT, indexA, itemA, A, X, Y, time_Ax, COMMtime)
    use hecmw_util
    use hecmw_matrix_misc
    implicit none
    type (hecmwST_local_mesh), intent(in) :: hecMESH
    ...
    if (hecmw_mat_get_flag_mpcmatvec(hecMAT) /= 0) then
      allocate(WK(hecMAT%NP * hecMAT%NDOF))
      call hecmw_TtmatTvec_33_A(hecMESH, hecMAT, indexA, itemA, A, X, Y, WK, time_Ax, Tcomm)
      deallocate(WK)
    else
      call hecmw_matvec_33_inner_A(hecMESH, hecMAT, indexA, itemA, A, X, Y, time_Ax, Tcomm)
    endif

    if (present(COMMtime)) COMMtime = COMMtime + Tcomm
  end subroutine hecmw_matvec_33_A
  ...
  subroutine hecmw_matvec_33_inner_A (hecMESH, hecMAT, indexA, itemA, A, X, Y, time_Ax, COMMtime)
    use hecmw_util
    use m_hecmw_comm_f
    use hecmw_matrix_misc
    use hecmw_jad_type
    ...
    if (hecmw_JAD_IS_INITIALIZED().ne.0) then
      ...
      do i = 1, N
        ...
        do j = jS, jE
          ...
          X2= X(3*in-1)
          X3= X(3*in  )
          YV1= YV1 + A(9*j-8)*X1 + A(9*j-7)*X2 + A(9*j-6)*X3
          YV2= YV2 + A(9*j-5)*X1 + A(9*j-4)*X2 + A(9*j-3)*X3
          YV3= YV3 + A(9*j-2)*X1 + A(9*j-1)*X2 + A(9*j  )*X3
        enddo
        Y(3*i-2)= YV1
        Y(3*i-1)= YV2
        Y(3*i  )= YV3
  ...
  end subroutine hecmw_matvec_33_inner_A
  ...
  subroutine hecmw_matresid_33_A (hecMESH, hecMAT, indexA, itemA, A, X, B, R, time_Ax, COMMtime)
    use hecmw_util
    implicit none
    type (hecmwST_local_mesh), intent(in) :: hecMESH
    type (hecmwST_matrix), intent(in)     :: hecMAT
    ...
    Tcomm = 0.d0
    call hecmw_matvec_33_A (hecMESH, hecMAT, indexA, itemA, A, X, R, time_Ax, Tcomm)
    if (present(COMMtime)) COMMtime = COMMtime + Tcomm
    !$acc kernels
    !$acc loop independent
    do i = 1, hecMAT%N * 3
    ...
    enddo
    !$acc end kernels
  end subroutine hecmw_matresid_33_A
...
#ifdef _OPENACC
    !$acc loop independent private(i,k,kk,j,jj)
#else
    !$omp do
#endif
  ...
  subroutine hecmw_TtmatTvec_33_A (hecMESH, hecMAT, indexA, itemA, A, X, Y, W, time_Ax, COMMtime)
    use hecmw_util
    implicit none
    type (hecmwST_local_mesh), intent(in) :: hecMESH
    type (hecmwST_matrix), intent(in)     :: hecMAT
    ...
    call hecmw_Tvec_33(hecMESH, X, Y, COMMtime)
    call hecmw_matvec_33_inner_A(hecMESH, hecMAT, indexA, itemA, A, Y, W, time_Ax, COMMtime)
    call hecmw_Ttvec_33(hecMESH, W, Y, COMMtime)

  end subroutine hecmw_TtmatTvec_33_A
