...
module hecmw_solver_las_33
  ...
  public :: hecmw_matvec_33_setup
  public :: hecmw_matvec_33_teardown
...
contains
  ...
  !C
  !C***
  !C*** hecmw_matvec_33_setup
  !C***
  !C
  subroutine hecmw_matvec_33_setup (hecMESH, hecMAT)
    use hecmw_util
    use hecmw_jad_type
    implicit none
    ...
    ! JAD takes over the whole matvec, so the tuned implementations must stay out of it
  ...
  !C
  !C***
  !C*** hecmw_matvec_33_generic ( private subroutine )
  !C***
  !C
  subroutine hecmw_matvec_33_generic (hecMESH, hecMAT, X, Y, time_Ax, COMMtime)
    ...
    if (isFirst) then
      call setup_tuning_parameters(N, NP, indexL, indexU)
      isFirst = .false.
    endif
#endif
    ! <<< added for tuning
  ...
  subroutine setup_tuning_parameters(N, NP, indexL, indexU)
    ...
    integer(kind=kint), pointer, intent(in) :: indexL(:), indexU(:)
    integer(kind=kint) :: i, blockNum, numOfBlock
    integer(kind=kint) :: numOfElement, elementCount
    ...
    do i= 1, N
      ...
      if (elementCount > (blockNum + 1) * numOfElementPerBlock) then
        endPos(blockNum) = i
        ! write(9000+hecmw_comm_get_rank(),*) mod(blockNum, numOfThread), &
    ...
    do i= blockNum+1, numOfBlock-1
      startPos(i) = N
      endPos(i) = N-1
      ! write(9000+hecmw_comm_get_rank(),*) mod(i, numOfThread), &
        !      startPos(i), endPos(i)
    end do
