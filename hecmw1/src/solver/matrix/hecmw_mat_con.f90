...
module hecmw_matrix_con
  ...
  public :: hecmw_mat_recon_withmpc
...
contains
  ...
  subroutine hecmw_mat_recon_withmpc ( hecMESH, hecMAT )
    type (hecmwST_matrix)     :: hecMAT
    type (hecmwST_local_mesh) :: hecMESH
    ...
    enddo
    !add mpc
    do icel= 1, hecMESH%mpc%n_mpc
      is= hecMESH%mpc%mpc_index(icel-1)
      iE= hecMESH%mpc%mpc_index(icel  )
      nn = iE-is
      do j=1,nn
        nid(j) = hecMESH%mpc%mpc_item(is+j)
      enddo
      do j=1,nn
        nid1 = nid(j)
        do k=1,nn
