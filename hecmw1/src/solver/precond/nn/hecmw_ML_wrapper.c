...
void hecmw_ML_wrapper_setup(int *id, int *sym, int *Ndof, int *ierr) {
  ...
  /* ML_Set_SpectralNorm_Iterations(); */      /* default is ten */
    ...
    if (SmootherType == SymGaussSeidel) {
      ...
      if (FlgUseHECMWSmoother) {
        ...
        if (*Ndof == 3) {
          ...
          hecmw_ml_smoother_setup_nn_(id, ierr);
          if (*ierr != HECMW_SUCCESS) return;
          ML_Set_Smoother(ml_object, 0, ML_BOTH, id, hecmw_ML_smoother_apply_nn, "HEC-MW");
        }
        level++;
      }
  ...
  if (SmootherType == SymBlockGaussSeidel) {
    if (FlgUseHECMWSmoother) {
      if (MLInfo[*id - 1].ndof == 3) {
        ...
        hecmw_ml_smoother_clear_nn_(id, ierr);
      }
    }
  }
}
