...
!C***
!C*** module hecmw_solver_GPBiCG_nn
!C***
!
      module hecmw_solver_GPBiCG_nn

      private
      public :: hecmw_solve_GPBiCG_nn

      contains
!C
!C*** hecmw_solve_GPBiCG_nn
!C
      subroutine hecmw_solve_GPBiCG_nn( hecMESH,  hecMAT, ITER, RESID, ERROR, &
     &                              Tset, Tsol, Tcomm )
      use hecmw_util
...
!C
!C-- SCALING
      call hecmw_solver_scaling_fw_nn(hecMESH, hecMAT, Tcomm)
...
!C +----------------------+
!C===
      call hecmw_precond_nn_setup(hecMAT, hecMESH, 0)
...
!C +----------------------+
!C===
      call hecmw_matresid_nn(hecMESH, hecMAT, X, B, WW(:,R), Tcomm)
...
!C===
      ...
      call hecmw_precond_nn_apply(hecMESH, hecMAT, WW(:,WK), WW(:,R), WW(:,ZQ), Tcomm)
!C===
...
!C
!C-- calc. {p_tld}= A{p}
      call hecmw_matvec_nn(hecMESH, hecMAT, WW(:,P), WW(:,PT), Tcomm)
...
!C         {W2}   = [Minv]{p_tld}
!C
      call hecmw_precond_nn_apply(hecMESH, hecMAT, WW(:,T), WW(:,TT), WW(:,ZQ), Tcomm)
      call hecmw_precond_nn_apply(hecMESH, hecMAT, WW(:,T0), WW(:,W2), WW(:,ZQ), Tcomm)
      do i= 1, NNDOF
        WW(i,T0)= WW(i,W2)
      enddo
      call hecmw_precond_nn_apply(hecMESH, hecMAT, WW(:,PT), WW(:,W2), WW(:,ZQ), Tcomm)

!C===

!C
!C-- calc. [A]{t_tld}
      call hecmw_matvec_nn(hecMESH, hecMAT, WW(:,TT), WW(:,WK), Tcomm)
...
!C
      ...
      end subroutine  hecmw_solve_GPBiCG_nn

      end module     hecmw_solver_GPBiCG_nn
