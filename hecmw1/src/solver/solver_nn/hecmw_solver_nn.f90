...
contains
  ...
  !C***
  !
  subroutine hecmw_solve_nn (hecMESH, hecMAT)

    use hecmw_util
    use hecmw_solver_CG_nn
    use hecmw_solver_BiCGSTAB_nn
    use hecmw_solver_GMRES_nn
    use hecmw_solver_GPBiCG_nn
    ...
    do i= 1, hecMAT%N
      do j = 1, hecMAT%NDOF
        if (dabs(hecMAT%D(hecMAT%NDOF*hecMAT%NDOF*(i-1)+(j-1)*(hecMAT%NDOF+1)+1)).eq.0.d0) IFLAG(1)= 1
      end do 
    enddo
    ...
    if (totalmpc > 0) then
      ...
      if (MPC_METHOD == 1) then  ! penalty
        ...
        call hecmw_matvec_nn_set_mpcmatvec_flg (.true.)
        allocate(Btmp(hecMAT%NP * hecMAT%NDOF))
        do i=1,hecMAT%NP * hecMAT%NDOF
          Btmp(i) = hecMAT%B(i)
        enddo
        call hecmw_trans_b_nn(hecMESH, hecMAT, Btmp, hecMAT%B, time_dumm)
        hecTKT => hecMAT
        ...
        call hecmw_trans_b_nn(hecMESH, hecMAT, hecMAT%B, hecTKT%B, time_dumm)
      endif
    else
      hecTKT => hecMAT
    endif
    ...
    !call hecmw_mat_diag_sr_nn(hecMESH, hecTKT)

    call hecmw_mat_dump(hecTKT, hecMESH)

    call hecmw_matvec_nn_set_async(hecTKT)
    ...
    call hecmw_matvec_nn_clear_timer()
    call hecmw_precond_nn_clear_timer()
    ...
    if (METHOD.eq.1) then
      if (hecMESH%my_rank.eq.0 .and. (ITERlog.eq.1 .or. TIMElog.ge.1)) then
        if (PRECOND.le.2) &
          write (*,'(a,i0,a,i0,a,i3)') '### NxN (',hecMAT%NDOF,'x',hecMAT%NDOF,') B-SSOR-CG(0)',iterPREmax
        if (PRECOND.eq.3) &
          write (*,'(a,i0,a,i0,a,i3)') '### NxN (',hecMAT%NDOF,'x',hecMAT%NDOF,') B-scale-CG',iterPREmax
        if (PRECOND.eq.10) &
          write (*,'(a,i0,a,i0,a,i3)') '### NxN (',hecMAT%NDOF,'x',hecMAT%NDOF,') B-IC-CG(0)',iterPREmax
        if (PRECOND.eq.11) &
          write (*,'(a,i0,a,i0,a,i3)') '### NxN (',hecMAT%NDOF,'x',hecMAT%NDOF,') B-IC-CG(1)',iterPREmax
        if (PRECOND.eq.12) &
          write (*,'(a,i0,a,i0,a,i3)') '### NxN (',hecMAT%NDOF,'x',hecMAT%NDOF,') B-IC-CG(2)',iterPREmax
        if (PRECOND.eq.5) &
          write (*,'(a,i0,a,i0,a,i3)') '### NxN (',hecMAT%NDOF,'x',hecMAT%NDOF,') ML-CG',iterPREmax
      endif
      call hecmw_solve_CG_nn( hecMESH,  hecTKT, ITER, RESID, ERROR,   &
    ...
    if (METHOD.eq.2) then
      if (hecMESH%my_rank.eq.0 .and. (ITERlog.eq.1 .or. TIMElog.ge.1)) then
        if (PRECOND.le.2) &
          write (*,'(a,i0,a,i0,a,i3)') '### NxN (',hecMAT%NDOF,'x',hecMAT%NDOF,') B-SSOR-BiCGSTAB(0)',iterPREmax
        if (PRECOND.eq.3) &
          write (*,'(a,i0,a,i0,a,i3)') '### NxN (',hecMAT%NDOF,'x',hecMAT%NDOF,') B-scale-BiCGSTAB',iterPREmax
        if (PRECOND.eq.10) &
          write (*,'(a,i0,a,i0,a,i3)') '### NxN (',hecMAT%NDOF,'x',hecMAT%NDOF,') B-IlU-BiCGSTAB(0)',iterPREmax
        if (PRECOND.eq.11) &
          write (*,'(a,i0,a,i0,a,i3)') '### NxN (',hecMAT%NDOF,'x',hecMAT%NDOF,') B-IlU-BiCGSTAB(1)',iterPREmax
        if (PRECOND.eq.12) &
          write (*,'(a,i0,a,i0,a,i3)') '### NxN (',hecMAT%NDOF,'x',hecMAT%NDOF,') B-IlU-BiCGSTAB(2)',iterPREmax
        if (PRECOND.eq.5) &
          write (*,'(a,i0,a,i0,a,i3)') '### NxN (',hecMAT%NDOF,'x',hecMAT%NDOF,') ML-BiCGSTAB',iterPREmax
      endif

      call hecmw_solve_BiCGSTAB_nn( hecMESH,  hecTKT, ITER, RESID, ERROR, &
           &                                TIME_setup, TIME_sol, TIME_comm )
    endif
    ...
    if (METHOD.eq.3) then
      if (hecMESH%my_rank.eq.0 .and. (ITERlog.eq.1 .or. TIMElog.ge.1)) then
        if (PRECOND.le.2) &
          write (*,'(a,i0,a,i0,a,i3)') '### NxN (',hecMAT%NDOF,'x',hecMAT%NDOF,') B-SSOR-GMRES(0)',iterPREmax
        if (PRECOND.eq.3) &
          write (*,'(a,i0,a,i0,a,i3)') '### NxN (',hecMAT%NDOF,'x',hecMAT%NDOF,') B-scale-GMRES',iterPREmax
        if (PRECOND.eq.10) &
          write (*,'(a,i0,a,i0,a,i3)') '### NxN (',hecMAT%NDOF,'x',hecMAT%NDOF,') B-ILU-GMRES(0)',iterPREmax
        if (PRECOND.eq.11) &
          write (*,'(a,i0,a,i0,a,i3)') '### NxN (',hecMAT%NDOF,'x',hecMAT%NDOF,') B-ILU-GMRES(1)',iterPREmax
        if (PRECOND.eq.12) &
          write (*,'(a,i0,a,i0,a,i3)') '### NxN (',hecMAT%NDOF,'x',hecMAT%NDOF,') B-ILU-GMRES(2)',iterPREmax
        if (PRECOND.eq.5) &
          write (*,'(a,i0,a,i0,a,i3)') '### NxN (',hecMAT%NDOF,'x',hecMAT%NDOF,') ML-GMRES',iterPREmax
      endif

      call hecmw_solve_GMRES_nn( hecMESH,  hecTKT, ITER, RESID, ERROR, &
           &                             TIME_setup, TIME_sol, TIME_comm )
    endif
    ...
    if (METHOD.eq.4) then
      if (hecMESH%my_rank.eq.0 .and. (ITERlog.eq.1 .or. TIMElog.ge.1)) then
        if (PRECOND.le.2) &
          write (*,'(a,i0,a,i0,a,i3)') '### NxN (',hecMAT%NDOF,'x',hecMAT%NDOF,') B-SSOR-GPBiCG(0)',iterPREmax
        if (PRECOND.eq.3) &
          write (*,'(a,i0,a,i0,a,i3)') '### NxN (',hecMAT%NDOF,'x',hecMAT%NDOF,') B-scale-GPBiCG',iterPREmax
        if (PRECOND.eq.10) &
          write (*,'(a,i0,a,i0,a,i3)') '### NxN (',hecMAT%NDOF,'x',hecMAT%NDOF,') B-ILU-GPBiCG(0)',iterPREmax
        if (PRECOND.eq.11) &
          write (*,'(a,i0,a,i0,a,i3)') '### NxN (',hecMAT%NDOF,'x',hecMAT%NDOF,') B-ILU-GPBiCG(1)',iterPREmax
        if (PRECOND.eq.12) &
          write (*,'(a,i0,a,i0,a,i3)') '### NxN (',hecMAT%NDOF,'x',hecMAT%NDOF,') B-ILU-GPBiCG(2)',iterPREmax
        if (PRECOND.eq.5) &
          write (*,'(a,i0,a,i0,a,i3)') '### NxN (',hecMAT%NDOF,'x',hecMAT%NDOF,') ML-GPBiCG',iterPREmax
      endif

      call hecmw_solve_GPBiCG_nn( hecMESH,  hecTKT, ITER, RESID, ERROR, &
           &                              TIME_setup, TIME_sol, TIME_comm )
    endif
    ...
    resid2=hecmw_rel_resid_L2_nn(hecMESH,hecTKT)
    if (hecMESH%my_rank.eq.0 .and. (ITERlog.eq.1 .or. TIMElog.ge.1)) then
    ...
    call hecmw_matvec_nn_unset_async
    ...
    if (totalmpc > 0) then
      if (MPC_METHOD == 1) then  ! penalty
        ...
        call hecmw_tback_x_nn(hecMESH, hecTKT%X, time_dumm)
        do i=1,hecMAT%NP * hecMAT%NDOF
          hecMAT%B(i) = Btmp(i)
        enddo
        deallocate(Btmp)
    ...
    time_Ax = hecmw_matvec_nn_get_timer()
    time_precond = hecmw_precond_nn_get_timer()
  ...
  end subroutine hecmw_solve_nn
end module hecmw_solver_nn
