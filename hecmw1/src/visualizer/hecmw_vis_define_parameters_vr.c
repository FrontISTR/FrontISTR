...
void generate_histogram_graph(double tmincolor, double tmaxcolor, double *var, int *empty_flag, int nx, int ny, int nz,int mynode,
		int pesize, HECMW_Comm VIS_COMM, int color_system_type)
{
	...
	if(mynode==0) {
		...
		if(max_number==0) {
			fprintf(stderr, "Cannot generate histogram graph, the number of voxels is 0\n");
			exit(0);
		}
...
void generate_interval_point(double tmincolor, double tmaxcolor, double *var, int *empty_flag,
