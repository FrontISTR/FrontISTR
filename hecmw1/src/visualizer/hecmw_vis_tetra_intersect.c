...
int get_tetra_data(Surface *sff, struct hecmwST_local_mesh *mesh,
                   struct hecmwST_result_data *data, int elemID, Tetra *tetra,
                   int tn_component) {
  int i, j;
  int nodeID;
  int c_base, s_base;
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (sff->surface_style == 2) {
      ...
      tetra->s_data[i] =
          get_value_equ(sff->cont_equ, sff->cross_type, tetra->axis[i * 3],
...
static double intersect_line(int v0, int v1, double isovalue, Tetra *tetra,
                             double point[3]) {
  int j;
  double rate, color;
  ...
  if (fabs(tetra->s_data[v1] - tetra->s_data[v0]) < EPSILON)
    ...
    for (j     = 0; j < 3; j++)
      point[j] = rate * (tetra->axis[v1 * 3 + j] - tetra->axis[v0 * 3 + j]) +
                 tetra->axis[v0 * 3 + j];
