...
void HECMW_vis_pvr_rendering(struct hecmwST_local_mesh *mesh,
                             ...
                             int *init_flag, int num_of_pvr, Parameter_vr *vr,
                             int stat_para[NUM_CONTROL_PVR], char *outfile,
                             HECMW_Comm VIS_COMM) {
  int mynode, pesize;
  ...
  int n_voxel;
  /*
int max_level, xr, yr, projection_style, num_of_lights, color_mapping_style,
interval_mapping_num,
    num_of_features, rotate_style, transfer_function_style,
color_mapping_bar_on, scale_marking_on, num_of_frames,
    color_system_type,color_bar_style, fixed_range_on, num_of_scale, mark_0_on,
specified_level[3],
    nv_xyz[3], remove_0_display_on, histogram_on;
char name_lookup[128], name_voxelfile[128],color_comp_name[128],
color_subcomp_name[5];

double *light_point, view_point_d[3], screen_point[3], up[3], k_ads[3],
*interval_point, opa_value, *fea_point,
  background_color[3], font_color[3], font_size, range_value[2],
display_range[6];
   */
...
fclose(contfp);
  ...
  if ((*init_flag == 1) || (num_of_pvr > 1)) {
    ...
    if ((voxel_dxyz == NULL) || (voxel_orig_xyz == NULL) || (level == NULL) ||
        (voxel_n_neighbor_pe == NULL) || (voxel_neighbor_pe == NULL))
    ...
    for (j = 0; j < pesize; j++) {
      ...
      for (k = 0; k < pesize; k++) voxel_neighbor_pe[j][k] = 0;
    }
    n_voxel = pesize;
    ...
    if (stat_para[22] == 1) {
       ...
       */
      read_voxel_file(vr->name_voxelfile, pesize, voxel_dxyz, voxel_orig_xyz,
                      level, voxel_n_neighbor_pe, voxel_neighbor_pe);
    }
  } /* end of *init_flag=1 */

  node1 = (double *)HECMW_calloc(mesh->n_node, sizeof(double));

  transform_ucd_pvr(data, node1, mesh, vr, mynode, pesize, VIS_COMM, voxel_dxyz,
...
if(surface==NULL) {
  ...
  if (*init_flag == 1) {
    ...
    calc_voxel_level(n_voxel, mesh, voxel_dxyz, voxel_orig_xyz, extent, level,
                     VIS_COMM);
    if (mynode == 0)
      for (i = 0; i < n_voxel; i++)
        fprintf(stderr, "voxel %d level: %d %d %d \n", i, level[i * 3],
                level[i * 3 + 1], level[i * 3 + 2]);
    if (vr->specified_level[0] > 0) {
      x_specified_l = vr->specified_level[0];
      for (i = 0; i < n_voxel; i++) level[i * 3] = x_specified_l;
    } else if (vr->max_level > 0) {
      for (i = 0; i < n_voxel; i++) {
        if (level[i * 3] > vr->max_level) level[i * 3] = vr->max_level;
      }
    }
    if (vr->specified_level[1] > 0) {
      y_specified_l = vr->specified_level[1];
      for (i = 0; i < n_voxel; i++) level[i * 3 + 1] = y_specified_l;
    } else if (vr->max_level > 0) {
      for (i = 0; i < n_voxel; i++) {
        if (level[i * 3 + 1] > vr->max_level) level[i * 3 + 1] = vr->max_level;
      }
    }
    if (vr->specified_level[2] > 0) {
      z_specified_l = vr->specified_level[2];
      for (i = 0; i < n_voxel; i++) level[i * 3 + 2] = z_specified_l;
    } else if (vr->max_level > 0) {
      for (i = 0; i < n_voxel; i++) {
        if (level[i * 3 + 2] > vr->max_level) level[i * 3 + 2] = vr->max_level;
      }
    }
  }
...
fprintf(stderr, "the time for reading file is %lf the total time up to now is
%lf\n", t2-t1, t2-t1);
}
   */
  t1 = HECMW_Wtime();
  ...
  if (mynode == 0) fprintf(stderr, "calc_gradient\n");
#ifdef slow
  calc_gradient(n_node, n_internal, n_elem, node, elem, node1, n_neighbor_pe,
                neighbor_pe, import_index, import_node, export_index,
                export_node, phi_x, phi_y, phi_z, mynode, VIS_COMM);
  t3 = HECMW_Wtime();
  if (mynode == 0) {
    fprintf(stderr,
            "The time for computing gradient is %lf the total time up to now "
            "is %lf\n",
            t3 - t2, t3 - t1);
    t2 = t3;
  }
...
#ifdef slow
  grad_var = (double *)HECMW_calloc((nx + 1) * (ny + 1) * (nz + 1) * 3,
                                    sizeof(double));
  ...
  refinement(mesh, node1, n_voxel, voxel_dxyz, voxel_orig_xyz, level,
             voxel_n_neighbor_pe, voxel_neighbor_pe, extent, mynode, VIS_COMM,
             empty_flag, var);
#ifdef slow
  HECMW_free(phi_x);
  HECMW_free(phi_y);
  ...
  if (mynode == 0) {
    ...
    fprintf(stderr,
            " The time for refinement is %lf the total time up to now is %lf\n",
            t3 - t2, t3 - t1);
    t2 = t3;
  }
  HECMW_free(node1);
  ...
  HECMW_Barrier(VIS_COMM);
  if (pesize > 1) {
    HECMW_Allreduce(&mincolor, &tmincolor, 1, HECMW_DOUBLE, HECMW_MIN,
                    VIS_COMM);
    HECMW_Allreduce(&maxcolor, &tmaxcolor, 1, HECMW_DOUBLE, HECMW_MAX,
                    VIS_COMM);
  } else {
    tmincolor = mincolor;
    tmaxcolor = maxcolor;
  }
  org_mincolor = tmincolor;
  org_maxcolor = tmaxcolor;
  ...
  if (vr->histogram_on == 1)
    output_histogram_vr(tmincolor, tmaxcolor, var, empty_flag, nx, ny, nz,
                        mynode, pesize, VIS_COMM);
  if (vr->histogram_on == 2)
    generate_histogram_graph_vr(tmincolor, tmaxcolor, var, empty_flag, nx, ny,
                                nz, mynode, pesize, VIS_COMM,
                                vr->color_system_type);
  if (vr->color_mapping_style == 4)
    generate_interval_point_vr(tmincolor, tmaxcolor, var, empty_flag, nx, ny,
                               nz, mynode, pesize, VIS_COMM,
                               vr->interval_point);
  ...
  if ((*init_flag == 1) || (num_of_pvr > 1)) {
    if (stat_para[4] == 0) {
      ...
      if (vr->light_point == NULL) HECMW_vis_memory_exit("light_point");
...
Define the sampling and projection parameters
  ...
  if (vr->rotate_style == 0)
    ...
    for (i = 0; i < 3; i++) o_v_point[i] = vr->view_point_d[i];
    m_scr[0] = m_scr[2] = 1.0E+17;
    m_scr[1] = m_scr[3] = -1.0E+17;

    for (ii = 0; ii < num_img; ii++) {
      if (ii != 0)
        view1_parameter_define(ii, vr->num_of_frames, vr->rotate_style,
                               vr->view_point_d, vr->screen_point,
                               vr->num_of_lights, vr->light_point, vr->up,
                               trange);
      for (i      = 0; i < 3; i++)
      ...
      get_frame_transform_matrix(vr->view_point_d, vr->screen_point, vr->up,
                                 coff_matrix);
    ...
    for (i = 0; i < 3; i++) vr->view_point_d[i] = o_v_point[i];
  }

  for (ii = 0; ii < num_img; ii++) {
    if (ii != 0)
      view_parameter_define(ii, vr->num_of_frames, vr->rotate_style,
                            vr->view_point_d, vr->screen_point, vr->up,
                            vr->num_of_lights, vr->light_point, trange);
    for (i      = 0; i < 3; i++)
      view_n[i] = vr->screen_point[i] - vr->view_point_d[i];
    nview_n     = sqrt(SQR(view_n[0]) + SQR(view_n[1]) + SQR(view_n[2]));
    if (fabs(nview_n) > EPSILON)
...
view_n[2]*vr->screen_point[2];
    ...
    if (mynode == 0)
      fprintf(stderr,
              ...
              vr->view_point_d[0], vr->view_point_d[1], vr->view_point_d[2],
              vr->screen_point[0], vr->screen_point[1], vr->screen_point[2],
              vr->up[0], vr->up[1], vr->up[2]);
    get_frame_transform_matrix(vr->view_point_d, vr->screen_point, vr->up,
                               coff_matrix);
    transform_frame(vr->screen_point, vertex, coff_matrix, n_vertex);
    /*  if(mynode==0){
...
exit(0);
      ...
      } else if ((vr->color_mapping_bar_on == 1) &&
        ...
        if (scale_type == 1) {
          start_x                               = 10 + 45 * scale;
          if (vr->color_bar_style == 2) start_x = 10 + 45 * scale + 15;
        } else if (scale_type == 2) {
          start_x                               = 10 + 63 * scale;
          if (vr->color_bar_style == 2) start_x = 10 + 63 * scale + 15;
        }
   ...
   grad_minmax[0]=tgrad_minmax[0]; grad_minmax[1]=tgrad_minmax[1];
			 */   }
                        ...
                        if (vr->transfer_function_style == 3) {
                          find_feap_minmax(vr->num_of_features, vr->fea_point,
                                           tmincolor, tmaxcolor, feap_minmax);
                          HECMW_Barrier(VIS_COMM);
                          HECMW_Allreduce(&feap_minmax[0], &tfeap_minmax[0], 1,
                                          HECMW_DOUBLE, HECMW_MIN, VIS_COMM);
                          HECMW_Allreduce(&feap_minmax[1], &tfeap_minmax[1], 1,
                                          HECMW_DOUBLE, HECMW_MAX, VIS_COMM);
                        ...
                        if (vr->transfer_function_style == 4) {
                          /*	   find_feai_minmax(num_of_features, fea_point,
         tmincolor, tmaxcolor, feai_minmax);
             HECMW_Barrier(VIS_COMM);
         HECMW_Allreduce(&feai_minmax[0], &tfeai_minmax[0], 1, HECMW_DOUBLE,
         HECMW_MIN, VIS_COMM);
         HECMW_Allreduce(&feai_minmax[1], &tfeai_minmax[1], 1, HECMW_DOUBLE,
         HECMW_MAX, VIS_COMM);
             feai_minmax[0]=tfeai_minmax[0];  feai_minmax[1]=tfeai_minmax[1];
                        ...
                        dis_minmax[0] = 1.0E17;
                        dis_minmax[1] = -1.0E17;
                        if ((vr->transfer_function_style == 5) ||
                            (vr->transfer_function_style == 6))
                          find_dis_minmax(vr->view_point_d, vertex, dis_minmax);
                        /*       HECMW_Allreduce(&dis_minmax[0], &dis_minmax[0],
        1, HECMW_DOUBLE, HECMW_MIN, VIS_COMM);
                   if(time_step==0) {
        ...
        opa_table\n");
           ...
           for(i=0;i<vd->leveltot;i++)
                   num_refine*=2;
                         */
                        av_length = sqrt(SQR(voxel_dxyz[mynode * 3] / nx) +
                                         SQR(voxel_dxyz[mynode * 3 + 1] / ny) +
                                         SQR(voxel_dxyz[mynode * 3 + 2] / nz));

                        HECMW_Barrier(VIS_COMM);
                        if (pesize > 1)
                          HECMW_Allreduce(&av_length, &tav_length, 1,
                                          HECMW_DOUBLE, HECMW_MAX, VIS_COMM);
                        else
                        ...
                        accum_opa = (double *)HECMW_calloc(vr->xr * vr->yr,
                                                           sizeof(double));
                        if (accum_opa == NULL)
                        ...
                        if (mynode == 0) {
                          fprintf(stderr,
                                  "The time for preprocessing of pvr is %lf, "
                                  "the total time up to now is %lf\n",
                                  t3 - t2, t3 - t1);
                          t2 = t3;
                        }
                        ...
                        /*   fprintf(stderr, "projection range is  %d %d %d %d",
                        ...
                        if (vr->color_mapping_style == 3) {
                          mincolor = vr->interval_point[0];
                          maxcolor =
                              vr->interval_point[2 * vr->interval_mapping_num];
                        }
       ...
       for(i=0;i<vr;i++) {
                             ...
                             */
                            /* for Multi-PEs, first must judge whether it is
                               within the PE projection range
                            ...
                            point_s[1] = y;
                            point_s[2] = 0.0;
                            tranverse_transform(vr->screen_point, point_s,
                                                inv_matrix, point_o);
                            /*find the first intersected voxel*/
                            for (m = 0; m < 3; m++)
                              ray_direction[m] =
                                  point_o[m] - vr->view_point_d[m];
                            /* new---here----to remove the overlap pixel
                             * computation */
                            trace_flag = 1;
                            if (fabs(ray_direction[0]) < EPSILON) {
                              if (fabs(orig_xyz[0] + dxyz[0] -
                                       vr->view_point_d[0]) < EPSILON) {
                                trace_flag = 0;
                              }
                            } else if (fabs(ray_direction[1]) < EPSILON) {
                              if (fabs(orig_xyz[1] + dxyz[1] -
                                       vr->view_point_d[1]) < EPSILON) {
                                trace_flag = 0;
                              }
                            } else if (fabs(ray_direction[2]) < EPSILON) {
                              if (fabs(orig_xyz[2] + dxyz[2] -
                                       vr->view_point_d[2]) < EPSILON) {
                                trace_flag = 0;
                              }
                            }
                            if (trace_flag == 1) {
                              nn = sqrt(SQR(ray_direction[0]) +
                                        SQR(ray_direction[1]) +
                                        SQR(ray_direction[2]));
                              if (fabs(nn) > EPSILON) {
                                for (m = 0; m < 3; m++) ray_direction[m] /= nn;
                              }
                              /*
 intersection=find_first_inter(point_o, vr, vd, root_tree, &voxel_p,
 ray_direction, first_p);
 if(intersection==1) {
         int connect[8][6];
         ray_trace(vr, vd, root_tree, voxel_p, first_p, ray_direction,
 tmincolor, tmaxcolor, connect,
         local_face, accum_rgba, grad_minmax, feap_minmax, feai_minmax,
 dis_minmax,
         opa_table, tav_length,time_step);
                               */
                              intersection = find_first_inter(
                                  point_o, vr->view_point_d, r_level, orig_xyz,
                                  dxyz, r_dxyz, ray_direction, first_p,
                                  first_ijk);
                              if (intersection == 1) {
                                ray_trace(
                                    vr->remove_0_display_on,
                                    vr->color_mapping_style, vr->interval_point,
                                    vr->transfer_function_style, vr->opa_value,
                                    vr->num_of_features, vr->fea_point,
                                    vr->view_point_d, vr->interval_mapping_num,
                                    vr->color_system_type, vr->num_of_lights,
                                    vr->light_point, vr->k_ads, orig_xyz, dxyz,
                                    r_dxyz, r_level, empty_flag, var, grad_var,
                                    first_p, first_ijk, ray_direction, mincolor,
                                    maxcolor, accum_rgba, grad_minmax,
                                    feap_minmax, feai_minmax, dis_minmax,
                                    opa_table, tav_length, time_step, i, j);
                        ...
                        if (mynode == 0) {
                          ...
                          t3 = HECMW_Wtime();
                          fprintf(stderr,
                                  "The  volume rendering on each PE is %lf the "
                                  "total time up to now is %lf\n",
                                  t3 - t2, t3 - t1);
                          t2 = t3;
                        }
                        ...
                        if (pesize > 1) {
                          ...
                          n_subimage = (double *)HECMW_calloc(pesize * pixn * 3,
                                                              sizeof(double));
                          n_subopa = (double *)HECMW_calloc(pesize * pixn,
                                                            sizeof(double));
                          ...
                          if (mynode == 0)
                            ...
                            for (j = 0; j < pixn; j++)
                              n_subopa[mynode * pixn + j] =
                                  accum_opa[pix0_num + (mynode - 1) * pix_num +
                                            j];

                          subimage =
                              (double *)HECMW_calloc(pixn * 3, sizeof(double));
                          ...
                          for (i = 0; i < pesize; i++) {
                            if (mynode == i) {
                              ...
                              for (k = 0; k < pesize; k++) {
                                if (k != i) {
                                  if (k == 0) {
                                    subimage1 = (double *)HECMW_calloc(
                                        pix0_num * 3, sizeof(double));
                                    subopa1 = (double *)HECMW_calloc(
                                        pix0_num, sizeof(double));
                                  } else if (k != 0) {
                                    subimage1 = (double *)HECMW_calloc(
                                        pix_num * 3, sizeof(double));
                                    subopa1 = (double *)HECMW_calloc(
                                        pix_num, sizeof(double));
                                  }
                                  ...
                                  if (k == 0) {
                                    HECMW_Send(subimage1, pix0_num * 3,
                                               HECMW_DOUBLE, k, 0, VIS_COMM);
                                    HECMW_Send(subopa1, pix0_num, HECMW_DOUBLE,
                                               k, 0, VIS_COMM);
                                  } else if (k != 0) {
                                    HECMW_Send(subimage1, pix_num * 3,
                                               HECMW_DOUBLE, k, 0, VIS_COMM);
                                    HECMW_Send(subopa1, pix_num, HECMW_DOUBLE,
                                               k, 0, VIS_COMM);
                                  }
                            ...
                            if (mynode != i) {
                              HECMW_Recv(subimage, pixn * 3, HECMW_DOUBLE, i,
                                         HECMW_ANY_TAG, VIS_COMM, &stat);
                              HECMW_Recv(subopa, pixn, HECMW_DOUBLE, i,
                                         HECMW_ANY_TAG, VIS_COMM, &stat);
                              for (j = 0; j < pixn * 3; j++)
                          ...
                          dis_c_v = sqrt(SQR(center[0] - vr->view_point_d[0]) +
                                         SQR(center[1] - vr->view_point_d[1]) +
                                         SQR(center[2] - vr->view_point_d[2]));
                          if (mynode != 0) {
                            HECMW_Send(&dis_c_v, 1, HECMW_DOUBLE, MASTER_PE, 0,
                                       VIS_COMM);
                            pe_id = (int *)HECMW_calloc(pesize, sizeof(int));
                            if (pe_id == NULL) HECMW_vis_memory_exit("pe_id");
                            HECMW_Recv(pe_id, pesize, HECMW_INT, MASTER_PE,
                                       HECMW_ANY_TAG, VIS_COMM, &stat);
                          }
                          if (mynode == 0) {
                            ndis_c_v =
                                (double *)HECMW_calloc(pesize, sizeof(double));
                            if (ndis_c_v == NULL)
                              HECMW_vis_memory_exit("ndis_c_v");
                            ndis_c_v[0] = dis_c_v;
                            for (i = 1; i < pesize; i++) {
                              HECMW_Recv(&dis_c_v, 1, HECMW_DOUBLE, i,
                                         HECMW_ANY_TAG, VIS_COMM, &stat);
                              ndis_c_v[i] = dis_c_v;
                            }
                            pe_id = (int *)HECMW_calloc(pesize, sizeof(int));
                            if (pe_id == NULL) HECMW_vis_memory_exit("pe_id");
                            for (i = 0; i < pesize; i++) pe_id[i] = i;
                            ...
                            for (i = 1; i < pesize; i++)
                              HECMW_Send(pe_id, pesize, HECMW_INT, i, 0,
                                         VIS_COMM);
                          }
                          /*	}
                           */

                          composite_subimage_vr(pesize, pe_id, pixn, n_subimage,
                                                n_subopa, subimage);
                          ...
                          if (mynode != 0)
                            HECMW_Send(subimage, pixn * 3, HECMW_DOUBLE, 0, 0,
                                       VIS_COMM);
                          if (mynode == 0) {
                            for (j     = 0; j < pix0_num * 3; j++)
                              image[j] = subimage[j];
                            for (i = 1; i < pesize; i++) {
                              HECMW_Recv(subimage, pix_num * 3, HECMW_DOUBLE, i,
                                         HECMW_ANY_TAG, VIS_COMM, &stat);
                              for (j = 0; j < pix_num * 3; j++)
                                image[pix0_num * 3 + (i - 1) * pix_num * 3 +
                                      j] = subimage[j];
                            }
                          }
                          HECMW_free(subimage);
                        ...
                        if (mynode == 0) {
                          ...
                          t3 = HECMW_Wtime();
                          fprintf(stderr,
                                  "The combining time is %lf the total time up "
                                  "to now is %lf\n",
                                  t3 - t2, t3 - t1);
                          t2 = t3;
                          if (vr->fixed_scale_mark == 1)
                            generate_color_bar(
                                vr->scale_marking_on, vr->font_size,
                                vr->color_bar_style, vr->mark_0_on,
                                vr->color_mapping_bar_on, vr->xr, vr->yr,
                                vr->font_color, vr->color_system_type,
                                vr->color_mapping_style, vr->interval_point,
                                vr->interval_mapping_num, vr->num_of_scale,
                                tmincolor, tmaxcolor, tmincolor, tmaxcolor,
                                image);
                            ...
                            generate_color_bar(
                                ...
                                vr->color_mapping_style, vr->interval_point,
                                vr->interval_mapping_num, vr->num_of_scale,
                                tmincolor, tmaxcolor, org_mincolor,
                                org_maxcolor, image);
            ...
            sprintf(fname, "%s.%d.%d.bmp", outfile1, *timestep, ii);
                          ...
                          if (!FP)
                            HECMW_vis_print_exit(
                                "ERROR: HEC-MW-VIS-E0009: Cannot open output "
                                "file");

                          header.bfSize = 54 + 3 * vr->xr * vr->yr;
#ifdef CONVERSE_ORDER
                          header.bfSize = change_unsigned_int_order(
                              54 + 3 * vr->xr * vr->yr);
#endif
...
#ifdef CONVERSE_ORDER
                          info.biSizeImage =
                              change_unsigned_int_order(3 * vr->xr * vr->yr);
#endif
                          ...
                          fwrite(&header.bfReserved1,
                                 sizeof(unsigned short int), 1, FP);
                          fwrite(&header.bfReserved2,
                                 sizeof(unsigned short int), 1, FP);
                          fwrite(&header.bfOffBits, sizeof(unsigned int), 1,
                                 FP);
                          fwrite(&info, 40, 1, FP);
                          for (j = 0; j < vr->yr; j++)
                            for (i = vr->xr - 1; i >= 0; i--) {
                              if ((image[(j * vr->xr + i) * 3] < 0.1) &&
                                  (image[(j * vr->xr + i) * 3 + 1] < 0.1) &&
                                  (image[(j * vr->xr + i) * 3 + 2] < 0.1)) {
                                ...
                                gi = (int)(image[(j * vr->xr + i) * 3 + 1] *
                                           256);
                                bi = (int)(image[(j * vr->xr + i) * 3 + 2] *
                                           256);
                              }
                              if (ri < 0) ri   = 0;
                              if (ri > 255) ri = 255;
  ...
  if (mynode == 0) {
    ...
    t3 = HECMW_Wtime();
    fprintf(stderr,
            " the time for output is %lf the total time of the module is %lf\n",
            t3 - t2, t3 - t1);
  }
  return;
}
