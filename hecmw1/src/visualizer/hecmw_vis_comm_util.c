...
void whole_copy_array(int *recv_num, int *global_recv_num, int mynode,
                      int pesize, HECMW_Comm repart_comm) {
  int i, j;
  int *tmp_recv;
  HECMW_Status stat;

  if (mynode == 0) {
    for (j = 0; j < pesize + 1; j++) global_recv_num[j] = recv_num[j];
    tmp_recv = (int *)HECMW_calloc(pesize + 1, sizeof(int));
    if (tmp_recv == NULL) HECMW_vis_memory_exit("tmp_recv");
    for (i = 1; i < pesize; i++) {
      HECMW_Recv(tmp_recv, pesize + 1, HECMW_INT, i, HECMW_ANY_TAG, repart_comm,
                 &stat);
      for (j                                  = 0; j < pesize + 1; j++)
        global_recv_num[i * (pesize + 1) + j] = tmp_recv[j];
    }
    for (i = 1; i < pesize; i++)
      HECMW_Send(global_recv_num, (pesize + 1) * pesize, HECMW_INT, i, 0,
                 repart_comm);
    HECMW_free(tmp_recv);
  } else {
    HECMW_Send(recv_num, pesize + 1, HECMW_INT, 0, 0, repart_comm);
    HECMW_Recv(global_recv_num, (pesize + 1) * pesize, HECMW_INT, 0,
               HECMW_ANY_TAG, repart_comm, &stat);
  }
  return;
}

int stack_part_send_recv(int neibpetot, int *neibpe, int *stack_import,
  ...
  if (nflag == 0) {
    sta1 =
        (HECMW_Status *)HECMW_calloc(HECMW_STATUS_SIZE, sizeof(HECMW_Status));
    if (sta1 == NULL) HECMW_vis_memory_exit("HECMW_STATUS: stat1");
    sta2 =
        (HECMW_Status *)HECMW_calloc(HECMW_STATUS_SIZE, sizeof(HECMW_Status));
    if (sta2 == NULL) HECMW_vis_memory_exit("HECMW_STATUS: stat2");
    if ((req1 = (HECMW_Request *)HECMW_calloc(neibpetot,
                                              sizeof(HECMW_Request))) == NULL)
      HECMW_vis_memory_exit("HECMW_STATUS: req1");
    if ((req2 = (HECMW_Request *)HECMW_calloc(neibpetot,
                                              sizeof(HECMW_Request))) == NULL)
  ...
  for (neib = 0; neib < neibpetot; neib++) {
    HECMW_Irecv(&stack_export[neib], 1, HECMW_INT, neibpe[neib], 0, repart_comm,
                &req2[neib]);
...
int stack_whole_send_recv(int pesize, int *stack_export, int *stack_import,
...
if (nflag == 0) {
sta1 = (HECMW_Status *)HECMW_calloc(HECMW_STATUS_SIZE*(pesize-1),
sizeof(HECMW_Status));
if (sta1 == NULL)
          HECMW_vis_memory_exit("HECMW_STATUS: stat1");
sta2 = (HECMW_Status *)HECMW_calloc(HECMW_STATUS_SIZE*(pesize-1),
sizeof(HECMW_Status));
if (sta2 == NULL)
...
HECMW_free(req2);
  ...
  for (i = 0; i < pesize; i++) {
    if (i != my_rank) {
      ...
      for (j = 0; j < pesize; j++) {
        if (j != my_rank) {
          HECMW_Recv(&stack_import[j + 1], 1, HECMW_INT, j, HECMW_ANY_TAG,
                     repart_comm, &stat);
        }
      }
      stack_import[0] = 0;
...
int int_part_send_recv(int n, int neibpetot, int *neibpe, int *stack_import,
                       int *nod_import, int *stack_export, int *nod_export,
  ...
  if ((ws == NULL) || (wr == NULL)) HECMW_vis_memory_exit("send_recv: ws, wr");
  if (nflag == 0) {
    sta1 =
        (HECMW_Status *)HECMW_calloc(HECMW_STATUS_SIZE, sizeof(HECMW_Status));
    if (sta1 == NULL) HECMW_vis_memory_exit("HECMW_STATUS: stat1");
  ...
  for (neib = 0; neib < neibpetot; neib++) {
    ...
    HECMW_Isend(&ws[stack_export[neib]], inum, HECMW_INT, neibpe[neib], 0,
                repart_comm, &req1[neib]);
  }

  /* RECEIVE */
  for (neib = 0; neib < neibpetot; neib++) {
    inum = stack_import[neib + 1] - stack_import[neib];
    HECMW_Irecv(&wr[stack_import[neib]], inum, HECMW_INT, neibpe[neib], 0,
                repart_comm, &req2[neib]);
  }
  HECMW_Barrier(repart_comm);
  /*
...
int double_part_send_recv(int n, int neibpetot, int *neibpe, int *stack_import,
                          int *nod_import, int *stack_export, int *nod_export,
                          double *x, HECMW_Comm repart_comm, int my_rank) {
  HECMW_Status *sta1, *sta2;
  HECMW_Request *req1, *req2;
  ...
  if ((ws == NULL) || (wr == NULL)) HECMW_vis_memory_exit("send_recv: ws, wr");
  if (nflag == 0) {
    sta1 = (HECMW_Status *)HECMW_calloc(HECMW_STATUS_SIZE * neibpetot,
                                        sizeof(HECMW_Status));
    if (sta1 == NULL) HECMW_vis_memory_exit("send_recv: sta1");
    sta2 = (HECMW_Status *)HECMW_calloc(HECMW_STATUS_SIZE * neibpetot,
                                        sizeof(HECMW_Status));
    if (sta2 == NULL) HECMW_vis_memory_exit("send_recv: sta12");
    if ((req1 = (HECMW_Request *)HECMW_calloc(neibpetot,
                                              sizeof(HECMW_Request))) == NULL)
  ...
  for (neib = 0; neib < neibpetot; neib++) {
    ...
    if (inum > 0) {
       ...
       * neibpe[neib]);
       */
      HECMW_Isend(&ws[stack_export[neib]], inum, HECMW_DOUBLE, neibpe[neib], 0,
                  repart_comm, &req1[neib]);
    }
  }
...
void int_whole_send_recv(int n1, int n2, int pesize, int *stack_import,
                         int *nod_import, int *stack_export, int *nod_export,
                         int *x, int *y, HECMW_Comm repart_comm, int my_rank) {
  ...
  if ((ws == NULL) || (wr == NULL)) HECMW_vis_memory_exit("ws, wr");
  if (nflag == 0) {
    /*    sta1 = (HECMW_Status *)HECMW_calloc(HECMW_STATUS_SIZE,
sizeof(HECMW_Status));
if (sta1 == NULL) {
...
exit(1);
  ...
  for (neib = 0; neib < pesize; neib++) {
    ...
    if (neib != my_rank) {
      ...
      HECMW_Isend(&ws[stack_export[neib]], inum, HECMW_INT, neib, 0,
                  repart_comm, &req1[neib]);
    }
  }
  ...
  for (neib = 0; neib < pesize; neib++) {
    ...
    inum = stack_import[neib + 1] - stack_import[neib];
    if (neib != my_rank)
      HECMW_Irecv(&wr[stack_import[neib]], inum, HECMW_INT, neib, 0,
                  repart_comm, &req2[neib]);
  }
  HECMW_Barrier(repart_comm);
  /*
HECMW_Waitall(neibpetot, req2, sta2);
  ...
  for (neib = 0; neib < pesize; neib++) {
    ...
    if (neib != my_rank) {
      ...
      for (k = 0; k < inum; k++)
        /*		for (k = stack_import[neib]; k < stack_import[neib]+inum;
         * k++)
         */
        y[nod_import[stack_import[neib] + k]] =
            x[nod_export[stack_export[neib] + k]];
    }
  }
  HECMW_Barrier(repart_comm);
  /*
...
void int2_whole_send_recv(int n1, int n2, int pesize, int *stack_import,
  ...
  if (nflag == 0) {
    sta1 = (HECMW_Status *)HECMW_calloc(HECMW_STATUS_SIZE * pesize,
                                        sizeof(HECMW_Status));
    if (sta1 == NULL) HECMW_vis_memory_exit("send_recv: sta1");
    sta2 = (HECMW_Status *)HECMW_calloc(HECMW_STATUS_SIZE * pesize,
                                        sizeof(HECMW_Status));
    if (sta2 == NULL) HECMW_vis_memory_exit("send_recv: sta2");
  ...
  for (neib = 0; neib < pesize; neib++) {
    if (neib != my_rank) {
      inum = stack_export[neib + 1] - stack_export[neib];
      if (inum != 0) {
        HECMW_Isend(&x[stack_export[neib]], inum, HECMW_INT, neib, 0,
                    repart_comm, &req1[neib]);
        /*		  fprintf(stderr, "sending from PE %d to PE %d nodes
         * %d\n", my_rank, neib, inum);
         */
      }
    }
  ...
  for (neib = 0; neib < pesize; neib++) {
    ...
    if ((neib != my_rank) && (inum != 0)) {
      HECMW_Irecv(&y[stack_import[neib]], inum, HECMW_INT, neib, 0, repart_comm,
                  &req2[neib]);
...
void double2_whole_send_recv(int n1, int n2, int pesize, int *stack_import,
  ...
  for (neib = 0; neib < pesize; neib++) {
    if (neib != my_rank) {
      ...
      if (inum != 0) {
         ...
         * %d\n", my_rank, neib, inum);
         */
        HECMW_Isend(&x[stack_export[neib]], inum, HECMW_DOUBLE, neib, 0,
                    repart_comm, &req1[neib]);
      }
    }
  }
  ...
  for (neib = 0; neib < pesize; neib++) {
    ...
    if ((neib != my_rank) && (inum != 0)) {
       ...
       * neib, inum);
       */
      HECMW_Irecv(&y[stack_import[neib]], inum, HECMW_DOUBLE, neib, 0,
                  repart_comm, &req2[neib]);
    }
  }
      ...
      for (j = 0; j < pesize; j++) {
        if (j != my_rank) {
          ...
          if (inum != 0)
            ...
            HECMW_Recv(&y[stack_import[j]], inum, HECMW_INT, j, HECMW_ANY_TAG,
                       repart_comm, &stat);
        }
      }
    }
...
void double_whole_send_recv(int n1, int n2, int pesize, int *stack_import,
                            int *nod_import, int *stack_export, int *nod_export,
...
exit(1);
  ...
  for (neib = 0; neib < pesize; neib++) {
    ...
    if (neib != my_rank) {
      ...
      HECMW_Isend(&ws[stack_export[neib]], inum, HECMW_DOUBLE, neib, 0,
                  repart_comm, &req1[neib]);
    }
  }
