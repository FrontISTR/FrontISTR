...
#ifdef surface_vr
int find_surface_point(int index, VR_data *vd, double in[3], double out[3],
                       double *surf_p) {
  int i, j, k, m, n1, n2, flag, flag_inside;
  double fp[3][3], n_f[4], v_f[3], d, point[3], s[3], sina, cosa, ss, t, color,
      dis[3], dist, n_norm, v_norm, nv_sign;
  ...
  for (m = 0; m < vd->surface[index].num; m++) {
    ...
    for (i = 0; i < 3; i++) v_f[i] = in[i] - out[i];
    v_norm = sqrt(v_f[0] * v_f[0] + v_f[1] * v_f[1] + v_f[2] * v_f[2]);
    if (fabs(v_norm) > EPSILON) {
      for (i = 0; i < 3; i++) v_f[i] /= v_norm;
    }
    nv_sign = n_f[0] * v_f[0] + n_f[1] * v_f[1] + n_f[2] * v_f[2];
    if (nv_sign < 0.0) {
      for (i = 0; i < 3; i++) n_f[i] = -n_f[i];
    }
    n_f[3] = -n_f[0] * fp[0][0] - n_f[1] * fp[0][1] - n_f[2] * fp[0][2];
    /*find intersection point*/
    if (fabs(n_f[0] * (out[0] - in[0]) + n_f[1] * (out[1] - in[1]) +
             n_f[2] * (out[2] - in[2])) > EPSILON) {
      t = (-n_f[3] - n_f[0] * in[0] - n_f[1] * in[1] - n_f[2] * in[2]) /
          (n_f[0] * (out[0] - in[0]) + n_f[1] * (out[1] - in[1]) +
           n_f[2] * (out[2] - in[2]));
      if ((t > -EPSILON) && (t < 1.0)) {
        for (j = 0; j < 3; j++) point[j] = in[j] + t * (out[j] - in[j]);
        ...
        for (j = 0; j < 3; j++) {
          ...
          n2              = j + 1;
          if (n2 == 3) n2 = 0;
          if ((fabs(point[0] - fp[n1][0]) < EPSILON) &&
              (fabs(point[1] - fp[n1][1]) < EPSILON) &&
              (fabs(point[2] - fp[n1][2]) < EPSILON)) {
            flag_inside = 1;
            break;
          } else
            cosa =
                ((point[0] - fp[n1][0]) * (fp[n2][0] - fp[n1][0]) +
                 (point[1] - fp[n1][1]) * (fp[n2][1] - fp[n1][1]) +
                 (point[2] - fp[n1][2]) * (fp[n2][2] - fp[n1][2])) /
                (sqrt(SQR(point[0] - fp[n1][0]) + SQR(point[1] - fp[n1][1]) +
                      SQR(point[2] - fp[n1][2])) *
                 sqrt(SQR(fp[n2][0] - fp[n1][0]) + SQR(fp[n2][1] - fp[n1][1]) +
                      SQR(fp[n2][2] - fp[n1][2])));
          sina = sqrt(1 - cosa * cosa);
          s[j] = sqrt(SQR(fp[n2][0] - fp[n1][0]) + SQR(fp[n2][1] - fp[n1][1]) +
                      SQR(fp[n2][2] - fp[n1][2])) *
                 sqrt(SQR(point[0] - fp[n1][0]) + SQR(point[1] - fp[n1][1]) +
                      SQR(point[2] - fp[n1][2])) *
                 sina / 2.0;
        }
        cosa = ((fp[2][0] - fp[0][0]) * (fp[1][0] - fp[0][0]) +
                (fp[2][1] - fp[0][1]) * (fp[1][1] - fp[0][1]) +
                (fp[2][2] - fp[0][2]) * (fp[1][2] - fp[0][2])) /
               (sqrt(SQR(fp[2][0] - fp[0][0]) + SQR(fp[2][1] - fp[0][1]) +
                     SQR(fp[2][2] - fp[0][2])) *
                sqrt(SQR(fp[1][0] - fp[0][0]) + SQR(fp[1][1] - fp[0][1]) +
                     SQR(fp[1][2] - fp[0][2])));
        sina = sqrt(1 - cosa * cosa);
        ss   = sqrt(SQR(fp[1][0] - fp[0][0]) + SQR(fp[1][1] - fp[0][1]) +
                  SQR(fp[1][2] - fp[0][2])) *
             sqrt(SQR(fp[2][0] - fp[0][0]) + SQR(fp[2][1] - fp[0][1]) +
                  SQR(fp[2][2] - fp[0][2])) *
             sina / 2.0;
        ...
        if (flag_inside == 1) {
          flag = 1;
          for (j = 0; j < 3; j++) {
            dis[j] = sqrt(SQR(point[0] - fp[j][0]) + SQR(point[1] - fp[j][1]) +
                          SQR(point[2] - fp[j][2]));
            if (dis[j] < EPSILON) {
              flag  = 0;
          ...
          if (flag == 1) {
            ...
            color = vd->surface[index].surf_data[m * 12 + 9] * dist / dis[0] +
                    vd->surface[index].surf_data[m * 12 + 10] * dist / dis[1] +
  ...
  if (num_surp > 0) {
    ...
    for (i  = 0; i < num_surp; i++)
      dd[i] = sqrt(SQR(surf_p1[i * 7 + 0] - in[0]) +
                   SQR(surf_p1[i * 7 + 1] - in[1]) +
                   SQR(surf_p1[i * 7 + 2] - in[2]));
    dd_norm =
        sqrt(SQR(out[0] - in[0]) + SQR(out[1] - in[1]) + SQR(out[2] - in[2]));
    for (i = 0; i < num_surp - 1; i++)
      for (j = i + 1; j < num_surp; j++) {
        if (dd[i] > dd[j]) {
          ...
          dd[i]  = dd[j];
          dd[j]  = tmp_dd;
          for (k = 0; k < 7; k++) tmp_p[k] = surf_p1[i * 7 + k];
          for (k = 0; k < 7; k++) surf_p1[i * 7 + k] = surf_p1[j * 7 + k];
          for (k = 0; k < 7; k++) surf_p1[j * 7 + k] = tmp_p[k];
        }
      }
    for (i = 0; i < num_surp; i++) o_flag[i] = 1;
    for (i = 1; i < num_surp; i++) {
      if (fabs(dd[i] - dd[i - 1]) < EPSILON * dd_norm * 100.0) o_flag[i] = 0;
    }
    /*        for(i=1;i<num_surp;i++)
o_flag[i]=0;
     */
    ...
    for (i = 0; i < num_surp; i++) {
      if (o_flag[i] == 1) {
        for (j = 0; j < 7; j++) surf_p[num_surp1 * 7 + j] = surf_p1[i * 7 + j];
        num_surp1++;
      }
    }
    HECMW_free(dd);
...
void compute_color_vr(int current_ijk[3], int color_mapping_style,
                      ...
                      int color_system_type, int num_of_lights,
                      double *light_point, double k_ads[3], int r_level[3],
                      double orig_xyz[3], double r_dxyz[3], double *var,
                      double *grad_var, double accum_rgba[4], double mincolor,
                      double maxcolor, double grad_minmax[2],
                      double feap_minmax[2], double feai_minmax[2],
                      double dis_minmax[2], double *opa_table,
                      double in_point[3], double out_point[3],
                      double tav_length, int time_step, int print_flag) {
  int i, j, k;
  double a_current;
  double color[3];
  double value;
  ...
  length =
      sqrt(SQR(out_point[0] - in_point[0]) + SQR(out_point[1] - in_point[1]) +
           SQR(out_point[2] - in_point[2]));
  coff_i = length / tav_length;
  index  = current_ijk[2] * r_level[0] * r_level[1] +
          current_ijk[1] * r_level[0] + current_ijk[0];
  ...
  vv[1 * 3] = vv[5 * 3] = vv[6 * 3] = vv[2 * 3] =
      orig_xyz[0] + (current_ijk[0] + 1) * r_dxyz[0];
  vv[0 * 3 + 2] = vv[1 * 3 + 2] = vv[2 * 3 + 2] = vv[3 * 3 + 2] =
      orig_xyz[2] + current_ijk[2] * r_dxyz[2];
  vv[4 * 3 + 2] = vv[5 * 3 + 2] = vv[6 * 3 + 2] = vv[7 * 3 + 2] =
      orig_xyz[2] + (current_ijk[2] + 1) * r_dxyz[2];
  vv[0 * 3 + 1] = vv[1 * 3 + 1] = vv[5 * 3 + 1] = vv[4 * 3 + 1] =
      orig_xyz[1] + current_ijk[1] * r_dxyz[1];
  vv[3 * 3 + 1] = vv[2 * 3 + 1] = vv[6 * 3 + 1] = vv[7 * 3 + 1] =
      orig_xyz[1] + (current_ijk[1] + 1) * r_dxyz[1];

  for (m = 0; m < 8; m++) {
    i     = m % 2;
    j     = (m / 2) % 2;
    k     = (m / 2) / 2;
    index = (current_ijk[2] + k) * (r_level[0] + 1) * (r_level[1] + 1) +
            (current_ijk[1] + j) * (r_level[0] + 1) + current_ijk[0] + i;
 ...
 */
  ...
  if (transfer_function_style == 3) {
    ...
    for (i = 0; i < num_of_features; i++) {
      ...
      if (t < fea_point[i * 3 + 1])
        a_current += fea_point[i * 3 + 2] * (fea_point[i * 3 + 1] - t) /
                     fea_point[i * 3 + 1];
    }
  }
  ...
  if (color_mapping_style == 3) {
    if (value < interval_point[0])
      ...
      for (i = 1; i < interval_mapping_num + 1; i++) {
        if ((value <= interval_point[i * 2]) &&
            (value > interval_point[(i - 1) * 2])) {
          value = (value - interval_point[(i - 1) * 2]) /
 ...
 */
 ...
 cosalpha=sqrt(1.0-pow(inprodLN,2));
  ...
  costheta = inprodLN * inprodVN -
             sqrt(1.0 - inprodLN * inprodLN) * sqrt(1.0 - inprodVN * inprodVN);
