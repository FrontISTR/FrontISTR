 ...
 *****************************************************************************/
/*----------------------------------------------------------------------
 * Subroutines in this file on isosurface generation for hexahedra by Marching
 *Cubes is based
 * on the revision of Dr. Yuriko Takeshima's codes when she was working part
 *time in RIST
 *----------------------------------------------------------------------*/
...
int chk_node_data(struct visual_buf *v, int s_comp, int c_comp) {
  ...
  for (i = 0; i < v->mesh->n_neighbor_pe; i++) {
    ...
    HECMW_Send(global_node_id, n_export_node, HECMW_INT, ne, 0,
               geofem_app_comm);
    ...
    HECMW_Recv(&imp_node, 1, HECMW_INT, ne, HECMW_ANY_TAG, geofem_app_comm,
               &stat);
...
int find_line_segment(double f[3][3], double c[3], double isocolor,
                      double iso_p[6]) {
  int j, m, j1;
  int pnum, flag;
  ...
  for (j = 0; j < 3; j++) {
    j1              = j + 1;
    if (j1 == 3) j1 = 0;
    if ((fabs(c[j] - c[j1]) < 0.0000001) &&
        (fabs(c[j] - isocolor) < 0.0000001)) {
      /*this edge is isoline*/
      flag = 1;

      for (m = 0; m < 3; m++) iso_p[m] = f[j][m];
      for (m = 0; m < 3; m++) iso_p[m + 3] = f[j1][m];
      return 1;
    } else if (((c[j] >= isocolor) && (c[j1] < isocolor)) ||
               ((c[j] < isocolor) && (c[j1] >= isocolor))) {
      pnum++;
      for (m = 0; m < 3; m++)
        iso_p[pnum * 3 + m] =
            f[j][m] + (isocolor - c[j]) / (c[j1] - c[j]) * (f[j1][m] - f[j][m]);
    }
  }
  if (pnum == 1) flag = 1;
...
void line_find(double isocolor, double c[3], Fgeom g[3], int k,
               Isohead *isohead) {
  Isoline *p1, *p2;
  int j, m, j1;
  int pnum;
  ...
  for (j = 0; j < 3; j++) {
    ...
    if ((fabs(c[j] - c[j1]) < 0.0000001) &&
      ...
      pnum++;
      isopoint[pnum].x =
          g[j].x + (isocolor - c[j]) / (c[j1] - c[j]) * (g[j1].x - g[j].x);
      isopoint[pnum].y =
          g[j].y + (isocolor - c[j]) / (c[j1] - c[j]) * (g[j1].y - g[j].y);
      isopoint[pnum].z =
          g[j].z + (isocolor - c[j]) / (c[j1] - c[j]) * (g[j1].z - g[j].z);
    }
  }
  if (pnum == 1) {
    isohead[k].linenum++;
