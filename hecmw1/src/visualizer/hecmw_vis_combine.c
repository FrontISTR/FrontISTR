		...
		double disp_max[5], int pesize)
{
	...
	for(ii=0;ii<sf[0].surface_style;ii++) {
		for(i=0;i<result[ii].n_vertex;i++) {
			...
			tmp=sqrt(result[ii].disp[i*3]*result[ii].disp[i*3]+result[ii].disp[i*3+1]*result[ii].disp[i*3+1]+
					result[ii].disp[i*3+2]*result[ii].disp[i*3+2]);
			if(l_disp_min[0]>tmp) l_disp_min[0]=tmp;
			if(l_disp_max[0]<tmp) l_disp_max[0]=tmp;
			if(l_disp_min[4]>result[ii].color[i]) l_disp_min[4]=result[ii].color[i];
			if(l_disp_max[4]<result[ii].color[i]) l_disp_max[4]=result[ii].color[i];
		}
	}
	if(pesize>1) {
		...
		Result *result, char *outfile,  HECMW_Comm VIS_COMM)
{
	...
	double    range[6], minx, miny, minz, maxx, maxy, maxz, tminx, tminy, tminz, tmaxx, tmaxy, tmaxz, trange[6];
	...
	if (mynode == MASTER_PE) {
		...
		for(i=0;i<data->nn_component;i++) {
			if(color_list[i]==1) {
				fprintf(stderr,"for the color component %s ,the current minimum color=%e  maximum color=%e\n",data->node_label[i], mivalue[i],mavalue[i]);
				/*  fprintf(stderr, "Please input the mincolor and maxcolor you would like:\n");
		...
		if (n_vertex[0]>0) {
			...
			for(ii=0;ii<sf[0].surface_style;ii++) {
				if(result[ii].n_vertex>0) {
					for(i=0;i<result[ii].n_vertex;i++) {
						if(sf[1].output_type==1)
							fprintf(outfp, "%d %e %e %e\n", i+s_vertex+1, result[ii].vertex[i*3], result[ii].vertex[i*3+1],
	   ...
	   plist[i*3+2]=v3;
			...
			for(ii=0;ii<sf[0].surface_style;ii++) {
				if(result[ii].n_patch>0) {
					for(i=0;i<result[ii].n_patch;i++) {
						if(sf[1].output_type==1)
							fprintf(outfp, "%d 1 tri %d %d %d\n", i+s_patch+1, result[ii].patch[i*3], result[ii].patch[i*3+1],result[ii].patch[i*3+2]);
						else if(sf[1].output_type==2)
		...
		for (i = 1; i < pesize; i++) {
			if (n_patch[i]>0) {
				...
				for(j=0;j<n_patch[i];j++) {
					if(sf[1].output_type==1)
						fprintf(outfp, "%d 1 tri %d %d %d\n", pbase+j+1,pplist[j*3]+nbase, pplist[j*3+1]+nbase,
								pplist[j*3+2]+nbase);
					else if(sf[1].output_type==2)
	...
	if(mynode==0) {
		if(sf[1].output_type==1) {
			if(sf[1].deform_display_on==0) {
				...
				fprintf(outfp, "4 1 1 1 1\n");
				fprintf(outfp, "data, unit_unknown\n");
				fprintf(outfp, "disp_x, unit_unknown\n");
				fprintf(outfp, "disp_y, unit_unknown\n");
				fprintf(outfp, "disp_z, unit_unknown\n");
			}
		}
		if(sf[1].output_type==4) {
#ifdef old_version
			...
			fprintf(outfp, "%e, %e, %e,\n", (tminx+tmaxx)/2.0, (tminy+tmaxy)/2.0, (tminz+tmaxz)/2.0);
		...
		if (n_vertex[0]>0) {
			...
			if((sf[1].output_type==4) && (sf[1].deform_display_on!=1)) {
				...
				fprintf(outfp, "%e %e %e,\n", mivalue[sf[1].color_comp],mavalue[sf[1].color_comp], tmp);
					...
					for(i=0;i<result[ii].n_vertex;i++) {
						...
						if(sf[ii+1].display_method!=4) {
							if(sf[1].normalize_flag==1) {
								color=(result[ii].color[i]-mivalue[sf[ii+1].color_comp])/(mavalue[sf[ii+1].color_comp]-mivalue[sf[ii+1].color_comp]);
						...
						if(sf[1].output_type==1) {
							...
							if(sf[ii+1].deform_display_on==0)
								fprintf(outfp, "%d, %e,\n",  i+s_vertex+1, result[ii].color[i]);
							else if(sf[ii+1].deform_display_on==1) {
								...
								tdisp[(i+s_vertex)*4]=sqrt(tdisp[(i+s_vertex)*4+1]*tdisp[(i+s_vertex)*4+1]+tdisp[(i+s_vertex)*4+2]
								                                                                                 *tdisp[(i+s_vertex)*4+2]+tdisp[(i+s_vertex)*4+3]*tdisp[(i+s_vertex)*4+3]);
							}
						}
						else if(sf[1].output_type==2) {

							if(color<=0.25*rgbrange[0]) {
								r=g=0.0; b=(0.5*rgbrange[0]-color)*2/rgbrange[0];
							}
							else if((color>0.25*rgbrange[0]) && (color<=0.5*rgbrange[0])) {
								r=0.0; g=(color-0.25*rgbrange[0])*2/rgbrange[0];
								b=(0.5*rgbrange[0]-color)*2/rgbrange[0];
							}
							else if((color>0.5*rgbrange[0]) && (color<((1-rgbrange[0]*0.5)/2
									+0.5*rgbrange[0]))) {
								r=(color-0.5*rgbrange[0])/((1-rgbrange[0]*0.5)/2)*0.5;
								g=(((1-rgbrange[0]*0.5)/2+0.5*rgbrange[0])-color)/((1-rgbrange[0]*0.5)/2)*0.5; b=0.0;
							}
							else if(color>=((1-rgbrange[0]*0.5)/2+0.5*rgbrange[0])) {
								r=(color-0.5*rgbrange[0])/((1-rgbrange[0]*0.5)/2)*0.5; g=b=0.0;
							}
		...
		for (i = 1; i < pesize; i++) {
			if (n_vertex[i]>0) {
				...
				HECMW_Recv(ccolor, n_vertex[i], HECMW_DOUBLE, i, HECMW_ANY_TAG,
						VIS_COMM, &stat);
				HECMW_Recv(vc_style, n_vertex[i], HECMW_INT, i, HECMW_ANY_TAG,
						VIS_COMM, &stat);
				...
				for(j=0;j<n_vertex[i];j++) {
					...
					if(vc_style[j]!=4) {
						if(sf[1].output_type==2) {
							color=(value-mivalue[v_colorid[j]])/(mavalue[v_colorid[j]]-mivalue[v_colorid[j]]);
							if(color>1.0) color=1.0;
							if(color<0.0) color=0.0;
						}
						if((sf[1].output_type==1) || (sf[1].output_type==4)){
							...
							if(sf[1].normalize_flag==1)
								color=(color-mivalue[v_colorid[j]])/(mavalue[v_colorid[j]]-mivalue[v_colorid[j]]);
						}
					}
	  ...
	  if(color<0.0) color=0.0;
					...
					if(sf[1].output_type==1) {
						if(sf[1].deform_display_on==0)
							...
							fprintf(outfp, "%d %e %e %e %e\n", nbase+j+1, color, cdisp[j*3], cdisp[j*3+1], cdisp[j*3+2]);
					}
					else if(sf[1].output_type==4) {
						if(sf[1].deform_display_on==0)
							fprintf(outfp, "%d, %e,\n",  nbase+j+1, color);
							...
							tdisp[(nbase+j)*4]=sqrt(cdisp[j*3]*cdisp[j*3]+cdisp[j*3+1]*cdisp[j*3+1]+cdisp[j*3+2]*cdisp[j*3+2]);
						}
					}
					else if(sf[1].output_type==2)
						fprintf(outfp, "%e %e %e\n", r,g,b);
