...
int find_surface_point(double fp[3][3], double point_o[3],
                       double view_point_d[3], double point[3], double n_f[4],
                       double v_f[9], double c_value[3], double *value,
                       double normal[9], int normal_flag, int smooth_shading) {
  ...
  for (i = 0; i < 3; i++) v_f[i] = view_point_d[i] - point_o[i];
  ...
  if (smooth_shading == 0)
    for (i = 0; i < 3; i++) normal[i] = n_f[i];
  /*find intersection point*/
  if (fabs(n_f[0] * (point_o[0] - view_point_d[0]) +
           n_f[1] * (point_o[1] - view_point_d[1]) +
           n_f[2] * (point_o[2] - view_point_d[2])) > EPSILON) {
    t = (-n_f[3] - n_f[0] * view_point_d[0] - n_f[1] * view_point_d[1] -
         n_f[2] * view_point_d[2]) /
        (n_f[0] * (point_o[0] - view_point_d[0]) +
         n_f[1] * (point_o[1] - view_point_d[1]) +
         n_f[2] * (point_o[2] - view_point_d[2]));
    if (t > -EPSILON) {
      for (j     = 0; j < 3; j++)
      ...
      for (j = 0; j < 3; j++) {
        ...
        cosa = ((point[0] - fp[n1][0]) * (fp[n2][0] - fp[n1][0]) +
      ...
      if (flag_inside == 0) {
        ...
        if (fabs(ss - s[0] - s[1] - s[2]) < EPSILON) {
          ...
          *value = c_value[0] / (dis[0] * dist) + c_value[1] / (dis[1] * dist) +
                   c_value[2] / (dis[2] * dist);
          if (smooth_shading == 1) {
            for (j = 0; j < 3; j++) {
              tmp_p[j] = normal[0 * 3 + j] / (dis[0] * dist) +
                         normal[1 * 3 + j] / (dis[1] * dist) +
                         normal[2 * 3 + j] / (dis[2] * dist);
            }
            for (j = 0; j < 3; j++) normal[j] = tmp_p[j];
          }
        }
      }
    }
...
int find_point_depth(double fp[3][3], double point_o[3], double view_point_d[3],
                     double n_f[4], double point[3], int normal_flag) {
  int i, j, flag_inside;
...
int find2_point_depth(double fp[3][3], double point_o[3],
                      double view_point_d[3], double point[3]) {
  int i, j, flag_inside;
  double v_f[3], t, n_norm, v_norm, nv_sign;
  double n_f[4];
...
void compute_color_sf(double p[3], double value, double n[3],
                      ...
                      double *light_point, double k_ads[3],
                      double accum_rgba[4], double mincolor, double maxcolor,
                      int display_method) {
  int i, j;
...
cosalpha=sqrt(1.0-pow(inprodLN,2));
    ...
    if (cosalpha > 0) {
      r += color[0] * (k_ads[0] + k_ads[1] * cosalpha +
                       k_ads[2] * costheta * costheta * costheta * costheta *
                           costheta * costheta);
      g += color[1] * (k_ads[0] + k_ads[1] * cosalpha +
                       k_ads[2] * costheta * costheta * costheta * costheta *
                           costheta * costheta);
      b += color[2] * (k_ads[0] + k_ads[1] * cosalpha +
                       k_ads[2] * costheta * costheta * costheta * costheta *
                           costheta * costheta);
    }
