...
static int ucd_print(const struct hecmwST_local_mesh *mesh,
  ...
  if (ofname == NULL) {
    HECMW_print_msg(HECMW_LOG_WARN, HECMW_ALL_E0101,
                    "output file name is not set");
    return 1;
  }
  ...
  for (i = 0; i < mesh->n_node; i++) {
    fprintf(fp, "%d %.7lE %.7lE %.7lE\n", i + 1, mesh->node[3 * i],
            mesh->node[3 * i + 1], mesh->node[3 * i + 2]);
  }
  ...
  for (i = 0; i < mesh->n_elem; i++) {
    switch (mesh->elem_type[i]) {
      ...
      case HECMW_ETYPE_ROD2:
        ...
        for (j = 0; j < HECMW_MAX_NODE_ROD2; j++) {
          if (conv_index_ucd2hec_rod2[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_rod2[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_TRI1:
        fprintf(fp, "%d %d %s", i + 1, 0,
                HECMW_get_ucd_label(HECMW_ETYPE_TRI1));
        node_index = mesh->elem_node_index[i];
        for (j = 0; j < HECMW_MAX_NODE_TRI1; j++) {
          if (conv_index_ucd2hec_tri1[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_tri1[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_TRI2:
        ...
        for (j = 0; j < HECMW_MAX_NODE_TRI2; j++) {
          if (conv_index_ucd2hec_tri2[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_tri2[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_QUA1:
        fprintf(fp, "%d %d %s", i + 1, 0,
                HECMW_get_ucd_label(HECMW_ETYPE_QUA1));
        node_index = mesh->elem_node_index[i];
        for (j = 0; j < HECMW_MAX_NODE_QUA1; j++) {
          if (conv_index_ucd2hec_qua1[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_qua1[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_QUA2:
        ...
        for (j = 0; j < HECMW_MAX_NODE_QUA2; j++) {
          if (conv_index_ucd2hec_qua2[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_qua2[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_TET1:
        fprintf(fp, "%d %d %s", i + 1, 0,
                HECMW_get_ucd_label(HECMW_ETYPE_TET1));
        node_index = mesh->elem_node_index[i];
        for (j = 0; j < HECMW_MAX_NODE_TET1; j++) {
          if (conv_index_ucd2hec_tet1[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_tet1[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_TET2:
        ...
        for (j = 0; j < HECMW_MAX_NODE_TET2; j++) {
          if (conv_index_ucd2hec_tet2[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_tet2[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_PRI1:
        fprintf(fp, "%d %d %s", i + 1, 0,
                HECMW_get_ucd_label(HECMW_ETYPE_PRI1));
        node_index = mesh->elem_node_index[i];
        for (j = 0; j < HECMW_MAX_NODE_PRI1; j++) {
          if (conv_index_ucd2hec_pri1[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_pri1[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_PRI2:
        ...
        for (j = 0; j < HECMW_MAX_NODE_PRI2; j++) {
          if (conv_index_ucd2hec_pri2[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_pri2[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_HEX1:
        fprintf(fp, "%d %d %s", i + 1, 0,
                HECMW_get_ucd_label(HECMW_ETYPE_HEX1));
        node_index = mesh->elem_node_index[i];
        for (j = 0; j < HECMW_MAX_NODE_HEX1; j++) {
          if (conv_index_ucd2hec_hex1[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_hex1[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_HEX2:
        ...
        for (j = 0; j < HECMW_MAX_NODE_HEX2; j++) {
          if (conv_index_ucd2hec_hex2[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_hex2[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_PYR1:
        fprintf(fp, "%d %d %s", i + 1, 0,
                HECMW_get_ucd_label(HECMW_ETYPE_PYR1));
        node_index = mesh->elem_node_index[i];
        for (j = 0; j < HECMW_MAX_NODE_PYR1; j++) {
          if (conv_index_ucd2hec_pyr1[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_pyr1[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_PYR2:
        ...
        for (j = 0; j < HECMW_MAX_NODE_PYR2; j++) {
          if (conv_index_ucd2hec_pyr2[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_pyr2[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_MST1:
        fprintf(fp, "%d %d %s", i + 1, 0,
                HECMW_get_ucd_label(HECMW_ETYPE_MST1));
        node_index = mesh->elem_node_index[i];
        for (j = 0; j < HECMW_MAX_NODE_MST1; j++) {
          if (conv_index_ucd2hec_mst1[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_mst1[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_MST2:
        ...
        for (j = 0; j < HECMW_MAX_NODE_MST2; j++) {
          if (conv_index_ucd2hec_mst2[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_mst2[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_MSQ1:
        fprintf(fp, "%d %d %s", i + 1, 0,
                HECMW_get_ucd_label(HECMW_ETYPE_MSQ1));
        node_index = mesh->elem_node_index[i];
        for (j = 0; j < HECMW_MAX_NODE_MSQ1; j++) {
          if (conv_index_ucd2hec_msq1[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_msq1[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_MSQ2:
        ...
        for (j = 0; j < HECMW_MAX_NODE_MSQ2; j++) {
          if (conv_index_ucd2hec_msq2[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_msq2[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_JTB1:
        fprintf(fp, "%d %d %s", i + 1, 0,
                HECMW_get_ucd_label(HECMW_ETYPE_JTB1));
        node_index = mesh->elem_node_index[i];
      ...
      case HECMW_ETYPE_JTT1:
        fprintf(fp, "%d %d %s", i + 1, 0,
                HECMW_get_ucd_label(HECMW_ETYPE_JTT1));
        node_index = mesh->elem_node_index[i];
        for (j = 0; j < HECMW_MAX_NODE_JTT1; j++) {
          if (conv_index_ucd2hec_jtt1[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_jtt1[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_JTT2:
        ...
        for (j = 0; j < HECMW_MAX_NODE_JTT2; j++) {
          if (conv_index_ucd2hec_jtt2[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_jtt2[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_JTQ1:
        fprintf(fp, "%d %d %s", i + 1, 0,
                HECMW_get_ucd_label(HECMW_ETYPE_JTQ1));
        node_index = mesh->elem_node_index[i];
        for (j = 0; j < HECMW_MAX_NODE_JTQ1; j++) {
          if (conv_index_ucd2hec_jtq1[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_jtq1[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_JTQ2:
        ...
        for (j = 0; j < HECMW_MAX_NODE_JTQ2; j++) {
          if (conv_index_ucd2hec_jtq2[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_jtq2[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_SHT1:
        fprintf(fp, "%d %d %s", i + 1, 0,
                HECMW_get_ucd_label(HECMW_ETYPE_SHT1));
        node_index = mesh->elem_node_index[i];
        for (j = 0; j < HECMW_MAX_NODE_SHT1; j++) {
          if (conv_index_ucd2hec_sht1[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_sht1[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_SHT2:
        ...
        for (j = 0; j < HECMW_MAX_NODE_SHT2; j++) {
          if (conv_index_ucd2hec_sht2[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_sht2[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_SHQ1:
        fprintf(fp, "%d %d %s", i + 1, 0,
                HECMW_get_ucd_label(HECMW_ETYPE_SHQ1));
        node_index = mesh->elem_node_index[i];
        for (j = 0; j < HECMW_MAX_NODE_SHQ1; j++) {
          if (conv_index_ucd2hec_shq1[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_shq1[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_SHQ2:
        ...
        for (j = 0; j < HECMW_MAX_NODE_SHQ2; j++) {
          if (conv_index_ucd2hec_shq2[j] >= 0) {
            fprintf(
                fp, " %d",
                mesh->elem_node_item[node_index + conv_index_ucd2hec_shq2[j]]);
          }
        }
        break;
      ...
      case HECMW_ETYPE_LN66:
        fprintf(fp, "%d %d %s", i + 1, 0,
                HECMW_get_ucd_label(HECMW_ETYPE_LN11));
        node_index = mesh->elem_node_index[i];
