...
static struct loglv_ent {
  ...
  char *str;
    ...
    {HECMW_LOG_ERROR, "Error"},
    {HECMW_LOG_WARN, "Warning"},
    {HECMW_LOG_INFO, "Info"},
    {HECMW_LOG_DEBUG, "Debug"},
};
...
static int check_loglv(int loglv) {
  int i;
...
int HECMW_openlog(const char *logfile, int loglv, int options) {
  ...
  if (enable_by_rank && loglv & loglevels) {
    if ((fp = fopen(logfilename, "a")) == NULL) {
      HECMW_set_error(HECMW_UTIL_E9011, "File %s, %s", logfilename,
                      strerror(errno));
      return -1;
    }
  } else {
...
static void output_log(int loglv, const char *fmt, va_list ap, FILE *fp) {
...
#ifndef HECMW_SERIAL
  {
    ...
    if (num_procs > 1) {
      len = strlen(buf);
      HECMW_snprintf(buf + len, sizeof(buf) - len, " PE[%d/%d]", my_rank,
                     num_procs);
    }
  }
#endif
...
#ifdef _OPENMP
  {
    ...
    if (num_threads > 1) {
      len = strlen(buf);
      HECMW_snprintf(buf + len, sizeof(buf) - len, " TH(%d/%d)", thread_num,
                     num_threads);
    }
  }
#endif
    ...
    if (loglv & loglevels & p->lv) {
      if (p->fp == NULL) {
        if ((p->fp = fopen(p->file, "a")) == NULL) {
          HECMW_set_error(HECMW_UTIL_E9011, "File %s, %s", p->file,
                          strerror(errno));
          return -1;
        }
      }
...
void hecmw_setloglv_if(int *loglv) { HECMW_setloglv(*loglv); }

void hecmw_setloglv_if_(int *loglv) { hecmw_setloglv_if(loglv); }

void hecmw_setloglv_if__(int *loglv) { hecmw_setloglv_if(loglv); }

void HECMW_SETLOGLV_IF(int *loglv) { hecmw_setloglv_if(loglv); }
...
void hecmw_openlog_if(char *logfile, int *loglv, int *options, int *id,
                      int *ierror, int len) {
...
void hecmw_openlog_if_(char *logfile, int *loglv, int *options, int *id,
                       int *ierror, int len) {
  hecmw_openlog_if(logfile, loglv, options, id, ierror, len);
}

void hecmw_openlog_if__(char *logfile, int *loglv, int *options, int *id,
                        int *ierror, int len) {
  hecmw_openlog_if(logfile, loglv, options, id, ierror, len);
}
...
void hecmw_closelog_if_(int *id, int *ierror) { hecmw_closelog_if(id, ierror); }

void hecmw_closelog_if__(int *id, int *ierror) {
  hecmw_closelog_if(id, ierror);
}

void HECMW_CLOSELOG_IF(int *id, int *ierror) { hecmw_closelog_if(id, ierror); }
