...
void HECMW_map_int_finalize(struct hecmw_map_int *map) {
  ...
  if (map->free_fnc != NULL) {
    ...
    for (i = 0; i < map->n_val; i++) map->free_fnc(map->vals[i].val);
  }
...
static int map_resize(struct hecmw_map_int *map, size_t new_max_val) {
  ...
  if (new_max_val == 0) {
    ...
    new_vals = (struct hecmw_map_int_value *)HECMW_realloc(
        map->vals, sizeof(struct hecmw_map_int_value) * new_max_val);
    if (new_vals == NULL) return HECMW_ERROR;
    map->vals = new_vals;

    new_pairs = (struct hecmw_map_int_pair *)HECMW_realloc(
        map->pairs, sizeof(struct hecmw_map_int_pair) * new_max_val);
...
int HECMW_map_int_add(struct hecmw_map_int *map, int key, void *value) {
  ...
  if (map->n_val == map->max_val)
    if (map_grow(map) != HECMW_SUCCESS) return HECMW_ERROR;
  ...
  if (map->n_val > 0 && map->sorted) {
    ...
    if (key_prev > key) map->sorted = map->checked = 0;

    if (map->checked && key_prev == key) map->checked = 0;
  }
...
int HECMW_map_int_key2local(const struct hecmw_map_int *map, int key,
  ...
  if (map_search(map, key, local) != HECMW_SUCCESS) return HECMW_ERROR;
...
void *HECMW_map_int_get(const struct hecmw_map_int *map, int key) {
  ...
  if (HECMW_map_int_key2local(map, key, &local) != HECMW_SUCCESS) return NULL;
...
int HECMW_map_int_iter_next(struct hecmw_map_int *map, int *key, void **value) {
  ...
  *key                      = map->vals[map->iter].key;
  if (value != NULL) *value = map->vals[map->iter].val;
...
int HECMW_map_int_mark_init(struct hecmw_map_int *map) {
  ...
  map->mark =
      (struct hecmw_bit_array *)HECMW_malloc(sizeof(struct hecmw_bit_array));
  if (map->mark == NULL) {
    return HECMW_ERROR;
  }
...
static void rebuild_pairs(struct hecmw_map_int *map) {
  ...
  for (i = 0; i < map->n_val; i++) {
    ...
    map->pairs[i].local = i;
    if (i > 0 && map->vals[i].key < map->vals[i - 1].key) sorted = 0;
  }
...
int HECMW_map_int_del_unmarked(struct hecmw_map_int *map) {
  ...
  for (i = 0; i < map->n_val; i++) {
    if (HECMW_bit_array_get(map->mark, i)) { /* marked */
      if (n_del > 0) map->vals[i - n_del] = map->vals[i];
    } else { /* not marked */
      if (map->free_fnc != NULL) map->free_fnc(map->vals[i].val);
      n_del++;
    }
  }
