...
module  hecmw_visualizer
  ...
  public  :: hecmw_visualize_by_addfname
...
contains
  ...
  subroutine  hecmw_visualize_by_addfname( mesh, result_data, step, addfname )
    implicit none
    type(hecmwST_local_mesh),  intent(in) :: mesh
    type(hecmwST_result_data), intent(in) :: result_data
    integer(kind=kint),        intent(in) :: step
    character(len=*),          intent(in) :: addfname
    integer(kind=kint)                    :: ierr
    ...
    call  hecmw_visualize_by_addfname_if( step, addfname, ierr )
    if( ierr /= 0 )  then
      call  hecmw_abort( hecmw_comm_get_comm( ) )
    endif
  ...
  end subroutine  hecmw_visualize_by_addfname
