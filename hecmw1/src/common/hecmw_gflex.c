...
#define YY_INPUT(buf,result,max_size) \
	if ( YY_CURRENT_BUFFER_LVALUE->yy_is_interactive ) \
		...
		{ \
		errno=0; \
		while ( (result = fread(buf, 1, max_size, HECMW_gfin))==0 && ferror(HECMW_gfin)) \
			{ \
			if( errno != EINTR) \
				{ \
				YY_FATAL_ERROR( "input in flex scanner failed" ); \
...
/* Code executed at the beginning of each rule, after HECMW_gftext and HECMW_gfleng
 * have been set up.
 */
...
{ len += HECMW_gfleng; lineno++; return HECMW_GFLEX_NL; }
	YY_BREAK
case 5:
YY_RULE_SETUP
...
{ len += HECMW_gfleng; return HECMW_gftext[0]; }
	YY_BREAK
...
static int yy_get_next_buffer (void)
{
	...
	if ((yy_size_t) ((yy_n_chars) + number_to_move) > YY_CURRENT_BUFFER_LVALUE->yy_buf_size) {
		...
		YY_CURRENT_BUFFER_LVALUE->yy_ch_buf = (char *) HECMW_gfrealloc((void *) YY_CURRENT_BUFFER_LVALUE->yy_ch_buf,new_size  );
...
YY_BUFFER_STATE HECMW_gf_scan_buffer  (char * base, yy_size_t  size )
{
	...
	b = (YY_BUFFER_STATE) HECMW_gfalloc(sizeof( struct yy_buffer_state )  );
	if ( ! b )
		YY_FATAL_ERROR( "out of dynamic memory in HECMW_gf_scan_buffer()" );
...
/** Setup the input buffer state to scan a string. The next call to HECMW_gflex() will
...
/** Setup the input buffer state to scan the given bytes. The next call to HECMW_gflex() will
...
YY_BUFFER_STATE HECMW_gf_scan_bytes  (yyconst char * yybytes, int  _yybytes_len )
{
	...
	if ( ! buf )
		YY_FATAL_ERROR( "out of dynamic memory in HECMW_gf_scan_bytes()" );
	...
	if ( ! b )
		YY_FATAL_ERROR( "bad buffer in HECMW_gf_scan_bytes()" );
...
int HECMW_gfget_debug  (void)
{
        return HECMW_gf_flex_debug;
}

void HECMW_gfset_debug (int  bdebug )
{
        HECMW_gf_flex_debug = bdebug ;
}
