...
/*  integer                                                                   */
/*----------------------------------------------------------------------------*/
static int get_int(int *i, FILE *fp) {
  int rtc;
...
/*  double                                                                    */
/*----------------------------------------------------------------------------*/
static int get_double(double *d, FILE *fp) {
  int rtc;
...
/*  string                                                                    */
/*----------------------------------------------------------------------------*/
static int get_string(char *s, int max, FILE *fp) {
  int c, len;
...
/*  HECMW_Comm                                                                */
/*----------------------------------------------------------------------------*/
static int get_comm(HECMW_Comm *i, FILE *fp) {
  int rtc;
...
/*  array ( int )                                                             */
/*----------------------------------------------------------------------------*/
static int get_int_ary(int *ary, int n, FILE *fp) {
  int rtc, i;
...
/*  array ( double )                                                          */
/*----------------------------------------------------------------------------*/
static int get_double_ary(double *ary, int n, FILE *fp) {
  int rtc, i;
...
/*  array ( string )                                                          */
/*----------------------------------------------------------------------------*/
static int get_string_ary(char **ary, int n, FILE *fp) {
...
/*                                                                            */
/*============================================================================*/
static int is_hecmw_dist_file(FILE *fp) {
...
/*                                                                            */
/*============================================================================*/
static int get_global_info(struct hecmwST_local_mesh *mesh, FILE *fp) {
  int flag_header;
  ...
  if (mesh->hecmw_n_file > 0) {
    if ((mesh->files = (char **)HECMW_calloc(mesh->hecmw_n_file,
                                             sizeof(char *))) == NULL) {
      HECMW_set_error(errno, "");
      return -1;
    }
...
/*                                                                            */
/*============================================================================*/
static int get_node_info(struct hecmwST_local_mesh *mesh, FILE *fp) {
  /* n_node*/
  ...
  if ((mesh->hecmw_flag_parttype == HECMW_FLAG_PARTTYPE_ELEMBASED ||
      ...
      mesh->nn_internal > 0) {
    mesh->node_internal_list =
        (int *)HECMW_malloc(sizeof(double) * mesh->nn_internal);
  ...
  if (mesh->hecmw_flag_initcon && mesh->n_node_gross > 0) {
    ...
    if (mesh->node_init_val_index[mesh->n_node_gross] == 0) {
      ...
      mesh->node_init_val_item = (double *)HECMW_malloc(
          sizeof(double) * mesh->node_init_val_index[mesh->n_node_gross]);
      if (mesh->node_init_val_item == NULL) {
        HECMW_set_error(errno, "");
        return -1;
      }
      if (get_double_ary(mesh->node_init_val_item,
                         mesh->node_init_val_index[mesh->n_node_gross], fp)) {
        return -1;
      }
    }
  }
...
/*                                                                            */
/*============================================================================*/
static int get_elem_info(struct hecmwST_local_mesh *mesh, FILE *fp) {
  /* n_elem */
  ...
  if ((mesh->hecmw_flag_parttype == HECMW_FLAG_PARTTYPE_NODEBASED ||
       mesh->hecmw_flag_parttype == HECMW_FLAG_PARTTYPE_UNKNOWN) &&
      mesh->ne_internal > 0) {
    mesh->elem_internal_list =
        (int *)HECMW_malloc(sizeof(int) * mesh->ne_internal);
  ...
  if (mesh->n_elem_gross > 0) {
    /* elem_ID */
    if ((mesh->elem_ID = (int *)HECMW_malloc(sizeof(int) * mesh->n_elem_gross *
                                             2)) == NULL) {
      HECMW_set_error(errno, "");
      return -1;
    }
    ...
    /* global_elem_ID */
    if ((mesh->global_elem_ID =
             (int *)HECMW_malloc(sizeof(int) * mesh->n_elem_gross)) == NULL) {
      HECMW_set_error(errno, "");
      return -1;
    }
  ...
  if (mesh->n_elem_type > 0) {
    /* elem_type_index */
    if ((mesh->elem_type_index = (int *)HECMW_malloc(
             sizeof(int) * (mesh->n_elem_type + 1))) == NULL) {
      HECMW_set_error(errno, "");
      return -1;
    }
    ...
    /* elem_type_item */
    if ((mesh->elem_type_item =
             (int *)HECMW_malloc(sizeof(int) * mesh->n_elem_type)) == NULL) {
      HECMW_set_error(errno, "");
      return -1;
    }
  ...
  if (mesh->n_elem_gross > 0) {
    ...
    /* elem_node_item */
    if ((mesh->elem_node_item = (int *)HECMW_malloc(
             sizeof(int) * mesh->elem_node_index[mesh->n_elem_gross])) ==
        NULL) {
      HECMW_set_error(errno, "");
      return -1;
    }
    if (get_int_ary(mesh->elem_node_item,
                    mesh->elem_node_index[mesh->n_elem_gross], fp)) {
      return -1;
    }
    ...
    /* elem_mat_ID_index */
    if ((mesh->elem_mat_ID_index = (int *)HECMW_malloc(
             sizeof(int) * (mesh->n_elem_gross + 1))) == NULL) {
      HECMW_set_error(errno, "");
      return -1;
    }
    ...
    /* elem_mat_ID_item */
    if ((mesh->elem_mat_ID_item = (int *)HECMW_malloc(
             sizeof(int) * mesh->elem_mat_ID_index[mesh->n_elem_gross])) ==
        NULL) {
      HECMW_set_error(errno, "");
      return -1;
    }
    if (get_int_ary(mesh->elem_mat_ID_item,
                    mesh->elem_mat_ID_index[mesh->n_elem_gross], fp)) {
      return -1;
    }
  }
...
/*                                                                            */
/*============================================================================*/
static int get_comm_info(struct hecmwST_local_mesh *mesh, FILE *fp) {
  /* zero */
  ...
  /* neighbor_pe */
  if ((mesh->neighbor_pe =
           (int *)HECMW_malloc(sizeof(int) * mesh->n_neighbor_pe)) == NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  /* import_index */
  if ((mesh->import_index = (int *)HECMW_malloc(
           sizeof(int) * (mesh->n_neighbor_pe + 1))) == NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  /* import_item */
  if ((mesh->import_item = (int *)HECMW_malloc(
           sizeof(int) * mesh->import_index[mesh->n_neighbor_pe])) == NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  if (get_int_ary(mesh->import_item, mesh->import_index[mesh->n_neighbor_pe],
                  fp)) {
    return -1;
  }

  /* export_index */
  if ((mesh->export_index = (int *)HECMW_malloc(
           sizeof(int) * (mesh->n_neighbor_pe + 1))) == NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  /* export_item */
  if ((mesh->export_item = (int *)HECMW_malloc(
           sizeof(int) * mesh->export_index[mesh->n_neighbor_pe])) == NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  if (get_int_ary(mesh->export_item, mesh->export_index[mesh->n_neighbor_pe],
                  fp)) {
    return -1;
  }

  /* shared_index */
  if ((mesh->shared_index = (int *)HECMW_malloc(
           sizeof(int) * (mesh->n_neighbor_pe + 1))) == NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  /* shared_item */
  if ((mesh->shared_item = (int *)HECMW_malloc(
           sizeof(int) * mesh->shared_index[mesh->n_neighbor_pe])) == NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  if (get_int_ary(mesh->shared_item, mesh->shared_index[mesh->n_neighbor_pe],
                  fp)) {
    return -1;
  }
...
/*                                                                            */
/*============================================================================*/
static int get_adapt_info(struct hecmwST_local_mesh *mesh, FILE *fp) {
  ...
  if (mesh->n_node_gross > 0) {
    ...
    if ((mesh->when_i_was_refined_node =
             (int *)HECMW_malloc(sizeof(int) * mesh->n_node_gross)) == NULL) {
      HECMW_set_error(errno, "");
      return -1;
    }
  ...
  if ((mesh->when_i_was_refined_elem =
           (int *)HECMW_malloc(sizeof(int) * mesh->n_elem_gross)) == NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  /* adapt_parent */
  if ((mesh->adapt_parent =
           (int *)HECMW_malloc(sizeof(int) * mesh->n_elem_gross * 2)) == NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  if ((mesh->adapt_children_index = (int *)HECMW_malloc(
           sizeof(int) * (mesh->n_elem_gross + 1))) == NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  /* adapt_children_item */
  if ((mesh->adapt_children_item = (int *)HECMW_malloc(
           sizeof(int) * mesh->adapt_children_index[mesh->n_elem_gross] * 2)) ==
      NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  if (get_int_ary(mesh->adapt_children_item,
                  mesh->adapt_children_index[mesh->n_elem_gross] * 2, fp)) {
    return -1;
  }
...
/*                                                                            */
/*============================================================================*/
static int get_section_info(struct hecmwST_section *sect, FILE *fp) {
  /* n_sect */
  ...
  /* sect_type */
  if ((sect->sect_type = (int *)HECMW_malloc(sizeof(int) * sect->n_sect)) ==
      NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  /* sect_opt */
  if ((sect->sect_opt = (int *)HECMW_malloc(sizeof(int) * sect->n_sect)) ==
      NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  /* sect_mat_ID_index */
  if ((sect->sect_mat_ID_index =
           (int *)HECMW_malloc(sizeof(int) * (sect->n_sect + 1))) == NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  if (sect->sect_mat_ID_index[sect->n_sect] > 0) {
    if ((sect->sect_mat_ID_item = (int *)HECMW_malloc(
             sizeof(int) * sect->sect_mat_ID_index[sect->n_sect])) == NULL) {
      HECMW_set_error(errno, "");
      return -1;
    }
    if (get_int_ary(sect->sect_mat_ID_item,
                    sect->sect_mat_ID_index[sect->n_sect], fp)) {
      return -1;
    }
  }

  /* sect_I_index */
  if ((sect->sect_I_index =
           (int *)HECMW_malloc(sizeof(int) * (sect->n_sect + 1))) == NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  if (sect->sect_I_index[sect->n_sect] > 0) {
    if ((sect->sect_I_item = (int *)HECMW_malloc(
             sizeof(int) * sect->sect_I_index[sect->n_sect])) == NULL) {
      HECMW_set_error(errno, "");
      return -1;
    }
  ...
  if (sect->sect_R_index[sect->n_sect] > 0) {
    if ((sect->sect_R_item = (double *)HECMW_malloc(
             sizeof(double) * sect->sect_R_index[sect->n_sect])) == NULL) {
      HECMW_set_error(errno, "");
      return -1;
    }
    if (get_double_ary(sect->sect_R_item, sect->sect_R_index[sect->n_sect],
                       fp)) {
      return -1;
    }
  }
...
/*                                                                            */
/*============================================================================*/
static int get_material_info(struct hecmwST_material *mat, FILE *fp) {
  /* n_mat */
  ...
  /* mat_name */
  if ((mat->mat_name = (char **)HECMW_malloc(sizeof(char *) * mat->n_mat)) ==
      NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  /* mat_item_index */
  if ((mat->mat_item_index =
           (int *)HECMW_malloc(sizeof(int) * (mat->n_mat + 1))) == NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  /* mat_subitem_index */
  if ((mat->mat_subitem_index =
           (int *)HECMW_malloc(sizeof(int) * (mat->n_mat_item + 1))) == NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  /* mat_table_index */
  if ((mat->mat_table_index = (int *)HECMW_malloc(
           sizeof(int) * (mat->n_mat_subitem + 1))) == NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  /* mat_val */
  if ((mat->mat_val =
           (double *)HECMW_malloc(sizeof(double) * mat->n_mat_table)) == NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
...
/*                                                                            */
/*============================================================================*/
static int get_mpc_info(struct hecmwST_mpc *mpc, FILE *fp,
  ...
  /* mpc_index */
  if ((mpc->mpc_index = (int *)HECMW_malloc(sizeof(int) * (mpc->n_mpc + 1))) ==
      NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  /* mpc_item */
  if ((mpc->mpc_item = (int *)HECMW_malloc(
           sizeof(int) * mpc->mpc_index[mpc->n_mpc])) == NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  /* mpc_dof */
  if ((mpc->mpc_dof = (int *)HECMW_malloc(
           sizeof(int) * mpc->mpc_index[mpc->n_mpc])) == NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  /* mpc_val */
  if ((mpc->mpc_val = (double *)HECMW_malloc(
           sizeof(double) * mpc->mpc_index[mpc->n_mpc])) == NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  /* mpc_const */
  if ((mpc->mpc_const = (double *)HECMW_calloc(mpc->n_mpc, sizeof(double))) ==
      NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
...
/*                                                                            */
/*============================================================================*/
static int get_amp_info(struct hecmwST_amplitude *amp, FILE *fp) {
  /* n_amp */
  ...
  /* amp_name */
  if ((amp->amp_name = (char **)HECMW_malloc(sizeof(char *) * amp->n_amp)) ==
      NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  /* amp_type_definition */
  if ((amp->amp_type_definition =
           (int *)HECMW_malloc(sizeof(int) * amp->n_amp)) == NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  /* amp_type_time */
  if ((amp->amp_type_time = (int *)HECMW_malloc(sizeof(int) * amp->n_amp)) ==
      NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  /* amp_type_value */
  if ((amp->amp_type_value = (int *)HECMW_malloc(sizeof(int) * amp->n_amp)) ==
      NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  /* amp_index */
  if ((amp->amp_index = (int *)HECMW_malloc(sizeof(int) * (amp->n_amp + 1))) ==
      NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  /* amp_val */
  if ((amp->amp_val = (double *)HECMW_malloc(
           sizeof(double) * amp->amp_index[amp->n_amp])) == NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  /* amp_table */
  if ((amp->amp_table = (double *)HECMW_malloc(
           sizeof(double) * amp->amp_index[amp->n_amp])) == NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
...
/*                                                                            */
/*============================================================================*/
static int get_node_group_info(struct hecmwST_node_grp *grp, FILE *fp) {
  /* n_grp */
  ...
  /* grp_name */
  if ((grp->grp_name = (char **)HECMW_malloc(sizeof(char *) * grp->n_grp)) ==
      NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  /* grp_index */
  if ((grp->grp_index = (int *)HECMW_malloc(sizeof(int) * (grp->n_grp + 1))) ==
      NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  if (grp->grp_index[grp->n_grp] > 0) {
    if ((grp->grp_item = (int *)HECMW_malloc(
             sizeof(int) * grp->grp_index[grp->n_grp])) == NULL) {
      HECMW_set_error(errno, "");
      return -1;
    }
...
/*                                                                            */
/*============================================================================*/
static int get_elem_group_info(struct hecmwST_elem_grp *grp, FILE *fp) {
  /* n_grp */
  if (get_int(&grp->n_grp, fp)) {
    return -1;
  }
...
/*                                                                            */
/*============================================================================*/
static int get_surf_group_info(struct hecmwST_surf_grp *grp, FILE *fp) {
  /* n_grp */
  if (get_int(&grp->n_grp, fp)) {
    return -1;
  }
  ...
  if (grp->grp_index[grp->n_grp] > 0) {
    if ((grp->grp_item = (int *)HECMW_malloc(
             sizeof(int) * grp->grp_index[grp->n_grp] * 2)) == NULL) {
      HECMW_set_error(errno, "");
      return -1;
    }
...
/*                                                                            */
/*============================================================================*/
static int get_contact_info(struct hecmwST_contact_pair *cpair, FILE *fp,
                            int hecmw_flag_version) {
  if (hecmw_flag_version < 3) {
    cpair->n_pair        = 0;
    cpair->type          = NULL;
  ...
  /* name */
  if ((cpair->name = (char **)HECMW_malloc(sizeof(char *) * cpair->n_pair)) ==
      NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
  ...
  /* type */
  if ((cpair->type = (int *)HECMW_malloc(sizeof(int) * (cpair->n_pair))) ==
      NULL) {
    HECMW_set_error(errno, "");
    return -1;
  }
...
/*                                                                            */
/*============================================================================*/
static int get_refine_info(struct hecmwST_local_mesh *mesh, FILE *fp) {
  if (mesh->hecmw_flag_version < 2) {
    mesh->n_refine     = 0;
...
static void set_comm(struct hecmwST_local_mesh *mesh) {
...
/*  print int                                                                 */
/*----------------------------------------------------------------------------*/
static int print_int(int item, FILE *fp) {
  int rtc;
...
/*  print double                                                              */
/*----------------------------------------------------------------------------*/
static int print_double(double item, FILE *fp) {
  int rtc;
...
/*  print string                                                              */
/*----------------------------------------------------------------------------*/
static int print_string(const char *item, FILE *fp) {
  int rtc;
...
/*  print HECMW_Comm                                                          */
/*----------------------------------------------------------------------------*/
static int print_comm(HECMW_Comm item, FILE *fp) {
  int rtc;
...
/*  print int (array)                                                         */
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
static int print_int_ary(const int *ary, int n, int cols, FILE *fp) {
  int rtc, i;
...
/*  print string (array)                                                      */
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
static int print_string_ary(char **ary, int n, FILE *fp) {
  int rtc, i;
...
/*  print header                                                              */
/*----------------------------------------------------------------------------*/
static int print_header(const struct hecmwST_local_mesh *mesh, FILE *fp) {
  int rtc;
  char header[HECMW_HEADER_LEN + 1];
...
/*----------------------------------------------------------------------------*/
static int print_global_info(const struct hecmwST_local_mesh *mesh, FILE *fp) {
  int flag_header;
...
/*                                                                            */
/*============================================================================*/
static int print_node_info(const struct hecmwST_local_mesh *mesh, FILE *fp) {
  /* n_node */
  ...
  if (mesh->hecmw_flag_parttype == HECMW_FLAG_PARTTYPE_ELEMBASED ||
      mesh->hecmw_flag_parttype == HECMW_FLAG_PARTTYPE_UNKNOWN) {
    if (print_int_ary(mesh->node_internal_list, mesh->nn_internal, COLS_INT_DEF,
                      fp)) {
      return -1;
    }
  }
  ...
  /* global_node_ID */
  if (print_int_ary(mesh->global_node_ID, mesh->n_node_gross, COLS_INT_DEF,
                    fp)) {
    return -1;
  }
  ...
  /* node_dof_index */
  if (print_int_ary(mesh->node_dof_index, mesh->n_dof_grp + 1, COLS_INT_DEF,
                    fp)) {
    return -1;
  }
  ...
  if (mesh->hecmw_flag_initcon) {
    /* node_init_val_index */
    if (print_int_ary(mesh->node_init_val_index, mesh->n_node_gross + 1,
                      COLS_INT_DEF, fp)) {
      return -1;
    }

    /* node_init_val_item */
    if (print_double_ary(mesh->node_init_val_item,
...
/*                                                                            */
/*============================================================================*/
static int print_elem_info(const struct hecmwST_local_mesh *mesh, FILE *fp) {
  /* n_elem */
  ...
  if (mesh->hecmw_flag_parttype == HECMW_FLAG_PARTTYPE_NODEBASED ||
      mesh->hecmw_flag_parttype == HECMW_FLAG_PARTTYPE_UNKNOWN) {
    if (print_int_ary(mesh->elem_internal_list, mesh->ne_internal, COLS_INT_DEF,
                      fp)) {
      return -1;
    }
  }
  ...
  /* global_elem_ID */
  if (print_int_ary(mesh->global_elem_ID, mesh->n_elem_gross, COLS_INT_DEF,
                    fp)) {
    return -1;
  }
  ...
  /* elem_type_index */
  if (print_int_ary(mesh->elem_type_index, mesh->n_elem_type + 1, COLS_INT_DEF,
                    fp)) {
    return -1;
  }

  /* elem_type_item */
  if (print_int_ary(mesh->elem_type_item, mesh->n_elem_type, COLS_INT_DEF,
                    fp)) {
    return -1;
  }
  ...
  /* elem_node_item */
  if (print_int_ary(mesh->elem_node_item,
                    mesh->elem_node_index[mesh->n_elem_gross], COLS_INT_DEF,
                    fp)) {
    return -1;
  }
  ...
  /* elem_mat_ID_index */
  if (print_int_ary(mesh->elem_mat_ID_index, mesh->n_elem_gross + 1,
                    COLS_INT_DEF, fp)) {
    return -1;
  }

  /* elem_mat_ID_item */
  if (print_int_ary(mesh->elem_mat_ID_item,
                    mesh->elem_mat_ID_index[mesh->n_elem_gross], COLS_INT_DEF,
                    fp)) {
    return -1;
  }
...
/*                                                                            */
/*============================================================================*/
static int print_comm_info(const struct hecmwST_local_mesh *mesh, FILE *fp) {
  /* zero */
  ...
  /* import_index */
  if (print_int_ary(mesh->import_index, mesh->n_neighbor_pe + 1, COLS_INT_DEF,
                    fp)) {
    return -1;
  }

  /* import_item */
  if (print_int_ary(mesh->import_item, mesh->import_index[mesh->n_neighbor_pe],
                    COLS_INT_DEF, fp)) {
    return -1;
  }

  /* export_index */
  if (print_int_ary(mesh->export_index, mesh->n_neighbor_pe + 1, COLS_INT_DEF,
                    fp)) {
    return -1;
  }

  /* export_item */
  if (print_int_ary(mesh->export_item, mesh->export_index[mesh->n_neighbor_pe],
                    COLS_INT_DEF, fp)) {
    return -1;
  }

  /* shared_index */
  if (print_int_ary(mesh->shared_index, mesh->n_neighbor_pe + 1, COLS_INT_DEF,
                    fp)) {
    return -1;
  }

  /* shared_item */
  if (print_int_ary(mesh->shared_item, mesh->shared_index[mesh->n_neighbor_pe],
                    COLS_INT_DEF, fp)) {
    return -1;
  }
...
/*                                                                            */
/*============================================================================*/
static int print_adapt_info(const struct hecmwST_local_mesh *mesh, FILE *fp) {
  ...
  /* when_i_was_refined_node */
  if (print_int_ary(mesh->when_i_was_refined_node, mesh->n_node_gross,
                    COLS_INT_DEF, fp)) {
    return -1;
  }

  /* when_i_was_refined_elem */
  if (print_int_ary(mesh->when_i_was_refined_elem, mesh->n_elem_gross,
                    COLS_INT_DEF, fp)) {
    return -1;
  }

  /* adapt_parent_type */
  if (print_int_ary(mesh->adapt_parent_type, mesh->n_elem_gross, COLS_INT_DEF,
                    fp)) {
    return -1;
  }
  ...
  /* adapt_children_index */
  if (print_int_ary(mesh->adapt_children_index, mesh->n_elem_gross + 1,
                    COLS_INT_DEF, fp)) {
    return -1;
  }

  /* adapt_children_item */
  if (print_int_ary(mesh->adapt_children_item,
                    2 * mesh->adapt_children_index[mesh->n_elem_gross],
                    COLS_TWO, fp)) {
    return -1;
  }
...
/*                                                                            */
/*============================================================================*/
static int print_section_info(const struct hecmwST_section *sect, FILE *fp) {
  /* n_sect */
  ...
  /* sect_mat_ID_index */
  if (print_int_ary(sect->sect_mat_ID_index, sect->n_sect + 1, COLS_INT_DEF,
                    fp)) {
    return -1;
  }

  /* sect_mat_ID_item */
  if (print_int_ary(sect->sect_mat_ID_item,
                    sect->sect_mat_ID_index[sect->n_sect], COLS_INT_DEF, fp)) {
    return -1;
  }
  ...
  /* sect_I_item */
  if (print_int_ary(sect->sect_I_item, sect->sect_I_index[sect->n_sect],
                    COLS_INT_DEF, fp)) {
    return -1;
  }
  ...
  /* sect_R_item */
  if (print_double_ary(sect->sect_R_item, sect->sect_R_index[sect->n_sect],
                       COLS_DOUBLE_DEF, fp)) {
    return -1;
  }
...
/*                                                                            */
/*============================================================================*/
static int print_material_info(struct hecmwST_material *mat, FILE *fp) {
  /* n_mat */
  ...
  /* mat_subitem_index */
  if (print_int_ary(mat->mat_subitem_index, mat->n_mat_item + 1, COLS_INT_DEF,
                    fp)) {
    return -1;
  }

  /* mat_table_index */
  if (print_int_ary(mat->mat_table_index, mat->n_mat_subitem + 1, COLS_INT_DEF,
                    fp)) {
    return -1;
  }
...
/*                                                                            */
/*============================================================================*/
static int print_mpc_info(const struct hecmwST_mpc *mpc, FILE *fp) {
  /* n_mpc */
  ...
  /* mpc_item */
  if (print_int_ary(mpc->mpc_item, mpc->mpc_index[mpc->n_mpc], COLS_INT_DEF,
                    fp)) {
    return -1;
  }

  /* mpc_dof */
  if (print_int_ary(mpc->mpc_dof, mpc->mpc_index[mpc->n_mpc], COLS_INT_DEF,
                    fp)) {
    return -1;
  }

  /* mpc_val */
  if (print_double_ary(mpc->mpc_val, mpc->mpc_index[mpc->n_mpc],
                       COLS_DOUBLE_DEF, fp)) {
    return -1;
  }
...
/*                                                                            */
/*============================================================================*/
static int print_amp_info(const struct hecmwST_amplitude *amp, FILE *fp) {
  /* n_amp */
  ...
  /* amp_val */
  if (print_double_ary(amp->amp_val, amp->amp_index[amp->n_amp],
                       COLS_DOUBLE_DEF, fp)) {
    return -1;
  }

  /* amp_table */
  if (print_double_ary(amp->amp_table, amp->amp_index[amp->n_amp],
                       COLS_DOUBLE_DEF, fp)) {
    return -1;
  }
...
/*                                                                            */
/*============================================================================*/
static int print_node_grp_info(const struct hecmwST_node_grp *grp, FILE *fp) {
  /* n_grp */
  ...
  /* grp_item */
  if (print_int_ary(grp->grp_item, grp->grp_index[grp->n_grp], COLS_INT_DEF,
                    fp)) {
    return -1;
  }
...
/*                                                                            */
/*============================================================================*/
static int print_elem_grp_info(const struct hecmwST_elem_grp *grp, FILE *fp) {
  /* n_grp */
  if (print_int(grp->n_grp, fp)) {
    return -1;
  }
...
/*                                                                            */
/*============================================================================*/
static int print_surf_grp_info(const struct hecmwST_surf_grp *grp, FILE *fp) {
  /* n_grp */
  if (print_int(grp->n_grp, fp)) {
    return -1;
  }
  ...
  /* grp_item */
  if (print_int_ary(grp->grp_item, grp->grp_index[grp->n_grp] * 2, COLS_TWO,
                    fp)) {
    return -1;
  }
...
/*                                                                            */
/*============================================================================*/
static int print_refine_info(const struct hecmwST_local_mesh *mesh, FILE *fp) {
  /* number of refinement performed */
  ...
  if (mesh->n_node_gross > mesh->nn_internal) {
    /* node_old2new */
    if (print_int_ary(mesh->node_old2new, mesh->n_node_gross, COLS_INT_DEF,
                      fp)) {
      return -1;
    }

    /* node_new2old */
    if (print_int_ary(mesh->node_new2old, mesh->n_node_gross, COLS_INT_DEF,
                      fp)) {
      return -1;
    }
  }
  ...
  if (mesh->n_elem_gross > mesh->n_elem) {
    /* elem_old2new */
    if (print_int_ary(mesh->elem_old2new, mesh->n_elem_gross, COLS_INT_DEF,
                      fp)) {
      return -1;
    }

    /* elem_new2old */
    if (print_int_ary(mesh->elem_new2old, mesh->n_elem_gross, COLS_INT_DEF,
                      fp)) {
      return -1;
    }
  }
...
/*                                                                            */
/*============================================================================*/
static int print_contact_info(const struct hecmwST_contact_pair *cpair,
                              FILE *fp) {
  /* n_pair */
...
/*                                                                            */
/*============================================================================*/
extern int HECMW_put_dist_mesh(const struct hecmwST_local_mesh *mesh,
                               char *fname) {
  FILE *fp;
