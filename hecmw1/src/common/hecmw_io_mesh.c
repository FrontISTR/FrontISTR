...
static void
do_logging(int loglv, int msgno, const char *fmt, va_list ap)
{
...
static void
set_warn(int msgno, const char *fmt, ...)
{
	va_list ap;
...
print_amp(FILE *fp)
{
	...
	for(p=_amp; p; p=p->next) {
		...
		fprintf(fp, "NAME: %s, DEFINITION: %d, TIME: %d, VALUE: %d\n",
				p->name, p->type_def, p->type_time, p->type_val);
...
print_sect(FILE *fp)
{
	...
	for(p=_sect; p; p=p->next) {
		fprintf(fp, "EGRP: %s, MATERIAL: %s, COMPOSITE: %d, SECOPT: %d\n",
				p->egrp, p->material, p->composite, p->secopt);
	...
	for(p=_mpc; p; p=p->next) {
		...
		for(i=0; i < p->neq; i++) {
			...
			fprintf(fp, "ngrp: %s, nod: %d, DOF: %d, A: %E\n",
					(item->node == -1) ? item->ngrp : "(none)",
					item->node, item->dof, item->a);
		}
	}
...
static int
free_header(struct hecmw_io_header *header)
{
...
static int
free_zero(struct hecmw_io_zero *zero)
{
...
static int
free_node(struct hecmw_map_int *node)
{
...
static int
free_elem(struct hecmw_map_int *elem)
{
...
static int
free_ngrp(struct hecmw_io_ngrp *ngrp)
{
...
static int
free_egrp(struct hecmw_io_egrp *egrp)
{
...
static int
free_sgrp(struct hecmw_io_sgrp *sgrp)
{
...
static int
free_mpc(struct hecmw_io_mpc *mpc)
{
...
static int
free_amp(struct hecmw_io_amplitude *amp)
{
...
static int
free_init(struct hecmw_io_initial *init)
{
...
static int
free_material(struct hecmw_io_material *mat)
{
	int i;
...
static int
free_sect(struct hecmw_io_section *sect)
{
...
static int
free_system(struct hecmw_system_param *system)
{
...
static int
free_contact(struct hecmw_io_contact *contact)
{
...
int
HECMW_io_set_gridfile(char *gridfile)
{
...
struct hecmw_io_node *
HECMW_io_add_node(int id, double x, double y, double z)
{
...
int
HECMW_io_get_nnode_in_ngrp(const char *name)
{
	struct hecmw_io_ngrp *p;
...
struct hecmw_io_id_array *
HECMW_io_get_node_in_ngrp(const char *name)
{
	int n, i, nid;
...
static int
HECMW_io_remove_node_in_ngrp(int node)
{
...
void
HECMW_io_set_header(struct hecmw_io_header *header)
{
	if(header == NULL) {
...
void
HECMW_io_set_system(struct hecmw_system_param *system)
{
	HECMW_free(_system);
...
static int
setup_flags(struct hecmwST_local_mesh *mesh)
{
	HECMW_assert(mesh);
...
static int
setup_gridfile(struct hecmwST_local_mesh *mesh)
{
	HECMW_assert(mesh);
...
static int
setup_files(struct hecmwST_local_mesh *mesh)
{
	HECMW_assert(mesh);
...
static int
setup_header(struct hecmwST_local_mesh *mesh)
{
	char *p;
...
static int
setup_zero(struct hecmwST_local_mesh *mesh)
{
	HECMW_assert(mesh);
...
setup_elem(struct hecmwST_local_mesh *mesh)
{
	...
	for(i=0; HECMW_map_int_iter_next(_elem, &id, (void **) &p); i++) {
		...
		for(j=0 ; j < n; j++) {
			...
			mesh->elem_node_item[start + j]
				= get_gid2lid_node(p->node[j]);
		}
...
setup_mpc(struct hecmwST_local_mesh *mesh)
{
	...
	for(p=_mpc; p; p=p->next) {
		...
		for(j=0; j < p->neq; j++) {
			...
			mpc->mpc_item[start + j]
				= get_gid2lid_node(p->item[j].node);
...
static int
setup_adapt(struct hecmwST_local_mesh *mesh)
{
	HECMW_assert(mesh);
...
static int
setup_refine(struct hecmwST_local_mesh *mesh)
{
	HECMW_assert(mesh);
...
static int
setup_pe(struct hecmwST_local_mesh *mesh)
{
	HECMW_assert(mesh);
...
static int
setup_elem_check_sectid(struct hecmwST_local_mesh *mesh)
{
	int i;
...
static int
setup_sect_set_sectid(struct hecmwST_local_mesh *mesh,
...
static int
setup_mpc_sectid(struct hecmwST_local_mesh *mesh)
{
	int i;
...
static int
setup_mpc_reorder(struct hecmwST_local_mesh *mesh)
{
...
setup_mat(struct hecmwST_local_mesh *mesh)
{
	...
	for(i=0,p=_mat; p; p=p->next,i++) {
		...
		for(j=0; j < p->nitem; j++) {
			...
			mat->mat_subitem_index[idx+1] = mat->mat_subitem_index[idx] + item->nval;
			...
			for(k=0, subitem=item->subitem; subitem; subitem=subitem->next,k++) {
...
static int
setup_elem_mat(struct hecmwST_local_mesh *mesh)
{
...
static int
post_elem_check_node_existence(void)
{
	int i, j, ncon, id;
	struct hecmw_io_element *p;
...
static char *
post_elem_make_matname(int id, char *buf, int bufsize)
{
...
post_ngrp(void)
{
	...
	for(p=_ngrp; p; p=p->next) {
		...
		if (n_dup > 0)
			set_warn(HECMW_IO_W1006, "%d node(s) in %s", n_dup, p->name);
...
static int
post_initial_check_node_exists(void)
{
	int ignore;
...
static int
post_initial_ngrp_to_node(void)
{
...
static int
post_equation_check_node_exists(void)
{
	int i,ignore;
...
static int
post_equation_ngrp_to_node(void)
{
	int i,j,ignore,**node;
	...
	for(p=_mpc; p; p=next) {
		...
		for(i=0; i < p->neq; i++) {
			...
			if(HECMW_io_get_ngrp(item->ngrp) == NULL) {
				set_warn(HECMW_IO_W1020,
					"Node group %s not found", item->ngrp);
				ignore = 1;
				break;
			}
		}
		...
		for(i=1; i < p->neq; i++) {
			...
			if(n != nnode) {
				set_err(HECMW_IO_E1021, "%d node%s in %s, %d node%s in %s",
					nnode, (nnode != 0) ? "s":"", p->item[0].ngrp,
					n, (n != 0) ? "s":"", p->item[i].ngrp);
				return -1;
			}
		}
...
static int
post_section_check_exists(void)
{
	if(_sect == NULL) {
			...
			switch(p->type) {
				case HECMW_SECT_TYPE_SHELL:
					if(!HECMW_is_etype_shell(elem->type)) {
						set_err(HECMW_IO_E1026, "Only shell element allowed in EGRP %s", p->egrp);
						goto error;
					}
					break;
				...
				case HECMW_SECT_TYPE_INTERFACE:
					if(!HECMW_is_etype_interface(elem->type)) {
						set_err(HECMW_IO_E1026, "Only interface element allowed in EGRP %s", p->egrp);
						goto error;
					}
					break;
				default: HECMW_assert(0);
			}
		}
	}
	return 0;
...
static int
post_section_check_mat_exists(void)
{
	int found;
	struct hecmw_io_section *p;
	struct hecmw_io_material *mat;
...
int
HECMW_io_check_mpc_dof(int dof)
{
...
int
HECMW_io_is_reserved_name(const char *name)
{
