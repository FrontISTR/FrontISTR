...
static void free_section(struct hecmwST_section *section) {
...
static void free_material(struct hecmwST_material *material) {
  int i;
...
static void free_mpc(struct hecmwST_mpc *mpc) {
  if (mpc == NULL) return;
...
static void free_amplitude(struct hecmwST_amplitude *amp) {
  int i;
...
static void free_ngrp(struct hecmwST_node_grp *grp) {
  int i;
...
static void free_egrp(struct hecmwST_elem_grp *grp) {
  int i;
...
static void free_sgrp(struct hecmwST_surf_grp *grp) {
  int i;
...
static void free_contact_pair(struct hecmwST_contact_pair *cpair) {
  int i;
...
void HECMW_dist_clean(struct hecmwST_local_mesh *mesh) {
  ...
  HECMW_free(mesh->n_node_refine_hist);
