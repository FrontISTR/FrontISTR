...
! for PERFORMANCE TUNING (BLOCK REORDERED NODE ID)
    ...
    integer(kind=kint), pointer :: nid_new2org(:) ! local node ID reorder table (reordered => original)
...
!        integer(kind=kint) :: nnode, nelem, n_step, i_step, ierr
        integer(kind=kint) :: n_step, i_step, ierr
...
! TUNING BLOCK NODE REORDER
        ...
        call hecmw_result_init_if(nnode, nelem, org_order_global_node_ID, hecMESH%global_elem_ID, n_step, i_step, header, ierr)
    ...
    subroutine hecmw_result_add(node_or_elem, n_dof, label, data)
        ...
        if (node_or_elem .eq. 1 ) then ! node
          ...
          if (tuning_block_reorder_on) then ! reorderd node ID
            ...
            call hecmw_result_add_if(node_or_elem, n_dof, label, org_order_data, ierr)
            if(ierr /= 0) call hecmw_abort(hecmw_comm_get_comm())
        ...
        else if (node_or_elem .eq. 2) then ! elem
          call hecmw_result_add_if(node_or_elem, n_dof, label, data, ierr)
          if(ierr /= 0) call hecmw_abort(hecmw_comm_get_comm())
        else
