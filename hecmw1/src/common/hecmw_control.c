...
static int table_entire_mesh[] = {
	HECMW_CTRL_FTYPE_HECMW_ENTIRE,	/* HEC-MW全体メッシュ */
	HECMW_CTRL_FTYPE_GEOFEM,		/* GeoFEM全体メッシュ */
	HECMW_CTRL_FTYPE_ABAQUS,		/* ABAQUSメッシュ */
	HECMW_CTRL_FTYPE_NASTRAN,		/* NASTRANメッシュ */
	HECMW_CTRL_FTYPE_FEMAP,			/* FEMAPメッシュ */
};
...
static struct mesh_entry *
get_mesh_entry(char *name_ID)
{
	struct mesh_entry *p;
...
get_mesh_grp_entry(char *name_ID)
{
...
static struct restart_entry *
get_restart_entry(char *name_ID)
{
...
static struct restart_entry *
get_restart_entry_by_io(int io)
{
	struct restart_entry *p;
...
static struct mesh_entry *
make_mesh_entry(char *name_ID, int type, int io, int refine, char *filename)
{
	char *p;
...
static int
add_mesh_entry(struct mesh_entry *mesh)
{
	struct mesh_entry *p,*q;
...
static int
add_mesh_group_entry(struct mesh_grp_entry *mesh)
{
	struct mesh_grp_entry *p,*q;
...
static struct result_entry *
make_result_entry(char *name_ID, int io, int fg_text, char *filename)
{
	char *p;
...
static int
add_result_entry(struct result_entry *result)
{
	struct result_entry *p,*q;
...
static struct restart_entry *
make_restart_entry(char *name_ID, int io, char *filename)
{
	char *p;
...
static int
add_restart_entry(struct restart_entry *restart)
{
	struct restart_entry *p,*q;
...
static struct ctrl_entry *
make_ctrl_entry(char *name_ID, char *filename)
{
	char *p;
...
static int
add_ctrl_entry(struct ctrl_entry *ctrl)
{
	struct ctrl_entry *p,*q;
...
do_logging(int loglv, int msgno, int add_location, const char *fmt, va_list ap)
{
	...
	if(add_location) {
		char *s = "";
		if(strlen(msg) > 0) s = ": ";
		HECMW_snprintf(line, sizeof(line), "%s:%d%s", ctrl_filename, HECMW_ctrllex_get_lineno(), s);
	}
...
static int
read_mesh_head_param_name(char *name)
{
	int token;
	char *p;
	...
	if(token != HECMW_CTRLLEX_NAME) {
		set_err_token(token, HECMW_UTIL_E0010, "NAME must begin with a letter or '_'");
		return -1;
	}
...
static int
read_mesh_head_param_type(int *type)
{
	int token;
...
static int
read_mesh_head_param_io(int *io)
{
	int token;
...
static int
read_mesh_head_param_refine(int *refine)
{
	int token;
...
static int
read_mesh_data(char *name, int type, int io, int refine)
{
	int token;
	char *p;
	struct mesh_entry *mesh;
...
static int
read_meshgrp_head_param_name(char *name)
{
	int token;
	char *p;
	...
	token = HECMW_ctrllex_next_token();
	if(token != HECMW_CTRLLEX_NAME) {
		set_err_token(token, HECMW_UTIL_E0050, "NAME must begin with a letter or '_'");
		return -1;
	}
	p = HECMW_ctrllex_get_text();
	if(strlen(p) > HECMW_NAME_LEN) {
...
read_meshgrp_data(char *name)
{
	...
	while(1) {
		...
		if(token != HECMW_CTRLLEX_NL && token != ',') {
			set_err_token(token, HECMW_UTIL_E0050, "','  or NL required after name_ID");
			return -1;
		}
	}
...
static int
read_result_param_name(char *name)
{
	int token;
	char *p;
	...
	token = HECMW_ctrllex_next_token();
	if(token != HECMW_CTRLLEX_NAME) {
		set_err_token(token, HECMW_UTIL_E0020, "NAME must begin with a letter or '_'");
		return -1;
	}
	p = HECMW_ctrllex_get_text();
	if(strlen(p) > HECMW_NAME_LEN) {
...
static int
read_result_param_type(int *fg_text)
{
	int token;
...
static int
read_result_data(char *name, int io, int fg_text)
{
	int token;
	char *p;
...
static int
read_restart_param_name(char *name)
{
	int token;
	char *p;
	...
	token = HECMW_ctrllex_next_token();
	if(token != HECMW_CTRLLEX_NAME) {
		set_err_token(token, HECMW_UTIL_E0040, "NAME must begin with a letter or '_'");
		return -1;
	}
	p = HECMW_ctrllex_get_text();
	if(strlen(p) > HECMW_NAME_LEN) {
...
static int
read_restart_data(char *name, int io)
{
	int token;
	char *p;
...
static int
read_control_head_param_name(char *name)
{
	int token;
	char *p;
	...
	token = HECMW_ctrllex_next_token();
	if(token != HECMW_CTRLLEX_NAME) {
		set_err_token(token, HECMW_UTIL_E0030, "NAME must begin with a letter or '_'");
		return -1;
	}
	p = HECMW_ctrllex_get_text();
	if(strlen(p) > HECMW_NAME_LEN) {
...
int
HECMW_ctrl_init_ex(const char *ctrlfile)
{
	FILE *fp;
	...
	if((fp = fopen(ctrl_filename, "r")) == NULL) {
		HECMW_set_error(HECMW_UTIL_E0001, "File: %s, %s", ctrl_filename, strerror(errno));
		return -1;
	}
	...
	if(fclose(fp)) {
		HECMW_set_error(HECMW_UTIL_E0002, "File: %s, %s", ctrl_filename, strerror(errno));
		return -1;
	}
...
int
HECMW_ctrl_init(void)
{
	return HECMW_ctrl_init_ex(HECMW_CTRL_FILE);
}



int
HECMW_ctrl_finalize(void)
{
...
void
HECMW_ctrl_free_meshfiles(struct hecmw_ctrl_meshfiles *meshfiles)
{
	int i;
...
get_result_file(char *name_ID, int nstep, int istep, int n_rank, int i_rank, int* fg_text, int flag_rank_none)
{
	...
	if(result == NULL) {
		HECMW_set_error(HECMW_UTIL_E0024, "NAME: %s", name_ID ? name_ID : "Not specified");
		return NULL;
	}
...
HECMW_ctrl_get_restart_file(char *name_ID)
{
	...
	if(restart == NULL) {
		HECMW_set_error(HECMW_UTIL_E0044, "NAME: %s", name_ID ? name_ID : "Not specified");
		return NULL;
	}
...
HECMW_ctrl_get_control_file(char *name_ID)
{
	...
	if(ctrl == NULL) {
		HECMW_set_error(HECMW_UTIL_E0033, "NAME: %s", name_ID ? name_ID : "Not specified");
		return NULL;
	}
...
int
HECMW_ctrl_is_exists_control(char *name_ID)
{
...
hecmw_ctrl_init_ex_if(char *ctrlfile, int *err, int len)
{
	...
	*err = 1;
	...
	if(HECMW_strcpy_f2c_r(ctrlfile, len, c_ctrlfile, sizeof(c_ctrlfile)) == NULL) return;
