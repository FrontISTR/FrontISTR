...
/*----------------------------------------------------------------------------*/
static int
old2new_node( struct hecmwST_local_mesh *local_mesh, int *node_new2old )
{
  double *new, *old;
  int i;
...
/*----------------------------------------------------------------------------*/
static int
old2new_node_ID( struct hecmwST_local_mesh *local_mesh, int *node_new2old )
{
  int *new, *old;
  int i;
...
/*----------------------------------------------------------------------------*/
static int
old2new_global_node_ID( struct hecmwST_local_mesh *local_mesh, int *node_new2old )
{
  int *new, *old;
  int i;
...
/*----------------------------------------------------------------------------*/
static int
old2new_node_init_val( struct hecmwST_local_mesh *local_mesh, int *node_new2old )
{
  ...
  new_item = (double *)HECMW_malloc( sizeof(double)*local_mesh->node_init_val_index[local_mesh->n_node] );
  if( new_item == NULL ) {
    HECMW_set_error(HECMW_COMMON_E_ALLOCATION, "");
    return -1;
  }
...
/*----------------------------------------------------------------------------*/
static int
old2new_elem_node_item( struct hecmwST_local_mesh *local_mesh, int *node_old2new )
{
  int i;

  for( i=0; i<local_mesh->elem_node_index[local_mesh->n_elem]; i++ ) {
    local_mesh->elem_node_item[i] = node_old2new[local_mesh->elem_node_item[i]-1];
  }
...
/*----------------------------------------------------------------------------*/
static int
old2new_mpc_item( struct hecmwST_local_mesh *local_mesh, int *node_old2new )
{
  int i;
  ...
  for( i=0; i<local_mesh->mpc->mpc_index[local_mesh->mpc->n_mpc]; i++ ) {
    local_mesh->mpc->mpc_item[i] = node_old2new[local_mesh->mpc->mpc_item[i]-1];
  }
...
/*----------------------------------------------------------------------------*/
static int
old2new_node_grp_item( struct hecmwST_local_mesh *local_mesh, int *node_old2new )
{
  int i, j;
  ...
  for( i=0; i<local_mesh->node_group->n_grp; i++ ) {
    if( strcmp( local_mesh->node_group->grp_name[i], HECMW_COMMON_EQUATION_BLOCK_NAME ) ) {
      for( j=local_mesh->node_group->grp_index[i]; j<local_mesh->node_group->grp_index[i+1]; j++ ) {
        local_mesh->node_group->grp_item[j] = node_old2new[local_mesh->node_group->grp_item[j]-1];
      }
    }
  }
...
/*============================================================================*/
static int
old2new_node_info( struct hecmwST_local_mesh *local_mesh, int *node_new2old, int *node_old2new )
{
  /* nodal coordinates */
...
/*----------------------------------------------------------------------------*/
static int
old2new_elem_type( struct hecmwST_local_mesh *local_mesh, int *elem_new2old )
{
  int *new, *old;
  int i;
...
/*----------------------------------------------------------------------------*/
static int
old2new_elem_node( struct hecmwST_local_mesh *local_mesh, int *elem_new2old )
{
  int *new_index, *old_index;
  ...
  new_item = HECMW_malloc( sizeof(int)*local_mesh->elem_node_index[local_mesh->n_elem] );
  if( new_item == NULL ) {
    HECMW_set_error(HECMW_COMMON_E_ALLOCATION, "");
    return -1;
  }
...
/*----------------------------------------------------------------------------*/
static int
old2new_elem_ID( struct hecmwST_local_mesh *local_mesh, int *elem_new2old )
{
  int *new, *old;
  int i;
...
/*----------------------------------------------------------------------------*/
static int
old2new_global_elem_ID( struct hecmwST_local_mesh *local_mesh, int *elem_new2old )
{
  int *new, *old;
  int i;
...
/*----------------------------------------------------------------------------*/
static int
old2new_section_ID( struct hecmwST_local_mesh *local_mesh, int *elem_new2old )
{
  int *new, *old;
  int i;
...
/*----------------------------------------------------------------------------*/
static int
old2new_mat_ID( struct hecmwST_local_mesh *local_mesh, int *elem_new2old )
{
  int *new_index, *old_index;
  int *new_item, *old_item;
  int old_id;
  ...
  new_item = (int *)HECMW_malloc( sizeof(int)*local_mesh->elem_mat_ID_index[local_mesh->n_elem] );
  if( new_item == NULL ) {
    HECMW_set_error(HECMW_COMMON_E_ALLOCATION, "");
    return -1;
  }
...
/*----------------------------------------------------------------------------*/
static int
old2new_elem_internal_list( struct hecmwST_local_mesh *local_mesh, int *elem_old2new )
{
  int i;

  for( i=0; i<local_mesh->ne_internal; i++ ) {
    local_mesh->elem_internal_list[i] = elem_old2new[local_mesh->elem_internal_list[i]-1];
  }
...
/*----------------------------------------------------------------------------*/
static int
old2new_elem_grp_item( struct hecmwST_local_mesh *local_mesh, int *elem_old2new )
{
  int i;

  for( i=0; i<local_mesh->elem_group->grp_index[local_mesh->elem_group->n_grp]; i++ ) {
    local_mesh->elem_group->grp_item[i] = elem_old2new[local_mesh->elem_group->grp_item[i]-1];
  }
...
/*----------------------------------------------------------------------------*/
static int
old2new_surf_grp_item( struct hecmwST_local_mesh *local_mesh, int *elem_old2new )
{
  int i;

  for( i=0; i<local_mesh->surf_group->grp_index[local_mesh->surf_group->n_grp]; i++ ) {
    local_mesh->surf_group->grp_item[2*i] = elem_old2new[local_mesh->surf_group->grp_item[2*i]-1];
  }
...
/*============================================================================*/
static int
old2new_elem_info( struct hecmwST_local_mesh *local_mesh, int *elem_new2old, int *elem_old2new )
{
  /* finite element type */
...
/*----------------------------------------------------------------------------*/
static int
count_each_elem_type( struct hecmwST_local_mesh *local_mesh, int *counter )
{
  int etype;
  int i;
...
/*----------------------------------------------------------------------------*/
static int
set_n_elem_type( struct hecmwST_local_mesh *local_mesh, int *counter )
{
  int i;
...
/*----------------------------------------------------------------------------*/
static int
set_elem_type_index( struct hecmwST_local_mesh *local_mesh, int *counter,
                     int *elem_new2old, int *elem_old2new )
{
  ...
  local_mesh->elem_type_item  = (int *)HECMW_malloc( sizeof(int)*local_mesh->n_elem_type );
...
HECMW_reorder_elem_type( struct hecmwST_local_mesh *local_mesh )
{
  ...
  int *elem_new2old, *elem_old2new;  /* conversion table between old and new element id */
...
/*----------------------------------------------------------------------------*/
static int
mask_node_dof_inner( struct hecmwST_local_mesh *local_mesh, char *node_flag,
                     const int is, const int ie, const int n_comp, const int n_dof )
{
  int nidx, node;
  int i, j;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
static int
mask_node_dof( struct hecmwST_local_mesh *local_mesh, char *node_flag )
{
  int n_comp, n_dof;
  int is, ie;
  int i;
  ...
  for( i=0; i<local_mesh->n_elem_type; i++ ) {
    ...
    if(mask_node_dof_inner( local_mesh, node_flag, is, ie, n_comp, n_dof )) return -1;
  }
...
/*----------------------------------------------------------------------------*/
static int
reorder_node_dof( struct hecmwST_local_mesh *local_mesh, char *node_flag,
                  int *node_new2old, int *node_old2new, char *dof_flag, int *n_dof_tot )
{
  int counter=0;
  int i;
...
/*----------------------------------------------------------------------------*/
static int
count_n_dof_grp( struct hecmwST_local_mesh *local_mesh, char *dof_flag )
{
  ...
  HECMW_assert( local_mesh->n_dof_grp > 0 && local_mesh->n_dof_grp <= HECMW_MESH_DOF_TOT );
...
/*----------------------------------------------------------------------------*/
static int
create_node_dof_item( struct hecmwST_local_mesh *local_mesh, char *dof_flag, int *n_dof_tot )
{
  int counter=0;
  ...
  local_mesh->node_dof_item = (int *)HECMW_malloc( sizeof(int)*local_mesh->n_dof_grp );
  ...
  /* three DOF */
  if( EVAL_BIT( *dof_flag, BIT_DOF_THREE ) ) {
    local_mesh->node_dof_index[counter+1] = local_mesh->node_dof_index[counter] + n_dof_tot[HECMW_MESH_DOF_THREE];
  ...
  /* two DOF */
  if( EVAL_BIT( *dof_flag, BIT_DOF_TWO ) ) {
    local_mesh->node_dof_index[counter+1] = local_mesh->node_dof_index[counter] + n_dof_tot[HECMW_MESH_DOF_TWO];
    local_mesh->node_dof_item[counter]    = HECMW_MESH_DOF_TWO;
    counter++;
  }

  HECMW_assert( counter == local_mesh->n_dof_grp );
  HECMW_assert( local_mesh->node_dof_index[local_mesh->n_dof_grp] == local_mesh->n_node );
...
{
  int *node_new2old, *node_old2new;  /* conversion table between old and new node id */
  ...
  if( local_mesh->mpc->n_mpc ) {
    int rtc;
    if((rtc = reorder_node_dof_4mpc( local_mesh, node_flag, node_new2old, node_old2new, &dof_flag, n_dof_tot )) < 0) {
      return -1;
	}
    if( rtc ) {
      if(reorder_node_dof( local_mesh, node_flag, node_new2old, node_old2new, &dof_flag, n_dof_tot )) {
        return -1;
	  }
    }
  } else {
...
/*----------------------------------------------------------------------------*/
static int
set_mpc_block( struct hecmwST_local_mesh *local_mesh,
               int *mpc_node_flag, int *mpc_index_flag )
{
  ...
  for( i=0; i<local_mesh->mpc->n_mpc; i++ ) {
    ...
    /* MPCグループの各節点が所属するグループを mpc_node_flag に格納   */
    /* 各MPCグループの所属するブロックを mpc_index_flag に格納 (暫定) */
    min_group = i;
    for( j=js; j<je; j++ ) {
      node = local_mesh->mpc->mpc_item[j];

      /* 注目している節点が既に他のグループに所属している場合  ：group >= 0 */
      /*                       他のグループに所属していない場合：group <  0 */
      group = mpc_node_flag[node-1];
      mpc_node_flag[node-1] = ( group < 0 ) ? i : group;

      /* i番目のMPCグループの各節点の所属するグループの最小グループを決定 */
      min_group = ( mpc_index_flag[mpc_node_flag[node-1]] < min_group ) ? mpc_index_flag[mpc_node_flag[node-1]] : min_group;
    }

    /* i番目のMPCグループの各節点が所属しているグループは最小グループに統一 */
    for( j=js; j<je; j++ ) {
      node  = local_mesh->mpc->mpc_item[j];
      group = mpc_node_flag[node-1];
...
/*----------------------------------------------------------------------------*/
static int
count_mpc_block( struct hecmwST_local_mesh *local_mesh,
...
/*----------------------------------------------------------------------------*/
static int
count_eqn_block( struct hecmwST_local_mesh *local_mesh,
                 struct equation_block *eqn_block, int *mpc_node_flag )
{
  int i;
...
/*----------------------------------------------------------------------------*/
static int
set_eqn_block_of_node( struct hecmwST_local_mesh *local_mesh,
                       struct equation_block *eqn_block,
                       int *mpc_node_flag, int *mpc_index_flag, int *mpc_group2block )
{
  int counter;
  int i;
...
/*----------------------------------------------------------------------------*/
static int
create_eqn_block_index( struct hecmwST_local_mesh *local_mesh,
                       struct equation_block *eqn_block,
                       int *mpc_node_flag, int *mpc_group2block )
{
  ...
  eqn_block->eqn_block_index = (int *)HECMW_calloc( eqn_block->n_eqn_block+1, sizeof(int) );
  ...
  for( i=0; i<eqn_block->n_eqn_block; i++ ) {
    eqn_block->eqn_block_index[i+1] = eqn_block->eqn_block_index[i] + n_block[i];
  }

  /* free */
  HECMW_free( n_block );

  HECMW_assert( eqn_block->eqn_block_index[eqn_block->n_eqn_block] == local_mesh->n_node );
...
/*----------------------------------------------------------------------------*/
static int
create_old2new_node( struct hecmwST_local_mesh *local_mesh,
                     struct equation_block *eqn_block, int *mpc_node_flag,
                     int *node_old2new, int *node_new2old )
{
  int *n_block;
  int new_id;
  int i;
  ...
  for( i=0; i<local_mesh->n_node; i++ ) {
    new_id               = eqn_block->eqn_block_index[mpc_node_flag[i]] + n_block[mpc_node_flag[i]];
...
/*----------------------------------------------------------------------------*/
static int
reconstruct_node_grp( struct hecmwST_local_mesh *local_mesh,
                      struct equation_block *eqn_block )
{
  struct hecmwST_node_grp *grp=local_mesh->node_group;
  int i;
  ...
  grp->grp_index = (int *)HECMW_realloc( grp->grp_index, sizeof(int)*(grp->n_grp+1) );
  if( grp->grp_index == NULL ) {
    HECMW_set_error(HECMW_COMMON_E_ALLOCATION, "");
    return -1;
  }

  grp->grp_index[grp->n_grp] = grp->grp_index[grp->n_grp-1] + eqn_block->n_eqn_block;

  /* name of node group */
  grp->grp_name = (char **)HECMW_realloc( grp->grp_name, sizeof(char *)*grp->n_grp );
  if( grp->grp_name == NULL ) {
    HECMW_set_error(HECMW_COMMON_E_ALLOCATION, "");
    return -1;
  }
  grp->grp_name[grp->n_grp-1] = (char *)HECMW_malloc( sizeof(char *)*(HECMW_NAME_LEN+1) );
  ...
  grp->grp_item = (int *)HECMW_realloc( grp->grp_item, sizeof(int)*grp->grp_index[grp->n_grp] );
  ...
  for( i=0; i<eqn_block->n_eqn_block; i++ ) {
    grp->grp_item[grp->grp_index[grp->n_grp-1]+i] = eqn_block->eqn_block_index[i+1];
  }
...
HECMW_reorder_node_mpc( struct hecmwST_local_mesh *local_mesh )
{
  ...
  eqn_block = (struct equation_block *)HECMW_malloc( sizeof(struct equation_block) );
  if( eqn_block == NULL ) {
    HECMW_set_error(HECMW_COMMON_E_ALLOCATION, "");
    return -1;
  }
  ...
  if(set_eqn_block_of_node( local_mesh, eqn_block, mpc_node_flag, mpc_index_flag, mpc_group2block )) {
    return -1;
  }

  /* create EQUATION_BLOCK */
  if(create_eqn_block_index( local_mesh, eqn_block, mpc_node_flag, mpc_group2block )) {
    return -1;
  }
  ...
  if(create_old2new_node( local_mesh, eqn_block, mpc_node_flag, node_old2new, node_new2old )) {
    return -1;
  }
