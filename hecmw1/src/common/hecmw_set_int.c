...
int HECMW_set_int_init(struct hecmw_set_int *set) {
  ...
  if (HECMW_varray_int_init(set->vals) != HECMW_SUCCESS) return HECMW_ERROR;
...
int HECMW_set_int_add(struct hecmw_set_int *set, int value) {
  ...
  if (nval > 0 && set->sorted) {
    ...
    if (val_prev > value) set->sorted = set->checked = 0;

    if (set->checked && val_prev == value) set->checked = 0;
  }
