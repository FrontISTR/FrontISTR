...
// 与えられたConNodeの"面"内外判定を行って,内ならば追加.
// ----
...
{
    int iprod;//外積の+,-のカウンター :: 面ベクトル(正規化)と同じ向きなら”+”. 逆向きなら"-"
    int numOfVert;
    ...
    for(ivert=0; ivert< numOfVert; ivert++){
        ...
        if(DotProduct() > 0) iprod++;
        if(DotProduct() < 0) iprod--;
    }
    //外積のカウンターが(頂点数 or -頂点数)と一致ならば,面内と判定 => Slave点
    if(iprod==numOfVert || iprod== -numOfVert) mvSlaveConNode.push_back(pConNode);
}
...
bool CMasterFace::NearCrossP(const vdouble& vL1, const vdouble& vL2, const vdouble& L1P0, CContactNode* pL2P0)
{
    ...
    if(mParam_D==0.0) bCrossP=false;//mParam_Dが0.0ならば平行 => 近接点は存在しない.
...
bool CMasterFace::NearCrossP(const vdouble& vL1, const vdouble& vL2, const vdouble& L1P0, const vdouble& L2P0)
{
    bool bCrossP(true);
    
    r_NearCrossP(L1P0,L2P0);
...
{
    mParam_A= vL1[0]*vL1[0] + vL1[1]*vL1[1] + vL1[2]*vL1[2];
}
double& CMasterFace::b_NearCrossP(const vdouble& vL1, const vdouble& vL2)
{
    mParam_B= vL1[0]*vL2[0] + vL1[1]*vL2[1] + vL1[2]*vL2[2];
}
double& CMasterFace::c_NearCrossP(const vdouble& vL1)
{
    mParam_C= vL1[0]*mvParam_R[0] + vL1[1]*mvParam_R[1] + vL1[2]*mvParam_R[2];
}
double& CMasterFace::d_NearCrossP()
{
    mParam_D= (mParam_A * mParam_E)-(mParam_B * mParam_B);
}
double& CMasterFace::e_NearCrossP(const vdouble& vL2)
{
    mParam_E= vL2[0]*vL2[0] + vL2[1]*vL2[1] + vL2[2]*vL2[2];
}
double& CMasterFace::f_NearCrossP(const vdouble& vL2)
{
    mParam_F= vL2[0]*mvParam_R[0] + vL2[1]*mvParam_R[1] + vL2[2]*mvParam_R[2];
}
...
//vdouble& CMasterFace::Normalized(CContactNode* pConBaseNode, CContactNode* pConNode)
//{
//    //2点から作られるベクトルを正規化
//    mNzVector[0]= pConNode->getX() - pConBaseNode->getX();
//    mNzVector[1]= pConNode->getY() - pConBaseNode->getY();
//    mNzVector[2]= pConNode->getZ() - pConBaseNode->getZ();
//
//    double dLength= sqrt(mNzVector[0]*mNzVector[0] + mNzVector[1]*mNzVector[1] + mNzVector[2]*mNzVector[2]);
//
//    mNzVector[0]/= dLength;
//    mNzVector[1]/= dLength;
//    mNzVector[2]/= dLength;
