    ...
    if(TagCheck(sLine, FileBlockName::StartContactMesh()) ){// スタート タグ
        ...
        while(true){
            ...
            for(icont=0; icont< numOfContactMesh; icont++){
                ...
                iss.clear();
                iss.str(sLine);
                // 接合MeshID::ContactMeshID
                iss >> contactID;
                
                mpFactory->GeneContactMesh(contactID);
                ...
                for(maslave=0; maslave< 2; maslave++){
                    ...
                    for(iface=0; iface< numOfContactFace; iface++){
                        ...
                        switch(maslave){
                            case(0)://マスター面 生成
                            mpFactory->GeneMasterFace(contactID, shapeType, contactFaceID, bmesh, meshID,elemID,elemFaceID,vConNodeID);
                            break;
                            
                            case(1)://スレーブ面 生成
                            mpFactory->GeneSlaveFace(contactID,shapeType,contactFaceID, bmesh, meshID,elemID,elemFaceID, vConNodeID);
                            break;
                        }//switch(maslave)
