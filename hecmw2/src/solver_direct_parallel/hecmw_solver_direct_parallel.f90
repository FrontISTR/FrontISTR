...
real(kind=kreal), intent(inout) :: b(:) ! (ndeg,neqns) right hand side value vector of equation.
...
idbg=idbg+m_pds_procinfo%myid
!!write(idbg,*) 'initial',m_pds_procinfo%myid !DEBUG
...
!call MPI_BCAST(hecMAT%b, hecMAT%NDOF*hecMAT%NP, MPI_REAL8, m_pds_procinfo%imp, MPI_COMM_WORLD, ierr)
...
subroutine sp_direct_parent(a0, b_in)
...
!!write(idbg,*) 'parent neqns_d', neqns_d !DEBUG
!write(idbg,*) 'parent ndm', ndm !DEBUG
!write(idbg,*) 'parent dsln', dsln !DEBUG
!write(idbg,*) 'parent diag', diag !DEBUG
!write(idbg,*) 'parent dm(1)%a%neqns', dm(1)%a%neqns !DEBUG
!write(idbg,*) 'parent dm(2)%a%neqns', dm(2)%a%neqns !DEBUG
...
!write(idbg,*) 'after receiving parent dsln', dsln !DEBUG
!write(idbg,*) 'after receiving parent diag', diag !DEBUG
...
!write(idbg,*) 'after nufct0 parent dsln', dsln !DEBUG
!write(idbg,*) 'after nufct0 parent diag', diag !DEBUG
...
!allocate(b(ndeg,a0%neqns), stat=ierr) !ORG
...
!write(idbg,*) 'parent after receive bd', bd !DEBUG
...
!write(idbg,*) 'parent solve Ax_d=b_d neqns_d', neqns_d !DEBUG
!write(idbg,*) 'parent solve Ax_d=b_d ndeg', ndeg !DEBUG
!write(idbg,*) 'parent solve Ax_d=b_d dsln', dsln !DEBUG
!write(idbg,*) 'parent solve Ax_d=b_d diag', diag !DEBUG
call nusol0_parent(dsln, diag, bd, neqns_d, ndeg)
!write(idbg,*) 'parent after solve Ax_d=b_d bd', bd !DEBUG
...
!write(idbg,*) 'child cm%a%ndeg',cm%a%ndeg !DEBUG
!write(idbg,*) 'child cm%a%neqns',cm%a%neqns !DEBUG
!write(idbg,*) 'child cm%a%nttbr',cm%a%nttbr !DEBUG
...
!write(idbg,*) 'child a%ndeg ',cm%a%ndeg !DEBUG
!write(idbg,*) 'child a%nttbr ',cm%a%nttbr !DEBUG
!write(idbg,*) 'child a%irow ',cm%a%irow !DEBUG
!write(idbg,*) 'child a%jcol ',cm%a%jcol !DEBUG
!write(idbg,*) 'child a%val ',cm%a%val !DEBUG
!write(idbg,*) 'child c%ndeg ',cm%c%ndeg !DEBUG
!write(idbg,*) 'child c%nttbr ',cm%c%nttbr !DEBUG
!write(idbg,*) 'child c%irow ',cm%c%irow !DEBUG
!write(idbg,*) 'child c%jcol ',cm%c%jcol !DEBUG
!write(idbg,*) 'child c%val ',cm%c%val !DEBUG
...
call lduDecomposeC(xlnzr_a,colno_a,invp_a,iperm_a, ndeg, nttbr_c, irow_c, jcol_c, cm%c%ncols, cm%c%nrows, xlnzr_c, colno_c, lncol_c)
...
!
  ...
  call elapout('sp_direct_child: entering nufct1_child') !elap DEBUG
...
!  call elapout('nufct1_child: before phase I') !elap DEBUG
!
! phase I
! LDU decompose of A (1..nstop-1)
!
      diag(1,1)=1.0d0/diag(1,1)
!  call elapout('nufct1_child:  phase I 1') !elap DEBUG
      l=parent(1)
!  call elapout('nufct1_child:  phase I 2') !elap DEBUG
      nch(l)=nch(l)-1
!  call elapout('nufct1_child:  phase I 3') !elap DEBUG
      nch(1)=-1
!  call elapout('nufct1_child:  phase I 4') !elap DEBUG
      do 100 ic=2,nstop-1
         call sum(ic,xlnzr,colno,zln(1,:),diag(1,:),nch,parent,neqns)
  100 continue

!  call elapout('nufct1_child: before phase II') !elap DEBUG
!
! phase II
...
!  call elapout('nufct1_child: before phase III') !elap DEBUG
!
! phase III
! Update D region.
!
...
! send D region to parent
      imp = m_pds_procinfo%imp
!  call elapout('nufct1_child: before MPI_SEND') !elap DEBUG
...
if (ndeg .eq. 1) then
!write(idbg,*) 'nufct0_parent sum3' !DEBUG
!write(idbg,*) 'nufct0_parent before sum3 neqns' , neqns !DEBUG
!write(idbg,*) 'nufct0_parent before sum3 dsln' , dsln !DEBUG
!write(idbg,*) 'nufct0_parent before sum3 diag' , diag !DEBUG
  call sum3(neqns, dsln(1,:), diag(1,:))
!write(idbg,*) 'nufct0_parent after sum3 neqns' , neqns !DEBUG
!write(idbg,*) 'nufct0_parent after sum3 dsln' , dsln !DEBUG
!write(idbg,*) 'nufct0_parent after sum3 diag' , diag !DEBUG
...
!  call elapout('sum:  entered') !elap DEBUG
...
!           zz=zln(k)-ddoti(jj,zln(j1),colno(j1),temp)
            ...
            t=t+zz*zln(k)
  200    continue
!         call elapout('sum:  loop1 end') !elap DEBUG
         piv=diag(ic)-t
         if(dabs(piv).gt.rmin) then
            diag(ic)=1.0d0/piv
         endif
!         call elapout('sum:  check point1') !elap DEBUG
    1 continue
...
!            goto 1 !DBG
!         endi !DBG
!         call elapout('sum:  check point2') !elap DEBUG
         return
         end subroutine sum
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine hecmw_solve_direct_parallel_interface(neqns, nttbr, nnzdiag, pointers, indices, values, b)
integer :: neqns, nttbr, nnzdiag
integer(kind=kint), pointer :: pointers(:)
integer(kind=kint), pointer :: indices(:)
real(kind=kreal), pointer :: values(:)
...
allocate(a0%irow(nttbr))
allocate(a0%jcol(nttbr))
allocate(a0%val(ndeg,nttbr))
call hecmw_solve_direct_parallel(a0,b)
return
end subroutine hecmw_solve_direct_parallel_interface
