 ...
 * George
 *
 * $Id: ometis.c,v 1.1 2008/02/25 05:55:53 kitayama Exp $
 *
 */
...
/********* my metis change start *************
   remove iteractive bisection
************************/
...
void MlevelNodeBisection(CtrlType *ctrl, GraphType *graph, int *tpwgts, float ubfactor)
{
  ...
  Refine2WayNode(ctrl, graph, cgraph, ubfactor);
 /***my metis change start**************************
  Add for extract separator 
 */
  count[0]=count[1]=count[2]=0;
  for(i=0;i<graph->nvtxs;i++)
	  count[graph->where[i]]++;
  separator=(Separator_result *)calloc(1, sizeof(Separator_result));
  separator->num_of_lgraph=count[0];
  separator->num_of_rgraph=count[1];
  separator->num_of_separator=count[2];
  separator->lgraph=(int *)calloc(count[0], sizeof(int));
  separator->rgraph=(int *)calloc(count[1], sizeof(int));
  separator->mseparator=(int *)calloc(count[2], sizeof(int));
  count[0]=count[1]=count[2]=0;
  for(i=0;i<graph->nvtxs;i++) {
	  if(graph->where[i]==0) {
           separator->lgraph[count[0]]=i;
           count[0]++;
          }
          else if(graph->where[i]==1) {
            separator->rgraph[count[1]]=i;
            count[1]++;
          }
          else if(graph->where[i]==2) {
            separator->mseparator[count[2]]=i;
            count[2]++;
          }
