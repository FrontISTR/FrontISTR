...
public:
    ...
    CCommElement* getCommElement(const uiint& com_index){ return mvCommElement[com_index];}
    CElement* getElement(const uiint& com_index){ return mvCommElement[com_index]->getElement();}
    ...
    CElement* getDElement(const uiint& dcom_index){ return mvDElement[dcom_index];}
    ...
    CNode* getNode(const uiint& com_index){ return mvNode[com_index];} 
    void setSendNode(CNode* pNode, const uiint& comNodeID){ mvSendCommNodeID.push_back(comNodeID); mvSendNode.push_back(pNode);}    
    CNode* getSendNodeIX(const uiint& index){ return mvSendNode[index];}
    uiint getNumOfSendNode(){ return mvSendNode.size();}
    uiint& getSendCommNodeID(const uiint& index){ return mvSendCommNodeID[index];}
    void setRecvNode(CNode* pNode, const uiint& comNodeID){ mvRecvCommNodeID.push_back(comNodeID); mvRecvNode.push_back(pNode);}    
    CNode* getRecvNodeIX(const uiint& index){ return mvRecvNode[index];}
    ...
    void setNodeRank(const uiint& commNodeID, const uiint& rank){ mvNodeRank[commNodeID]= rank;}
