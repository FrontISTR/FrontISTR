    ...
    switch(mShapeType){
        ...
        case(ElementType::Beam):case(ElementType::Beam2):
            ...
            for(iprog=0; iprog< 2; iprog++){
                ...
                pFace->setRank(mRank);
                pFace->setLevel(mLevel+1);
                if(mnOrder==ElementOrder::First){  pFace->resizeNode(2); pFace->setShapeType(ElementType::Beam); }
                if(mnOrder==ElementOrder::Second){ pFace->resizeNode(3); pFace->setShapeType(ElementType::Beam2);}
                pFace->setID(faceID);
                faceID++;            
                if(mbSelfDom){
                    pFace->markingSelf();
