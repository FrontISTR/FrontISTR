...
void mw_gene_linear_algebra_(iint* num_of_algebra, iint dof[])
{
    ...
    uiint* vDOF= new uiint[nNumOfAlgebra];
...
int mw_mpi_int_(){ return (int)MPI_INT; }        
int mw_mpi_double_(){ return (int)MPI_DOUBLE; }  
int mw_mpi_comm_(){ return (int)MPI_COMM_WORLD; }
int mw_mpi_sum_(){ return (int)MPI_SUM;}
int mw_mpi_max_(){ return (int)MPI_MAX;}
int mw_mpi_min_(){ return (int)MPI_MIN;}
int mw_get_rank_()
{
    return pMW->GetRank();
}
...
void mw_allreduce_r_(double val[], int* val_size, int* op)
{
    ...
    pMW->AllReduce(val, rval, *val_size, (int)MPI_DOUBLE, *op, (int)MPI_COMM_WORLD);
	delete [] rval;
}
void mw_allreduce_i_(iint val[], int* val_size, int* op)
{
    ...
    pMW->AllReduce(val, rval, *val_size, (int)MPI_INT, *op, (int)MPI_COMM_WORLD);
	delete [] rval;
}
int mw_barrier_()
{
    return pMW->Barrier((int)MPI_COMM_WORLD);
}
int mw_abort_(iint* error)
{
    return pMW->Abort((int)MPI_COMM_WORLD, *error);
}
