...
|
|   ./src/FileReaderBoundaryVolumeMesh.cpp
|
...
string CFileReaderBoundaryVolumeMesh::Name()
{
    return  "FileReaderBoundaryVolumeMesh";
}
    ...
    if( TagCheck(sLine, FileBlockName::StartBoundaryVolumeMesh()) ) {
        while(!ifs.eof()) {
            ...
            for(ibound=0; ibound < numOfBoundary; ibound++) {
                ...
                mpFactory->GeneBoundaryVolumeMesh(mgLevel, mesh_id, bnd_id, bnd_type, s_bnd_name, numOfDOF, vDOF, mStrForm);
            };
        };
        return true;
    } else {
...
bool CFileReaderBoundaryVolumeMesh::Read_bin(ifstream& ifs)
{
    ...
    while(!ifs.eof()) {
        ...
        for(ibound=0; ibound < nNumOfBoundary; ibound++) {
            ...
            //TODO:数式 読み込み
            //
            mpFactory->GeneBoundaryVolumeMesh(mgLevel, mesh_id, bnd_id, bnd_type, s_bnd_name, nNumOfDOF, vDOF, mStrForm);
        };
    };
    return true;
}
