...
bool CFileReaderBoundaryVolume::Read(ifstream& ifs, string& sLine)
{
    ...
    if(TagCheck(sLine, FileBlockName::StartBoundaryVolume()) ){
        ...
        while(!ifs.eof()){
            ...
            // Vol形状 BVolumeID ElementID Entity番号
            ...
            // Dirichletの場合は、dof val はダミー値 ( 関数を共有している )
...
bool CFileReaderBoundaryVolume::Read_bin(ifstream& ifs)
{
    ...
    ifs.read((char*)&nNumOfBVol, sizeof(uiint));  if(bOrder) pBinCheck->ByteOrderSwap(nNumOfBVol);
    ...
    while(!ifs.eof()){
        ...
        ifs.read((char*)&bvol_id, sizeof(uiint));  if(bOrder) pBinCheck->ByteOrderSwap(bvol_id);
        ...
        switch(shape_type){
            case(pmw::ElementType::Hexa):
                vBNodeID.resize(pmw::NumberOfNode::Hexa());
                for(ibnode=0; ibnode < pmw::NumberOfNode::Hexa(); ibnode++){
                    ifs.read((char*)&vBNodeID[ibnode], sizeof(uiint));  if(bOrder) pBinCheck->ByteOrderSwap(vBNodeID[ibnode]);
                };
                break;
            case(pmw::ElementType::Hexa2):
                vBNodeID.resize(pmw::NumberOfNode::Hexa2());
