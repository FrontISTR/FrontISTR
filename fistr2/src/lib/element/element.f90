...
module elementInfo
  ...
  !-------------------------------------------
    ...
    integer, parameter :: fe_hex8nic  = 363
 ...
 ! ---------------------------------------------
  ...
  integer(kind=kind(2)) function getSpaceDimension( etype )
      ...
      case (fe_line2n, fe_line3n)
        getSpaceDimension = 1
      case (fe_tri3n, fe_tri6n, fe_quad4n, fe_quad8n)
        getSpaceDimension = 2
      case default
        getSpaceDimension = 3
      end select
  ...
  integer function NumOfQuadPoints( fetype )
      ...
      case ( fe_quad8n )
        NumOfQuadPoints = 9
      case ( fe_hex8n, fe_hex8nic, fe_mitc4_shell )
        NumOfQuadPoints = 8
  ...
  subroutine getShapeDeriv( fetype, localcoord, shapederiv )
      ...
      case (fe_tet4n)
        ! error check
        call ShapeDeriv_tet4n(localcoord,shapederiv(1:4,1:3))
      case (fe_tet10n)
        ! error check
...
! INVERSION OF JACOBIAN
 ...
 !  normal = normal/dsqrt(dot_product(normal, normal))
  ...
  !> Calculate cuuvature tensor at a point along 3d surface
