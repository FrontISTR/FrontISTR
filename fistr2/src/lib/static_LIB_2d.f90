...
!
        ...
        enddo
!
        dstrain(1) = gdispderiv(1,1) + 0.5d0*dot_product( gdispderiv(:,1), gdispderiv(:,1) )
        dstrain(2) = gdispderiv(2,2) + 0.5d0*dot_product( gdispderiv(:,2), gdispderiv(:,2) )
        dstrain(3) = gdispderiv(1,2) + gdispderiv(2,1)                                    &
                     + gdispderiv(1,1)*gdispderiv(1,2) + gdispderiv(2,1)*gdispderiv(2,2)
!
        gausses(LX)%strain(1:4) = dstrain(1:4)
        gausses(LX)%stress(1:4) = matmul( D(1:4, 1:4), dstrain(1:4) )
!
      else if( flag==UPDATELAG ) then
        write(*,'(a)') '  program stop: do not make the function updated strain/stress  for U.L.'
        stop
      else
!
      endif
