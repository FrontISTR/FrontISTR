...
check_result () {
  res_name=$1
  if ! ls $ref_dir/${res_name}.* > /dev/null 2>&1; then
    return 0
  fi
  ...
  find . -name "${res_name}.*" \
    ...
    | sort \
    | uniq \
    | xargs -I{} $rmerge -n $mpi_num_process -s {} -e {} ${res_name} >/dev/null 2>&1

  #
  # Compare result
  #
  for t in $(find $ref_dir -name "${res_name}.*" | awk -F. '{print $NF}' | sort | uniq); do
    echo "  $res_name.$t:"
    TAGT=$PWD/$res_name.$t
    REF=$ref_dir/$res_name.0.$t
...
check_all_results () {
  ...
  #
  # rmerge merges whatever is registered as fstrRES in hecmw_ctrl.dat,
  # so point it at the time history result while merging it
  #
  if [ -e $ref_dir/$cnt_eig ]; then
    write_hecmw_ctrl $cnt $dres
    check_result $dres
