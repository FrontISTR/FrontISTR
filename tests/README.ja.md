...
状態をJSONに記録します。各リビジョンはそれぞれが持つCTest一式を実行し、
両方に同名で存在するテストだけの所要時間を比較します。追加・削除された
テストは記録しますが、性能比較には含めません。CTestの標準ログを利用する
ため、プロジェクトのCMake最低バージョンを引き上げません。
...
GitLab CIではUbuntu 24.04の既存ビルド成果物を現行版として再利用し、比較元の
4ビルドを並行して作成します。その後、利用可能な9通りのビルド・実行モードを
別ジョブで測定します。各ジョブ内の比較元と現行版は同じRunner上で直列実行
されます。
　
テストの追加方法
-----------------
