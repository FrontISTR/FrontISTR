...
!======================================================================!
!======================================================================!
!>  \brief   This module provide functions for viscoplastic calculation
!!
!>  \author     Xi YUAN (AdavanceSoft)
!>  \date       2010/10/28
!>  \version    0.00
!======================================================================!
module m_ViscoPlastic
  use mMaterial
  use m_ElasticLinear
  ...
  contains
   ...
   !> This subroutine calculates ViscoPlastic constitutive relation
   SUBROUTINE calViscoPlasticMatrix( matl, sectType, stress, istat, extval, ctime, dt, D, temperature )
     ...
     REAL(KIND=kreal) :: AA,mm,nn,C1,C2,C3, back(6), outa(3), ina(1)
     REAL(KIND=kreal) :: J1,J2,J3, fai, sita, harden, khard, da(6), devia(6)
     REAL(KIND=kreal) :: xxa, xxn, a1

	 ytype = getYieldFunction( matl%mtype )
	 if( ytype==3 ) THEN
       call uViscoPlasticMatrix( matl, stress, istat, extval, D  )
       return
     endif
	 
     IF( sectType /=D3 ) stop "ViscoPlastic calculation support only Solid element currently"
   ...
   subroutine BackwardEuler( matl, stress, plstrain, istat, fstat, temp )
      ...
      if( ierr ) then
         stop " fail to fetch young's modulus in ViscoPlastic calculation"
      else
        youngs = ee(1)
        poisson = ee(2)
      endif
