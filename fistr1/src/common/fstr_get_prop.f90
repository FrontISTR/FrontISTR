...
!C***
!C
	SUBROUTINE fstr_get_prop(hecMESH,isect,ee,pp,rho,alpha,thick,alpha_over_mu,n_totallayer_ls,shell_matltype,shell_variables,  &
...
!<<********************   shell analysis   ********************
			if (section_type == 2)then
				CALL fstr_get_prop_shell(hecMESH,n_subitem,ee,pp,rho,alpha,thick,alpha_over_mu,n_totallayer_ls,shell_matltype,shell_variables)
			else
...
!-----Modifed by N.Morita GSFS, Univ. of Tokyo ------------------------------------------------------
	Subroutine FSTR_GET_PROP_SHELL(hecMESH,n_subitem,ee,pp,rho,alpha,thick,alpha_over_mu,n_totallayer_ls,shell_matltype,shell_variables)
		use m_fstr
		IMPLICIT REAL(kind=kreal) (A-H,O-Z)
		real(kind=kreal) :: shell_variables(200)
		integer(kind=kint) :: shell_matltype, n_ls, n_mod, n_totallayer_ls, section_type
