...
contains
  ...
  subroutine getContactStiffness_Slag_ss(ctState,tSurf,iter,tPenalty,fcoeff,lagrange,stiffness,weight,phi,shapefunc_s)
    ...
    real(kind=kreal)    :: q((l_max_surface_node+1)*3)  !< q = Tm^T * tangent (for slip)
    real(kind=kreal)    :: dot_tn  !< tangent · normal (for slip)
    real(kind=kreal)    :: weight,phi,shapefunc_s
    ...
    ! Compute Tm and Tt matrices using standard shape functions
    call computeTm_Tt_Dmortar(ctState, tSurf, fcoeff, Tm, Tt, weight, phi, shapefunc_s)
  ...
  end subroutine getContactStiffness_Slag_ss
  ...
  subroutine getContactNodalForce_Slag_ss(ctState,tSurf,ndCoord,ndDu,tPenalty,fcoeff,lagrange,ctNForce,ctTForce,&
    ...
    real(kind=kreal)       :: ctTForce(:)                     !< contact force vector
    logical            :: cflag  !< is necessary to update tangentForce_final
    real(kind=kreal) :: weight,phi,shapefunc_s !< kcsNONE or kcsNAGATA
  ...
  end subroutine getContactNodalForce_Slag_ss
