...
contains
  ...
  subroutine process_concentrated_loads(cstep, ctime, hecMESH, fstrSOLID)
    ...
    do ig0 = 1, fstrSOLID%CLOAD_ngrp_tot
      ...
      ! Handle torque loads
      if (fstrSOLID%CLOAD_ngrp_rotID(ig0) > 0) then
      ...
      ! Apply regular concentrated loads
      do ik = iS0, iE0
    ...
    do rid = 1, n_rot
      ...
      do ik = iS0, iE0
        ...
        call cross_product(normal(1:ndof), cdiff(1:ndof), vect(1:ndof))
  ...
  subroutine process_distributed_loads(cstep, ctime, hecMESH, fstrSOLID)
    ...
    do ig0 = 1, fstrSOLID%DLOAD_ngrp_tot
      ...
      ! Determine if surface or element group
      fg_surf = (ltype == 100)
      if (fg_surf) then
      ...
      do ik = iS0, iE0
        ...
        if (fstrSOLID%DLOAD_follow == 0) then
          call get_original_coordinates(hecMESH, is, nn, ndof, nodLocal, xx, yy, zz, iwk)
        else
          call get_deformed_coordinates(hecMESH, fstrSOLID, is, nn, ndof, nodLocal, xx, yy, zz, iwk)
        endif
        ...
        ! Calculate load vector based on element type
        call calculate_element_load_vector(ic_type, nn, xx, yy, zz, rho, thick, ltype, &
                                          params, vect, nsize, iset, isect, hecMESH, &
                                          fstrSOLID, icel, ndof)
        
        ! Add vector to global load
        do j = 1, nsize
  ...
  !======================================================================!
  !> Calculate element load vector based on element type
  !======================================================================!
  subroutine calculate_element_load_vector(ic_type, nn, xx, yy, zz, rho, thick, ltype, &
                                          params, vect, nsize, iset, isect, hecMESH, &
                                          fstrSOLID, icel, ndof)
    integer(kind=kint), intent(in)       :: ic_type, nn, ltype, iset, isect, icel, ndof
    real(kind=kreal), intent(in)         :: xx(*), yy(*), zz(*), rho, thick, params(0:*)
    ...
    if (ic_type == 301) then
      ...
      call DL_C1(ic_type, nn, xx(1:nn), yy(1:nn), zz(1:nn), rho, thick, ltype, &
                params, vect(1:nn*ndof), nsize)
    
    ! 2D elements
    else if (ic_type == 241 .or. ic_type == 242 .or. ic_type == 231 .or. &
             ic_type == 232 .or. ic_type == 2322) then
      call DL_C2(ic_type, nn, xx(1:nn), yy(1:nn), rho, thick, ltype, &
                params, vect(1:nn*ndof), nsize, iset)
    
    ! 3D elements
    else if (ic_type == 341 .or. ic_type == 351 .or. ic_type == 361 .or. &
             ic_type == 342 .or. ic_type == 352 .or. ic_type == 362) then
      call DL_C3(ic_type, nn, xx(1:nn), yy(1:nn), zz(1:nn), rho, ltype, &
                params, vect(1:nn*ndof), nsize)
    ...
    ! Shell elements
      ...
      call DL_Shell(ic_type, nn, ndof, xx, yy, zz, rho, thick, ltype, params, &
                   vect, nsize, fstrSOLID%elements(icel)%gausses)
  ...
  end subroutine calculate_element_load_vector

  !======================================================================!
  !> Get original node coordinates
  !======================================================================!
  subroutine get_original_coordinates(hecMESH, is, nn, ndof, nodLocal, xx, yy, zz, iwk)
    type(hecmwST_local_mesh), intent(in) :: hecMESH
    integer(kind=kint), intent(in)       :: is, nn, ndof
    integer(kind=kint), intent(out)      :: nodLocal(*), iwk(*)
    real(kind=kreal), intent(out)        :: xx(*), yy(*), zz(*)
  ...
  end subroutine get_original_coordinates

  !======================================================================!
  !> Get deformed node coordinates
  !======================================================================!
  subroutine get_deformed_coordinates(hecMESH, fstrSOLID, is, nn, ndof, nodLocal, xx, yy, zz, iwk)
    type(hecmwST_local_mesh), intent(in) :: hecMESH
    type(fstr_solid), intent(in)         :: fstrSOLID
    integer(kind=kint), intent(in)       :: is, nn, ndof
    integer(kind=kint), intent(out)      :: nodLocal(*), iwk(*)
    ...
    do j = 1, nn
      nodLocal(j) = hecMESH%elem_node_item(is+j)
      ! Nodal coordinate with displacement
      if (ndof == 2) then
  ...
  end subroutine get_deformed_coordinates
  ...
  subroutine process_thermal_loads(cstep, ctime, hecMESH, hecMAT, fstrSOLID)
    ...
    if (fstrSOLID%TEMP_ngrp_tot > 0 .or. fstrSOLID%TEMP_irres > 0) then
      ! Set temperature from node groups
      ...
      do itype = 1, hecMESH%n_elem_type
        ...
        do icel = is, iE
          ! Get node IDs and coordinates
          is = hecMESH%elem_node_index(icel-1)
          do j = 1, nn
            nodLocal(j) = hecMESH%elem_node_item(is+j)
            
            ! Nodal coordinates with displacement
            if (ndof == 2) then
          ...
          ! Handle special case for beam elements
          if (ic_type == 641) then
            isect = hecMESH%section_ID(icel)
            ihead = hecMESH%section%sect_R_index(isect-1)
            
            call TLOAD_Beam_641(ic_type, nn, ndof, xx(1:nn), yy(1:nn), zz(1:nn), &
                               tt(1:nn), tt0(1:nn), &
                               fstrSOLID%elements(icel)%gausses, &
                               hecMESH%section%sect_R_item(ihead+1:), &
                               vect(1:nn*ndof))
  ...
  subroutine calculate_thermal_load(ic_type, nn, xx, yy, zz, tt, tt0, isect, ndof, &
    ...
    ! 3D elements - linear hex element with various formulations
      ...
      if (fstrSOLID%sections(isect)%elemopt361 == kel361FI) then
        ...
        call TLOAD_C3D8IC(ic_type, nn, xx(1:nn), yy(1:nn), zz(1:nn), tt(1:nn), tt0(1:nn), &
                         fstrSOLID%elements(icel)%gausses, vect(1:nn*ndof), cdsys_ID, coords)
      else if (fstrSOLID%sections(isect)%elemopt361 == kel361FBAR) then
        call TLOAD_C3D8Fbar(ic_type, nn, xx(1:nn), yy(1:nn), zz(1:nn), tt(1:nn), tt0(1:nn), &
                           fstrSOLID%elements(icel)%gausses, vect(1:nn*ndof), cdsys_ID, coords)
      endif
    ...
    ! Shell elements - not supported yet
    else if (ic_type == 741 .or. ic_type == 743 .or. ic_type == 731) then
      if (myrank == 0) then
        write(IMSG,*) '*------------------------', &
                     '-------------------*'
    ...
    ! Add thermal load vector to right-hand side
    do j = 1, ndof*nn
