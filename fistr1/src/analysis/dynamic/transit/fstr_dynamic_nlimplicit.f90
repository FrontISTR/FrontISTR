...
contains
  ...
  subroutine fstr_Newton_dynamic_contactSLag(cstep, hecMESH, hecMAT, fstrSOLID, fstrEIG, &
    ...
    allocate(resid_work(hecMESH%n_node*ndof + hecmw_mat_get_NP(conMAT)*ndof))
    ...
    count_step = count_step + 1
      ...
      do iter = 1, fstrSOLID%step_ctrl(cstep)%max_iter
        ...
        do j=1, hecMESH%n_node*  hecMESH%n_dof
          call hecmw_mat_set_B_i(hecMAT, j, hecmw_mat_get_B_i(hecMAT, j) - fstrSOLID%QFORCE(j) + fstrSOLID%DFORCE(j))
        enddo
