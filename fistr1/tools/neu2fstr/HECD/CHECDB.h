...
inline bool operator==(const checdb_id_class& a, const checdb_id_class& b) {
  return a.ID == b.ID;
}
inline bool operator<(const checdb_id_class& a, const checdb_id_class& b) {
  return a.ID < b.ID;
 ...
 public:
  ...
  class CElemItem : public checdb_id_class {
   public:
    ...
    CElemItem(int type, int id = -1) : checdb_id_class(id), node(0), node_n(0) {
      Init(type);
    }
    CElemItem(const CElemItem& e) : checdb_id_class(e.ID), node(0), node_n(0) {
      InitNode(e.node_n);
      for (int i = 0; i < node_n; i++) node[i] = e.node[i];
    }
    virtual ~CElemItem() { delete[] node; }
    void Init(int type) { InitNode(NodeNumber(type)); }
    void InitNode(int n) {
      delete[] node;
      node_n = n;
      node   = new int[node_n];
...
class CHECDB_Section : public CHECDataBlock {
 public:
  // type
  enum { TYPE_UNKNOWN = 0, TYPE_SOLID, TYPE_SHELL, TYPE_BEAM, TYPE_INTERFACE };
