...
const char *CFSTRDB_Film::LoadTypeName(int type) {
  const char *pn[] = {"F0", "F1", "F2", "F3", "F4", "F5", "F6", "unknown"};
...
CFSTRDB_Film::CFSTRDB_Film() : CFSTRDataBlock(FSTRDB_FILM), ItemList() {
  amp1[0] = 0;
  amp2[0] = 0;
}

CFSTRDB_Film::~CFSTRDB_Film() { Clear(); }
...
void CFSTRDB_Film::Write(CHECData *hecd) {
  ...
  size_t len = snprintf(buff, sizeof(buff), "!FILM");
