...
CNFData::CNFData() : version(DefaultCNFDataVersion), fp(0), line(0) {
  neu_file[0] = 0;
  title[0]    = 0;
  log_fp      = stdout;
}
...
#define GENRATE_CODE(x)                  \
  if (!CNFData::WriteDataBlock(fp, x)) { \
    fclose(fp);                          \
    throw CNFError(NFE_WRITEDATA_ERROR); \
  }
  GENRATE_CODE(100)
  GENRATE_CODE(402)
  GENRATE_CODE(403)
...
static void float_write(FILE *fp, double x) {
  fprintf(fp, "%lg,", x);
  /*******************************************
...
void CNFData::WriteBlockSeparator(FILE *fp) { fprintf(fp, "   -1\n"); }
...
#undef GENRATE_CODE
  ...
  for (iter = non_supported_block_list.begin();
       iter != non_supported_block_list.end(); iter++) {
    if (i == 10) {
      fprintf(fp, "\n");
      i = 0;
    }
