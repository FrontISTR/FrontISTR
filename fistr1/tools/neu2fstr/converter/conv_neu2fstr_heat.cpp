...
inline bool operator==(const cntemp &a, const cntemp &b) {
  return a.nid == b.nid;
}
inline bool operator<(const cntemp &a, const cntemp &b) {
  return a.nid < b.nid;
...
static void SetFixtemp(CNFData &neu, CHECData &hec) {
...
static void SetCFlux(CNFData &neu, CHECData &hec) {
...
static void SetDFlux(CNFData &neu, CHECData &hec) {
  ...
  for (iter = neu.DB_507.begin(); iter != neu.DB_507.end(); iter++) {
    ...
    for (siter = p->structural_load_list.begin();
      ...
      if (siter->loadtype != FEA_load_eHeatFlux &&
          siter->loadtype != FEA_load_eHeatGen)
        continue;
      ...
      if (siter->loadtype == FEA_load_eHeatFlux) {
        int fg_front;
        load_type =
            CFSTRDB_DFlux::TYPE_S0 + hec_face_no(hec_e_type, surf_no, fg_front);
...
static void SetFilm(CNFData &neu, CHECData &hec) {
...
static void SetRadiate(CNFData &neu, CHECData &hec) {
