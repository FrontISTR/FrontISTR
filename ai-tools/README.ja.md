...
**注意**: Windows上のVS Codeを使用し、WSL内で開発する場合は、WSL内で仮想環境の作成とパッケージのインストールを行ってください。
...
**注意**: WSL環境では`${workspaceFolder}`変数が使用できないため、上記の`/mnt/c/path/to/your/workspace`部分を実際のワークスペースのWSLパスに置き換えてください。
